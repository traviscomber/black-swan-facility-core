...
export type ItDataHealthSnapshot = {
  ...
  vehicles: { total: number; with_missing_fields: number; canonical_identity_present: number; external_identifier_unrecorded: number; missing_classification: number; missing_cost_center: number; missing_responsible_team: number }
...
const COPY = {
  en: {
    ...
    booking: 'Bookings', bookingDescription: 'Latest scheduled canonical booking-health execution.', noRun: 'No booking-health run has been recorded.', runs: 'Runs', checks: 'Checks', passed: 'Passed', warnings: 'Warnings', critical: 'Critical', latest: 'Latest run',
    vehicles: 'Vehicle registry', vehiclesDescription: 'Canonical identity is the internal registry code. External identifiers remain explicit and are never inferred for vehicles or equipment.', total: 'Total', incomplete: 'With tracked gaps', canonicalIdentity: 'Canonical ID present', externalIdentifier: 'External ID unrecorded', classification: 'Classification missing', costCenter: 'Cost center missing', team: 'Responsible team missing',
  ...
  es: {
    ...
    booking: 'Reservas', bookingDescription: 'Última ejecución canónica programada de booking health.', noRun: 'No hay ninguna ejecución de booking health registrada.', runs: 'Ejecuciones', checks: 'Checks', passed: 'Aprobados', warnings: 'Advertencias', critical: 'Críticos', latest: 'Última ejecución',
    vehicles: 'Registro de vehículos', vehiclesDescription: 'La identidad canónica es el código interno del registro. Los identificadores externos se muestran por separado y nunca se infieren para vehículos o equipos.', total: 'Total', incomplete: 'Con brechas registradas', canonicalIdentity: 'ID canónico presente', externalIdentifier: 'ID externo no registrado', classification: 'Sin clasificación', costCenter: 'Sin centro de costo', team: 'Sin equipo responsable',
  ...
  de: {
    ...
    booking: 'Reservierungen', bookingDescription: 'Letzte geplante kanonische Ausführung der Booking-Prüfung.', noRun: 'Es wurde noch keine Booking-Prüfung aufgezeichnet.', runs: 'Ausführungen', checks: 'Prüfungen', passed: 'Bestanden', warnings: 'Warnungen', critical: 'Kritisch', latest: 'Letzte Ausführung',
    vehicles: 'Fahrzeugregister', vehiclesDescription: 'Die kanonische Identität ist der interne Registercode. Externe Kennungen bleiben separat sichtbar und werden für Fahrzeuge oder Geräte nicht abgeleitet.', total: 'Gesamt', incomplete: 'Mit erfassten Lücken', canonicalIdentity: 'Kanonische ID vorhanden', externalIdentifier: 'Externe ID nicht erfasst', classification: 'Klassifizierung fehlt', costCenter: 'Kostenstelle fehlt', team: 'Verantwortliches Team fehlt',
...
export function ItDataHealth({ snapshot }: { snapshot: ItDataHealthSnapshot | null }) {
  ...
  return <section className="space-y-4 px-4 pb-8 md:px-8">
    ...
    {!snapshot ? <Card><CardContent className="py-6 text-sm text-muted-foreground">{text.unavailable}</CardContent></Card> : <div className="grid gap-4 xl:grid-cols-2">
      ...
      <HealthCard icon={CarFront} title={text.vehicles} description={text.vehiclesDescription}><Metrics values={[[text.total,snapshot.vehicles.total],[text.canonicalIdentity,snapshot.vehicles.canonical_identity_present],[text.externalIdentifier,snapshot.vehicles.external_identifier_unrecorded],[text.classification,snapshot.vehicles.missing_classification],[text.costCenter,snapshot.vehicles.missing_cost_center],[text.team,snapshot.vehicles.missing_responsible_team],[text.incomplete,snapshot.vehicles.with_missing_fields]]} number={number}/></HealthCard>
