...
export function ActivitiesTimeline({ activities, activityTypes, dates, onEdit }: { activities: Activity[]; activityTypes: ActivityType[]; dates: Date[]; onEdit: (activity: Activity) => void }) {
          ...
          <div style={{ minWidth: LABEL_WIDTH + width }}>
            <div className="sticky top-0 z-20 flex border-b bg-background" style={{ height: ROW_HEIGHT }}>
              ...
              <div className="relative flex" style={{ width }}>
                {dates.map((date) => <div key={date.toISOString()} className={`flex shrink-0 items-center justify-center border-r text-[10px] ${isSameDay(date, new Date()) ? 'bg-amber-50 font-bold' : 'text-muted-foreground'}`} style={{ width: DAY_WIDTH }}>{format(date, 'dd MMM')}</div>)}
              </div>
            </div>

            {rows.length === 0 && <div className="p-8 text-center text-sm text-muted-foreground">No activities match the selected filters.</div>}
            {rows.map((row) => <div key={row.id} className="flex border-b" style={{ height: ROW_HEIGHT }}>
              ...
              <div className="relative" style={{ width, backgroundImage: `repeating-linear-gradient(to right, transparent 0, transparent ${DAY_WIDTH - 1}px, hsl(var(--border)) ${DAY_WIDTH - 1}px, hsl(var(--border)) ${DAY_WIDTH}px)` }}>
                {row.events.map((raw) => {
                  const event = raw as unknown as Activity & { left: number; width: number }
