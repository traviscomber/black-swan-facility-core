...
import { AlertTriangle, ArrowLeft, Banknote, BedDouble, CalendarCheck, ClipboardCheck, FileText, History, Loader2, MessageSquareText, ShoppingCart, UserRound, Wrench } from "lucide-react"
...
export function BookingReservationStayCockpit({ reservation }: { reservation: Reservation }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [room, guest, payments, invoices, housekeeping, maintenance, hospitality, extras, guestRequests, shopping, issues, exceptions, documents, events, logistics, handoverItems, activityBookings] = await Promise.all([
        ...
        safe(supabase.from("reservation_activity_bookings").select("id,attendee_count,status,unit_price,total_amount,transport_required,pickup_location,notes,activity:activities(id,title,start_date,start_time,end_date,end_time,location,status)").eq("reservation_id",reservation.id).order("created_at",{ascending:false}).limit(10), []),
      ])
      if (alive) { setRelated({room,guest,payments,invoices,housekeeping,maintenance,hospitality,extras,guestRequests,shopping,issues,exceptions,documents,events,logistics,handoverItems,activityBookings}); setLoading(false) }
    }
  ...
  return <div data-stay-cockpit className="min-h-screen bg-[#171512] text-[#e7e1d8]">
    ...
    <div className="grid gap-2 p-3 xl:grid-cols-2 2xl:grid-cols-3">
      ...
      <Panel title={c.activities} icon={<CalendarCheck className="h-3.5 w-3.5"/>}>{(related?.activityBookings ?? []).map((x:any)=>{const activity=Array.isArray(x.activity)?x.activity[0]:x.activity;const detail=[activity?.start_date ? `${activity.start_date}${activity.start_time ? ` · ${activity.start_time.slice(0,5)}` : ""}` : null,activity?.location,x.attendee_count ? `${x.attendee_count} ${c.attendees}` : null,x.transport_required ? c.transport : null,x.pickup_location ? `${c.pickup}: ${x.pickup_location}` : null,x.total_amount ? money(x.total_amount) : null].filter(Boolean).join(" · ");return <Row key={x.id} title={activity?.title || c.activities} detail={detail || x.notes} status={x.status || activity?.status}/>})}{!loading && !related?.activityBookings.length && <Row title={c.empty}/>}</Panel>
