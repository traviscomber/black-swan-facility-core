...
export function BookingReservationStayCockpit({ reservation }: { reservation: Reservation }) {
  ...
  return <div data-stay-cockpit className="min-h-screen bg-[#171512] text-[#e7e1d8]">
    ...
    <div className="grid gap-2 p-3 xl:grid-cols-2 2xl:grid-cols-3">
      ...
      <Panel title={c.finance} icon={<Banknote className="h-3.5 w-3.5"/>}><Row title={c.total} detail={`${money(reservation.total_amount)} · ${c.paid}: ${money(paid)} · ${c.balance}: ${money(balance)}`} status={reservation.payment_status}/>{(related?.payments ?? []).map((x:any)=><Row key={`p-${x.id}`} title={`${money(x.amount)} · ${x.payment_method || "payment"}`} detail={x.paid_at || x.created_at} status={x.payment_status}/>)}{(related?.invoices ?? []).map((x:any)=><Row key={`i-${x.id}`} title={x.invoice_number || c.invoice} detail={money(x.total_amount)} status={x.payment_status || x.status}/>)}{!loading && !(related?.payments.length || related?.invoices.length) && <Row title={c.empty}/>}</Panel>
