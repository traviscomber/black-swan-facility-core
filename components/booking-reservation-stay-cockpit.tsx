...
export function BookingReservationStayCockpit({ reservation }: { reservation: Reservation }) {
  ...
  return <div data-stay-cockpit className="min-h-screen bg-[#171512] text-[#e7e1d8]">
    ...
    <div className="grid gap-2 p-3 xl:grid-cols-2 2xl:grid-cols-3">
      ...
      {!historicalImported && <><Panel title={c.housekeeping} icon={<ClipboardCheck className="h-3.5 w-3.5"/>}>{(related?.housekeeping ?? []).map((x:any)=><Row key={x.id} title={operationalLabel(x.task_type || "Housekeeping", language)} detail={x.service_date} status={x.status}/>)}{!loading && !related?.housekeeping.length && <Row title={c.empty}/>}</Panel>
