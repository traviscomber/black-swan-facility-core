...
export function BookingReservationStayCockpit({ reservation }: { reservation: Reservation }) {
  ...
  return <div data-stay-cockpit className="min-h-screen bg-[#171512] text-[#e7e1d8]">
    <header className="sticky top-0 z-20 flex min-h-[64px] flex-wrap items-center justify-between gap-4 border-b border-white/[.055] bg-[#211e1a]/95 px-5 py-3 backdrop-blur-sm"><div className="min-w-0"><div className="mb-1.5 flex items-center gap-3"><Link href={`/${language}/bookings/calendar`} className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[.07em] text-[#8f867b] hover:text-[#e7e1d8]"><ArrowLeft className="h-3.5 w-3.5"/>{c.back}</Link><span className="text-[9px] uppercase tracking-[.1em] text-[#6f675f]">{c.object}</span></div><h1 className="truncate text-[18px] font-medium leading-6 tracking-[-.015em]">{reservation.guest_name || guest?.name || c.noGuest}</h1><p className="mt-0.5 text-[11px] text-[#8f867b]">{reservation.check_in} → {reservation.check_out} · {reservation.num_guests ?? 1} {c.guests}{reservation.source ? ` · ${c.source}: ${operationalLabel(reservation.source, language)}` : ""}</p></div><div className="flex shrink-0 items-center gap-3"><button type="button" onClick={() => setInvoiceOpen(true)} className="inline-flex h-8 items-center gap-2 border border-white/10 bg-[#2b2722] px-3 text-[10px] font-medium uppercase tracking-[.07em] text-[#e7e1d8] hover:bg-[#332e28]"><ReceiptText className="h-3.5 w-3.5"/>{c.createInvoice}</button><div className="text-right"><div className={`text-[9px] font-medium uppercase tracking-[.09em] ${statusClass(reservation.arrival_status || reservation.status)}`}>{reservation.arrival_status || reservation.status || "—"}</div><div className="mt-1 text-lg font-medium tracking-[-.02em] text-[#e7e1d8]">{knownMoney(reservation.total_amount)}</div></div></div></header>
    ...
    <div className="grid gap-2 p-3 xl:grid-cols-2 2xl:grid-cols-3">
      ...
      <Panel title={c.housekeeping} icon={<ClipboardCheck className="h-3.5 w-3.5"/>}>{(related?.housekeeping ?? []).map((x:any)=><Row key={x.id} title={operationalLabel(x.task_type || "Housekeeping", language)} detail={x.service_date} status={x.status}/>)}{!loading && !related?.housekeeping.length && <Row title={c.empty}/>}</Panel>
      ...
      <Panel title={c.activities} icon={<History className="h-3.5 w-3.5"/>}>{(related?.activityBookings ?? []).map((x:any)=>{const activity=x.activity;const detail=[activity?.start_date ? `${activity.start_date}${activity.start_time ? ` · ${String(activity.start_time).slice(0,5)}` : ""}` : null,activity?.location,x.attendee_count ? `${x.attendee_count} ${c.attendees}` : null,x.transport_required ? c.transport : null,x.pickup_location ? `${c.pickup}: ${x.pickup_location}` : null,x.total_amount != null ? money(x.total_amount) : null].filter(Boolean).join(" · ");return <Row key={x.id} title={activity?.title || c.activities} detail={detail || x.notes} status={x.status || activity?.status}/>})}{!loading && !related?.activityBookings.length && <Row title={c.empty}/>}</Panel>
    ...
    <InvoiceEditorModal open={invoiceOpen} onOpenChange={setInvoiceOpen} reservationId={reservation.id} guestName={reservation.guest_name || ""} guestEmail={reservation.guest_email || ""} guestPhone={reservation.guest_phone || ""} />
  </div>
}
