...
export function BookingReservationStayCockpit({ reservation }: { reservation: Reservation }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [room, guest, payments, invoices, housekeeping, maintenance, hospitality, extras, guestRequests, shopping, issues, exceptions, documents, events] = await Promise.all([
      ...
      if (alive) { setRelated({room,guest,payments,invoices,housekeeping,maintenance,hospitality,extras,guestRequests,shopping,issues,exceptions,documents,events}); setLoading(false) }
    }
