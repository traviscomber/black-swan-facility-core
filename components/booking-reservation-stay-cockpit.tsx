...
import { AlertTriangle, ArrowLeft, Banknote, BedDouble, ClipboardCheck, FileText, History, Loader2, MessageSquareText, ShoppingCart, UserRound, Wrench } from "lucide-react"
...
export function BookingReservationStayCockpit({ reservation }: { reservation: Reservation }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [room, guest, payments, invoices, housekeeping, maintenance, hospitality, extras, guestRequests, shopping, issues, exceptions, documents, events, logistics, handoverItems] = await Promise.all([
      ...
      if (alive) { setRelated({room,guest,payments,invoices,housekeeping,maintenance,hospitality,extras,guestRequests,shopping,issues,exceptions,documents,events,logistics,handoverItems}); setLoading(false) }
    }
  ...
  const balance = useMemo(() => (related?.invoices ?? []).reduce((sum:number,x:any)=>sum+Math.max(0,Number(x.total_amount??0)-Number(x.amount_paid??0)),0), [related])
  ...
  return <div data-stay-cockpit className="min-h-screen bg-[#171512] text-[#e7e1d8]">
    ...
    <div className="grid gap-2 p-3 xl:grid-cols-2 2xl:grid-cols-3">
      ...
      <Panel title={c.guest} icon={<UserRound className="h-3.5 w-3.5"/>}>{guest ? <><Row title={`${guest.name || reservation.guest_name || c.noGuest}${guest.vip_status ? " · VIP" : ""}`} detail={`${guest.email || reservation.guest_email || "—"} · ${guest.phone || reservation.guest_phone || "—"}`}/>{guest.allergies && <Row title={c.allergies} detail={guest.allergies}/>} {guest.mobility_requirements && <Row title={c.mobility} detail={guest.mobility_requirements}/>} {guest.housekeeping_preferences && <Row title={c.housekeepingPref} detail={guest.housekeeping_preferences}/>}</> : <Row title={c.noGuest} detail={`${reservation.guest_email || "—"} · ${reservation.guest_phone || "—"}`}/>}</Panel>
      ...
      <Panel title={c.shopping} icon={<ShoppingCart className="h-3.5 w-3.5"/>}><div className="mb-2 flex justify-end"><Link href={`/${language}/bookings/shopping-list?reservation_id=${reservation.id}`} className="text-[10px] uppercase tracking-[.07em] text-[#9db69f] hover:text-[#e7e1d8]">{c.openList}</Link></div>{(related?.shopping ?? []).map((x:any)=><Row key={x.id} title={`${x.item_name} × ${x.quantity} ${x.unit}`} detail={`${x.source_strategy}${x.required_date ? ` · ${x.required_date}` : ""}`} status={x.status}/>)}{!loading && !related?.shopping.length && <Row title={c.empty}/>}</Panel>
      <Panel title={c.logistics} icon={<History className="h-3.5 w-3.5"/>}>{[...(related?.logistics ?? []).map((x:any)=>({id:`l-${x.id}`,title:x.direction==="arrival"?c.arrival:c.departure,detail:[x.hub,x.anchor_at,x.notes].filter(Boolean).join(" · "),status:x.status})),...(related?.handoverItems ?? []).map((x:any)=>({id:`h-${x.id}`,title:x.title,detail:[x.detail,x.due_at].filter(Boolean).join(" · "),status:x.status}))].map((x:any)=><Row key={x.id} title={x.title} detail={x.detail} status={x.status}/>)}{!loading && !((related?.logistics.length??0)+(related?.handoverItems.length??0)) && <Row title={c.noLogistics}/>}</Panel>
      ...
      <Panel title={c.issues} icon={<AlertTriangle className="h-3.5 w-3.5"/>}>{(related?.issues ?? []).map((x:any)=><Row key={x.id} title={x.title || x.category || c.issue} detail={x.description} status={x.status}/>)}{!loading && !related?.issues.length && <Row title={c.empty}/>}</Panel>
      <Panel title={c.documents} icon={<FileText className="h-3.5 w-3.5"/>}>{(related?.documents ?? []).map((x:any)=><Row key={x.id} title={x.title || x.document_number || x.document_type || c.document} detail={x.issued_at} status={x.status}/>)}{!loading && !related?.documents.length && <Row title={c.empty}/>}</Panel>
      ...
      <Panel title={c.activity} icon={<History className="h-3.5 w-3.5"/>}>{(related?.events ?? []).map((x:any)=><Row key={x.id} title={x.title || x.event_type || c.event} detail={`${x.occurred_at}${x.description ? ` · ${x.description}` : ""}`} status={x.new_state}/>)}{!loading && !related?.events.length && <Row title={c.empty}/>}</Panel>
    </div>
  </div>
}
