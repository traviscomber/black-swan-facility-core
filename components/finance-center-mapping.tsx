...
export function FinanceCenterMapping() {
  ...
  async function save(row: CenterRow) {
    ...
    const { data, error } = await supabase.rpc('map_finance_historical_center', {
      ...
      p_note: 'Mapeo canónico validado desde bandeja financiera',
    })
    if (error) toast.error(error.message)
    else {
      const result = data as { documents_updated?: number; rules_promoted?: number } | null
      toast.success(`Mapeo guardado · ${result?.documents_updated ?? 0} documentos · ${result?.rules_promoted ?? 0} reglas actualizadas.`)
      ...
      <div className="p-5 md:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--bs-text-secondary)]">Solo se muestran centros P&L y categorías de costo provenientes del Budget canónico. Categorías antiguas e Income quedan fuera de este flujo.</p>
          </div>
        ...
        <div className="mt-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="relative w-full max-w-md"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-[var(--bs-text-muted)]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar centro histórico" className="h-10 w-full bg-[var(--bs-surface-secondary)] pl-10 pr-3 text-sm text-[var(--bs-text-primary)] outline-none placeholder:text-[var(--bs-text-muted)]" /></div>
        ...
        <table className="w-full min-w-[980px] text-sm">
          ...
          <tbody>
            {visibleRows.map((row) => {
              ...
              const isMapped = row.mapping_status === 'mapped' && Boolean(row.category_id)
              return <tr key={row.id} className="border-t border-[var(--bs-divider-subtle)] align-middle">
                ...
                <td className="px-5 py-4"><select value={choice.category} disabled={!choice.division} onChange={(event) => setDraft((current) => ({ ...current, [row.id]: { ...choice, category: event.target.value } }))} className="h-10 min-w-64 bg-[var(--bs-surface-secondary)] px-3 text-sm text-[var(--bs-text-primary)] disabled:opacity-40"><option value="">Seleccionar categoría de costo</option>{availableCategories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></td>
