...
export function IssueObjectView({ issue, asset, reservation, tasks, maintenance, partial }: { issue: IssueObject; asset: RelatedAsset | null; reservation: RelatedReservation | null; tasks: RelatedTask[]; maintenance: RelatedMaintenance[]; partial: boolean }) {
  ...
  return <AppLayout><div className="space-y-6 p-4 md:p-6">
    ...
    <section className="space-y-3"><div><h2 className="text-lg font-semibold">{copy.cause}</h2><p className="text-sm text-muted-foreground">{copy.causeDetail}</p></div><div className="grid gap-4 lg:grid-cols-2">
      ...
      <RelationCard icon={ClipboardList} title={copy.tasks}>{tasks.length ? <div className="space-y-2">{tasks.map((task) => <ObjectLink key={task.id} href={`/tasks?selected=${task.id}`} title={task.title} detail={[task.status, task.priority, task.due_date ? `${copy.due} ${formatDate(task.due_date)}` : null].filter(Boolean).join(' · ')} />)}</div> : <Empty text={copy.noTasks} />}</RelationCard>
      <RelationCard icon={Wrench} title={copy.maintenance}>{maintenance.length ? <div className="space-y-2">{maintenance.map((item) => <ObjectLink key={item.id} href={`/maintenance/${item.id}`} title={item.title} detail={[item.bloqueado ? copy.blocked : null, item.status, item.prioridad, item.fecha_objetivo ? `${copy.due} ${formatDate(item.fecha_objetivo)}` : null].filter(Boolean).join(' · ')} />)}</div> : <Empty text={copy.noMaintenance} />}</RelationCard>
    </div></section>
  </div></AppLayout>
}
