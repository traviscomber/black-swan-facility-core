...
function isHistoricalAsanaSnapshot(task: Task) {
...
export function TaskDetailPanel({ task, onUpdate, onClose, onEdit }: { task: Task; onUpdate: () => void; onClose: () => void; onEdit: (task: Task) => void }) {
  ...
  const readOnlyAsanaSnapshot = isHistoricalAsanaSnapshot(task)
  ...
  async function saveStatus() {
    if (readOnlyAsanaSnapshot || status === task.status) return
    setSaving(true)
  ...
  async function addComment() {
    if (readOnlyAsanaSnapshot) return
  ...
  return <div className="space-y-5 p-5 sm:p-6">
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs text-muted-foreground">Detalle de tarea</p><h2 className="mt-1 text-xl font-semibold">{task.title}</h2></div><div className="flex gap-2">{!readOnlyAsanaSnapshot && <Button variant="outline" size="icon" onClick={() => onEdit(task)} aria-label="Editar tarea"><Pencil className="h-4 w-4" /></Button>}<Button variant="ghost" size="icon" onClick={onClose} aria-label="Cerrar detalle"><X className="h-4 w-4" /></Button></div></div>
    ...
    {readOnlyAsanaSnapshot && <Card className="border-amber-400/40 bg-amber-500/5"><CardContent className="p-4"><p className="text-sm font-medium">Snapshot histórico de Asana · sólo lectura</p><p className="mt-1 text-sm text-muted-foreground">Este registro conserva evidencia histórica importada. Su estado, responsable y fecha actuales deben verificarse en Asana en vivo antes de usarse como verdad operacional. No se puede editar, comentar, adjuntar evidencia, notificar ni cambiar de estado desde este panel.</p></CardContent></Card>}
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Users className="h-4 w-4" />Responsables{readOnlyAsanaSnapshot ? " observados" : " y WhatsApp"}</CardTitle></CardHeader><CardContent>{task.task_assignments.length === 0 ? <p className="text-sm text-muted-foreground">Sin responsables asignados.</p> : <ul className="space-y-3">{task.task_assignments.map((assignment, index) => { const person = assignment.employees ?? assignment.volunteers; const kind = assignment.volunteer_id ? "Voluntario" : "Trabajador"; return <li key={assignment.employee_id ?? assignment.volunteer_id ?? index} className="flex flex-col gap-2 rounded-md border p-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0 text-sm"><p className="font-medium">{person?.name ?? "Persona no disponible"}</p><p className="truncate text-xs text-muted-foreground">{kind}{person?.phone && !readOnlyAsanaSnapshot ? ` · ${person.phone}` : ""}</p></div>{person?.phone && !readOnlyAsanaSnapshot && <Button asChild variant="outline" size="sm"><a href={taskNotificationHref(person.name, person.phone)} target="_blank" rel="noreferrer"><MessageCircle className="mr-2 h-4 w-4" />Enviar por WhatsApp</a></Button>}</li> })}</ul>}{readOnlyAsanaSnapshot ? <p className="mt-3 text-xs text-muted-foreground">Los responsables de este snapshot son referencia histórica y no habilitan notificaciones.</p> : <p className="mt-3 text-xs text-muted-foreground">Proveedor activo: WhatsApp Web, con confirmación manual. {notificationCapabilities.futureProvider === "greenapi" ? "GreenAPI podrá incorporarse después sin cambiar esta pantalla." : null}</p>}</CardContent></Card>

    <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Camera className="h-4 w-4" />Evidencia de ejecución</CardTitle></CardHeader><CardContent className="space-y-4">{!readOnlyAsanaSnapshot && <><div className="grid gap-3 sm:grid-cols-[1fr_auto]"><Input value={caption} onChange={(event) => setCaption(event.target.value)} placeholder="Descripción opcional de la evidencia" /><label className="inline-flex cursor-pointer items-center justify-center rounded-md border bg-background px-4 py-2 text-sm font-medium hover:bg-muted"><input type="file" accept="image/*,application/pdf" className="sr-only" onChange={uploadEvidence} disabled={uploading} />{uploading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Upload className="mr-2 h-4 w-4" />}Adjuntar</label></div><p className="text-xs text-muted-foreground">Imágenes o PDF, máximo 10 MB. Los archivos se guardan en un contenedor privado.</p></>}{readOnlyAsanaSnapshot && <p className="text-xs text-muted-foreground">La evidencia asociada, si existe, se muestra sólo como historial.</p>}{loadingActivity ? <Loader2 className="h-5 w-5 animate-spin" /> : evidence.length === 0 ? <p className="text-sm text-muted-foreground">Aún no hay evidencia adjunta.</p> : <div className="grid gap-3 sm:grid-cols-2">{evidence.map((item) => <a key={item.id} href={item.signed_url || "#"} target="_blank" rel="noreferrer" className="rounded-md border p-3 hover:bg-muted/40"><div className="flex items-start gap-3">{item.mime_type?.startsWith("image/") ? <Camera className="mt-0.5 h-4 w-4" /> : <FileText className="mt-0.5 h-4 w-4" />}<div className="min-w-0"><p className="truncate text-sm font-medium">{item.file_name}</p>{item.caption && <p className="mt-1 text-xs text-muted-foreground">{item.caption}</p>}<p className="mt-1 text-xs text-muted-foreground">{formatTimestamp(item.created_at)}{item.uploader_email ? ` · ${item.uploader_email}` : ""}</p></div></div></a>)}</div>}</CardContent></Card>

    <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><MessageCircle className="h-4 w-4" />Comentarios y novedades</CardTitle></CardHeader><CardContent className="space-y-4">{!readOnlyAsanaSnapshot && <div className="space-y-2"><Textarea value={comment} onChange={(event) => setComment(event.target.value)} rows={3} maxLength={2000} placeholder="Registrar avance, novedad, bloqueo o resultado" /><Button onClick={addComment} disabled={savingComment || !comment.trim()}>{savingComment ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />}Guardar comentario</Button></div>}{readOnlyAsanaSnapshot && <p className="text-xs text-muted-foreground">Los comentarios existentes se conservan como historial; este snapshot no acepta nuevos comentarios.</p>}{loadingActivity ? <Loader2 className="h-5 w-5 animate-spin" /> : comments.length === 0 ? <p className="text-sm text-muted-foreground">Aún no hay comentarios.</p> : <div className="space-y-3">{comments.map((item) => <div key={item.id} className="rounded-md border p-3"><p className="whitespace-pre-wrap text-sm">{item.comment}</p><p className="mt-2 text-xs text-muted-foreground">{formatTimestamp(item.created_at)}{item.author_email ? ` · ${item.author_email}` : ""}</p></div>)}</div>}</CardContent></Card>

    {readOnlyAsanaSnapshot ? <Card><CardHeader><CardTitle className="text-base">Estado histórico</CardTitle></CardHeader><CardContent><div className="flex items-center justify-between gap-3"><Badge variant="outline">{statusLabels[task.status]}</Badge><span className="text-xs text-muted-foreground">No editable</span></div><p className="mt-3 text-xs text-muted-foreground">El valor mostrado pertenece al snapshot importado y no sustituye el estado vivo de Asana.</p></CardContent></Card> : <Card><CardHeader><CardTitle className="text-base">Actualizar estado</CardTitle></CardHeader><CardContent className="space-y-3"><Select value={status} onValueChange={(value: TaskStatus) => setStatus(value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="nueva">Pendiente</SelectItem><SelectItem value="en_progreso">En curso</SelectItem><SelectItem value="completada">Completada</SelectItem><SelectItem value="cancelada">Cancelada</SelectItem></SelectContent></Select><Button className="w-full" onClick={saveStatus} disabled={saving || status === task.status}>{saving ? "Guardando…" : "Guardar estado"}</Button><p className="text-xs text-muted-foreground">Las tareas se completan o cancelan para conservar trazabilidad. No se eliminan desde esta vista.</p></CardContent></Card>}
  </div>
}
