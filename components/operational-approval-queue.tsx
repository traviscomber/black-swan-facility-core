...
export function OperationalApprovalQueue() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("id, action_key, title, summary, source_type, requested_by_name, priority, reason, created_at")
      .eq("status", "pending")
  ...
  async function decide(id: string, decision: "approved" | "rejected") {
    ...
    else toast.success(decision === "approved" ? "Acción aprobada" : "Acción rechazada")
    setSavingId(null)
    await load()
  }
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)]">Acciones preparadas para aprobación rápida</p>
          </div>
        </div>
