  ...
  useEffect(() => {
    ...
    for (const o of objects.filter((x) => area(x.object_type))) {
      ...
      beds.forEach((bed, index) => {
        ...
        const bedLayer = L.polygon(points, { color: hasPlan ? "#d7b17a" : "#8f8a81", weight: active ? 1.35 : .9, opacity: hasPlan ? .96 : .72, fillColor: hasPlan ? "#d7b17a" : "#5d5a54", fillOpacity: hasPlan ? .24 : .08, interactive: true })
        bedLayer.bindTooltip?.(`<b>${safe(bedCode(bed))}</b><span>${safe(bedPlanText(bed, text.noPlan))}</span>`, { permanent: false, sticky: true, direction: "top", opacity: 1, className: `orchard-bed-hover${hasPlan ? "" : " orchard-bed-hover-empty"}` })
