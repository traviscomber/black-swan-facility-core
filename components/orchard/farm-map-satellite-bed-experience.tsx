...
import { cropFamilyColor, normalizeCropIdentity } from "@/lib/orchard/crop-visual-identity"
...
type BedAllocation = { crop: string; family: string | null; start: string | null; end: string | null; lengthM: number }
...
function primaryBedAllocation(bed: BedPlan) {
  if (!bed.allocations.length) return null
  const firstStart = bed.allocations.reduce<string | null>((earliest, a) => !earliest || (a.start && a.start < earliest) ? a.start : earliest, null)
  const firstPhase = bed.allocations.filter((a) => (a.start ?? null) === firstStart)
  return [...firstPhase].sort((a, b) => b.lengthM - a.lengthM || a.crop.localeCompare(b.crop))[0] ?? bed.allocations[0]
}
...
export function FarmMapSatelliteExperience() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      supabase.from("orchard_crop_library").select("crop_name,crop_family"),
    ]).then((rs) => {
      ...
      const familyByCrop = new Map(((rs[5].data ?? []) as { crop_name: string; crop_family: string | null }[]).map((x) => [normalizeCropIdentity(x.crop_name), x.crop_family]))
      ...
      for (const a of (rs[2].data ?? []) as { bed_id: string; crop_succession_id: string; allocated_length_m: number | string | null; planned_start_date: string | null; planned_end_date: string | null }[]) {
        ...
        list.push({ crop, family: familyByCrop.get(normalizeCropIdentity(crop)) ?? null, start: a.planned_start_date, end: a.planned_end_date, lengthM: num(a.allocated_length_m) })
  ...
  useEffect(() => {
    ...
    void import("leaflet").then((m) => {
      ...
      leaflet.current = L
      map = L.map(node.current, { zoomControl: false, attributionControl: true, minZoom: 4, maxZoom: 19 }).setView(CENTER, 18)
      handleZoom = () => { if (map) setZoom(map.getZoom()) }
      mapRef.current = map
      map.on("zoomend", handleZoom)
      L.control.zoom({ position: "bottomleft" }).addTo(map)
      let fails = 0
      const base = L.tileLayer(ESRI, { maxZoom: 19, maxNativeZoom: 18, tileSize: 256, attribution: "Imagery © Esri and contributors" })
  ...
  useEffect(() => {
    ...
    for (const o of objects.filter((x) => area(x.object_type))) {
      ...
      const layer = L.polygon(start, { color: active ? "#bde1cf" : "#f1eee7", weight: active ? 3.2 : detail ? 1.7 : 2.2, opacity: 1, fillColor: active ? "#91c9ae" : "#d7b17a", fillOpacity: detail ? .045 : active ? .22 : .14, className: "cursor-move" })
  ...
  return <main ref={root} className="relative h-[calc(100dvh-var(--orchard-nav-height,0px))] min-h-[620px] w-full overflow-hidden bg-[#11110f]">
    ...
    <style jsx global>{`.orchard-label{background:rgba(18,18,16,.90)!important;border:1px solid rgba(241,238,231,.82)!important;border-radius:0!important;color:#f1eee7!important;padding:5px 7px!important;text-align:center!important;box-shadow:0 6px 18px rgba(0,0,0,.35)!important}.orchard-label:before,.orchard-bed-hover:before{display:none!important}.orchard-label b{display:block;font-size:11px;line-height:1.1}.orchard-label span{display:block;margin-top:3px;color:#d7b17a;font-size:8px;font-weight:600;letter-spacing:.07em;line-height:1.1;text-transform:uppercase;white-space:nowrap}.orchard-label-active{border-color:#bde1cf!important}.orchard-label-active span{color:#bde1cf}.orchard-bed-hover{background:rgba(18,18,16,.95)!important;border:1px solid rgba(241,238,231,.42)!important;border-radius:0!important;color:#f1eee7!important;padding:5px 6px!important;text-align:left!important;box-shadow:0 5px 14px rgba(0,0,0,.36)!important}.orchard-bed-hover b{display:flex;align-items:center;gap:5px;color:#bde1cf;font-size:9px;line-height:1}.orchard-bed-hover b i{display:inline-block;width:7px;height:7px}.orchard-bed-hover span{display:block;margin-top:3px;max-width:180px;color:#e7d8bd;font-size:8px;font-weight:600;letter-spacing:.035em;line-height:1.25;text-transform:uppercase;white-space:normal}.orchard-bed-hover-empty{border-color:rgba(143,138,129,.55)!important}.orchard-bed-hover-empty span{color:#8f8a81!important}`}</style>
    ...
    {block && !drawer ? <aside className="absolute right-4 top-[70px] z-30 w-[360px] max-w-[calc(100%-32px)] border border-white/15 bg-[#171512]/97"><div className="flex justify-between border-b border-white/10 p-4"><div><div className="text-sm text-[#f1eee7]">{block.name}</div><div className="text-[11px] text-[#8f8a81]">{text.field} · {selectedBeds.length} {text.beds.toLowerCase()}</div></div><button onClick={() => setSelected(null)} className="border border-[#91c9ae]/40 bg-[#173328] px-3 text-xs text-[#bde1cf]">{text.done}</button></div>{summary ? <div className="border-b border-white/10 px-4 py-3"><div className="text-[10px] uppercase tracking-[.12em] text-[#91c9ae]">{text.season}{summary.season ? ` · ${summary.season}` : ""}</div><div className="mt-1 text-xs leading-5 text-[#e8e5dc]">{summary.crops.join(" · ")}</div></div> : null}<div className="border-b border-white/10 px-4 py-3"><div className="text-[10px] uppercase tracking-[.12em] text-[#91c9ae]">{text.bedDetail} · zoom {BED_DETAIL_ZOOM}</div><div className="mt-1 text-[11px] leading-4 text-[#aaa69c]">{zoom >= BED_DETAIL_ZOOM ? text.bedHint : `${text.beds}: zoom ${BED_DETAIL_ZOOM}`}</div></div>{zoom >= BED_DETAIL_ZOOM && selectedBeds.length ? <div className="max-h-[280px] overflow-y-auto border-b border-white/10 px-4 py-2">{selectedBeds.map((bed) => { const primary = primaryBedAllocation(bed), swatch = primary ? cropFamilyColor(primary.family) : "#5d5a54"; return <div key={bed.id} className="grid grid-cols-[10px_34px_1fr] gap-2 border-b border-white/[.06] py-2 last:border-b-0"><span className="mt-[3px] h-2 w-2" style={{ background: swatch }} /><div className="text-[10px] font-semibold tracking-[.08em] text-[#bde1cf]">{bedCode(bed)}</div><div className={`text-[10px] leading-4 ${bed.allocations.length ? "text-[#e7d8bd]" : "text-[#77736c]"}`}>{bedPlanText(bed, text.noPlan)}</div></div> })}</div> : null}<div className="p-4"><div className="mb-3 flex justify-between text-xs text-[#aaa69c]"><span>{text.rotation}</span><span>{Math.round(num(block.rotation_deg))}°</span></div><input className="w-full accent-[#91c9ae]" type="range" min={-180} max={180} value={Math.round(num(block.rotation_deg))} onChange={(e) => rotate(Number(e.currentTarget.value))} onPointerUp={(e) => rotate(Number(e.currentTarget.value), true)} onBlur={(e) => rotate(Number(e.currentTarget.value), true)} /></div></aside> : null}
