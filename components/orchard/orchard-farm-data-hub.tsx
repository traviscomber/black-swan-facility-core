...
const copy={
 en:{title:"Farm data",help:"Canonical Orchard reference data, organized in the same working groups as Heirloom.",search:"Search…",tasks:"Tasks",containers:"Nursery containers",families:"Crop families",tags:"Tags",companies:"Seed companies",name:"Name",type:"Type",category:"Category",description:"Description",minutes:"Minutes / 30 m bed",cells:"Number of cells",width:"Container width",length:"Container length",crops:"Crops",usage:"Usage",company:"Company",lots:"Seed lots",none:"No canonical records are configured for this section.",observed:"Observed task",orchard:"Orchard",unavailable:"—",records:"records"},
 es:{title:"Datos de granja",help:"Datos canónicos de referencia de Orchard, organizados en los mismos grupos de trabajo que Heirloom.",search:"Buscar…",tasks:"Tareas",containers:"Contenedores de vivero",families:"Familias de cultivos",tags:"Etiquetas",companies:"Empresas de semillas",name:"Nombre",type:"Tipo",category:"Categoría",description:"Descripción",minutes:"Minutos / cama de 30 m",cells:"Número de celdas",width:"Ancho del contenedor",length:"Largo del contenedor",crops:"Cultivos",usage:"Uso",company:"Empresa",lots:"Lotes de semillas",none:"No hay registros canónicos configurados para esta sección.",observed:"Tarea observada",orchard:"Orchard",unavailable:"—",records:"registros"},
 de:{title:"Hofdaten",help:"Kanonische Orchard-Referenzdaten in denselben Arbeitsgruppen wie bei Heirloom.",search:"Suchen…",tasks:"Aufgaben",containers:"Anzuchtbehälter",families:"Kulturfamilien",tags:"Tags",companies:"Saatgutfirmen",name:"Name",type:"Typ",category:"Kategorie",description:"Beschreibung",minutes:"Minuten / 30-m-Beet",cells:"Zellenanzahl",width:"Behälterbreite",length:"Behälterlänge",crops:"Kulturen",usage:"Verwendung",company:"Firma",lots:"Saatgutlose",none:"Für diesen Bereich sind keine kanonischen Datensätze konfiguriert.",observed:"Beobachtete Aufgabe",orchard:"Orchard",unavailable:"—",records:"Einträge"}
} as const
...
export function OrchardFarmDataHub(){
 ...
 ]).then(results=>{if(!live)return;const failure=results.find(r=>r.error)?.error;if(failure)setError(failure.message);setTasks((results[0].data??[]) as TaskRow[]);setNursery((results[1].data??[]) as NurseryRow[]);setCrops((results[2].data??[]) as CropRow[]);setSeeds((results[3].data??[]) as SeedRow[]);setLoading(false)});return()=>{live=false}},[supabase])
 ...
 if(section==="tasks"){heads=[t.name,t.type,t.category,t.description,t.minutes];rows=taskRows.map(r=>[r.name,r.type,t.orchard,r.description,t.unavailable])}
