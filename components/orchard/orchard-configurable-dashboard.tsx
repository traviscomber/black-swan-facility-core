...
type RevenueMonth = { key:string; revenue:number; priced:number; eligible:number }
...
export function OrchardConfigurableDashboard(){
  ...
  const weatherWidget=<WidgetShell title={text.weather} icon={CloudSun} wide={false}>
    {weather?.daily?<div>{weather.daily.time.map((day,index)=>{const code=weather.daily!.weather_code[index]??0;const Icon=code<=3?CloudSun:CloudRain;return <div key={day} className="grid grid-cols-[1fr_auto] items-center gap-4 border-t border-[var(--orchard-line)] px-4 py-3 first:border-t-0"><div className="flex min-w-0 items-center gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--bs-surface-secondary)] text-[var(--orchard-green)]"><Icon className="h-4 w-4"/></span><div className="min-w-0"><p className="truncate text-sm font-medium">{dateLabel(day,locale)}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{weatherLabel(code,lang)} · PoP {Math.round(weather.daily!.precipitation_probability_max[index]??0)}%</p></div></div><div className="text-right"><p className="text-lg font-medium tabular-nums">{Math.round(weather.daily!.temperature_2m_mean[index]??0)}{weather.unit}</p><p className="text-[10px] tabular-nums text-muted-foreground">{Math.round(weather.daily!.temperature_2m_min[index]??0)}{weather.unit} / {Math.round(weather.daily!.temperature_2m_max[index]??0)}{weather.unit} · {(weather.daily!.precipitation_sum[index]??0).toFixed(1)} mm</p></div></div>})}</div>:<p className="p-6 text-sm text-muted-foreground">{text.weatherError}</p>}
  ...
  const revenueWidget=<WidgetShell title={text.revenue} icon={CircleDollarSign} wide={false}><p className="px-4 pt-4 text-xs leading-5 text-muted-foreground">{text.revenueHelp}</p>{months.length?<RevenueTimeline months={months} locale={locale} money={money}/>:<p className="p-6 text-sm text-muted-foreground">{text.noRevenue}</p>}<div className="border-t border-[var(--orchard-line)] p-3"><Link href={`/${language}/orchard/harvest/season${planQuery}`} className="inline-flex items-center gap-1.5 text-xs">{text.openHarvest}<ArrowRight className="h-3.5 w-3.5"/></Link></div></WidgetShell>
  ...
  const cropsWidget=<WidgetShell title={text.crops} icon={Sprout} wide={false}><div className="max-h-[520px] overflow-y-auto">{crops.slice(0,17).map(crop=><div key={crop.name} className="grid grid-cols-[1fr_auto] items-center gap-4 border-t border-[var(--orchard-line)] px-4 py-3 first:border-t-0"><div className="min-w-0"><div className="flex items-center gap-3"><span className="h-8 w-8 shrink-0 rounded-full" style={{backgroundColor:cropColor(crop.name,null)}}/><div className="min-w-0"><p className="truncate text-sm font-medium">{crop.name}</p><p className="mt-0.5 text-xs text-muted-foreground">{crop.successions} {text.plantings}</p></div></div><div className="ml-11 mt-2 h-1 bg-[var(--orchard-line)]"><div className="h-full" style={{width:`${maxBed?Math.max(3,crop.bedM/maxBed*100):0}%`,backgroundColor:cropColor(crop.name,null)}}/></div></div><div className="text-right"><p className="text-base font-medium tabular-nums">{crop.bedM.toLocaleString(locale,{maximumFractionDigits:1})}</p><p className="text-[10px] text-muted-foreground">{text.bed}</p></div></div>)}</div></WidgetShell>
  ...
  const isWide=(_key:WidgetKey)=>false
...
function RevenueTimeline({months,locale,money}:{months:RevenueMonth[];locale:string;money:(value:number)=>string}){
  const values=months.map(row=>row.revenue)
  const max=Math.max(...values,1)
  const width=960,height=260,left=42,right=18,top=24,bottom=52
  const innerWidth=width-left-right,innerHeight=height-top-bottom
  const x=(index:number)=>left+(months.length<=1?innerWidth/2:index/(months.length-1)*innerWidth)
  const y=(value:number)=>top+innerHeight-(value/max)*innerHeight
  const points=months.map((month,index)=>`${x(index)},${y(month.revenue)}`).join(" ")
  const area=`${left},${top+innerHeight} ${points} ${left+innerWidth},${top+innerHeight}`
  return <div className="p-4 pt-2">
    <div className="overflow-x-auto"><svg viewBox={`0 0 ${width} ${height}`} className="min-w-[620px] w-full" role="img" aria-label="Source-backed monthly revenue timeline">
      <defs><linearGradient id="orchardRevenueArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--orchard-green)" stopOpacity="0.24"/><stop offset="100%" stopColor="var(--orchard-green)" stopOpacity="0.02"/></linearGradient></defs>
      {[0,0.5,1].map(ratio=>{const yy=top+innerHeight*ratio;return <g key={ratio}><line x1={left} y1={yy} x2={left+innerWidth} y2={yy} stroke="var(--orchard-line)" strokeWidth="1"/><text x={left-8} y={yy+4} textAnchor="end" fill="currentColor" opacity="0.48" fontSize="10">{money(max*(1-ratio))}</text></g>})}
      <polygon points={area} fill="url(#orchardRevenueArea)"/>
      <polyline points={points} fill="none" stroke="var(--orchard-green)" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round"/>
      {months.map((month,index)=><g key={month.key}><circle cx={x(index)} cy={y(month.revenue)} r={month.revenue>0?4:2.5} fill="var(--bs-surface-primary)" stroke="var(--orchard-green)" strokeWidth="2"><title>{new Date(`${month.key}-01T12:00:00`).toLocaleDateString(locale,{month:"long",year:"numeric"})}: {money(month.revenue)} · {month.priced}/{month.eligible}</title></circle><text x={x(index)} y={height-22} textAnchor="middle" fill="currentColor" opacity="0.58" fontSize="11">{new Date(`${month.key}-01T12:00:00`).toLocaleDateString(locale,{month:"short"})}</text></g>)}
    </svg></div>
    <div className="mt-1 flex items-center justify-between gap-4 text-[10px] text-muted-foreground"><span>{months.filter(month=>month.revenue>0).length}/{months.length} months with source-backed revenue</span><span>{money(values.reduce((sum,value)=>sum+value,0))} total</span></div>
  </div>
}
