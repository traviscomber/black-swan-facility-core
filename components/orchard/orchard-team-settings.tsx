...
const copy={
 en:{title:"Team",help:"People canonically associated with Orchard work. Access is not inferred from job titles.",member:"Team members",status:"Status",role:"Role",active:"Active",inactive:"Inactive",unknown:"—",none:"No canonical Orchard team members available.",people:"people"},
 es:{title:"Equipo",help:"Personas asociadas canónicamente al trabajo de Orchard. El acceso no se infiere desde el cargo.",member:"Miembros del equipo",status:"Estado",role:"Rol",active:"Activo",inactive:"Inactivo",unknown:"—",none:"No hay miembros canónicos del equipo Orchard.",people:"personas"},
 de:{title:"Team",help:"Personen, die kanonisch mit Orchard-Arbeit verbunden sind. Zugriff wird nicht aus Rollen abgeleitet.",member:"Teammitglieder",status:"Status",role:"Rolle",active:"Aktiv",inactive:"Inaktiv",unknown:"—",none:"Keine kanonischen Orchard-Teammitglieder verfügbar.",people:"Personen"}
} as const
export function OrchardTeamSettings(){
 ...
 return <AppLayout><OrchardNavigation/><main className="mx-auto flex min-h-[calc(100vh-3.5rem)] w-full max-w-[1320px] flex-col px-4 pb-5 pt-4 sm:px-6 lg:px-8">
  ...
  <div className="mt-4 min-h-0 flex-1 overflow-auto border-y border-[var(--orchard-line)]"><table className="w-full min-w-[620px] border-collapse text-[12px]"><thead className="sticky top-0 z-10 bg-[var(--bs-surface-primary)] shadow-[0_1px_0_var(--orchard-line)]"><tr>{[t.member,t.status,t.role].map(h=><th key={h} className="px-3 py-2.5 text-left text-[9px] font-medium uppercase tracking-[.09em] text-muted-foreground">{h}</th>)}</tr></thead><tbody>{people.map(({employee,profile})=><tr key={employee.id} className="transition-colors hover:bg-[var(--bs-surface-secondary)]/55"><td className="border-b border-[var(--orchard-line-soft)] px-3 py-2.5 font-medium text-[#eee9e1]"><div>{employee.name}</div>{employee.email?<div className="mt-0.5 text-[10px] font-normal text-muted-foreground">{employee.email}</div>:null}</td><Td>{employee.is_active?t.active:t.inactive}</Td><Td>{profile.canonical_job_title||employee.role||t.unknown}</Td></tr>)}</tbody></table>{!loading&&!people.length?<div className="flex min-h-[280px] items-center justify-center px-6 text-center text-sm text-muted-foreground">{t.none}</div>:null}</div>
 </main></AppLayout>
}
