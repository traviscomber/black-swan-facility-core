...
function bedPlanText(bed: BedPlan, noPlan: string) {
  ...
  return [...groups.values()].map((group) => group.map((a) => `${a.crop}${a.lengthM > 0 && a.lengthM < 9.95 ? ` ${a.lengthM:g}m` : ""}`.replace(":g", "")).join(" + ")).join(" → ")
}
...
export function FarmMapSatelliteExperience() {
  ...
  const root = useRef<HTMLElement>(null), node = useRef<HTMLDivElement>(null), mapRef = useRef<LMap | null>(null), leaflet = useRef<Leaflet | null>(null), cleanup = useRef<(() => void) | null>(null)
  ...
  useEffect(() => {
    ...
    void import("leaflet").then((m) => {
      ...
      leaflet.current = L
      const map = L.map(node.current, { zoomControl: false, attributionControl: true, minZoom: 4, maxZoom: 20 }).setView(CENTER, 18)
      ...
      base.addTo(map)
      setReady(true)
      void Promise.all(overlays.filter((x) => visibleRef.current.has(x.id)).map((x) => loadOverlay(L, map, x))).then(() => { if (!dead) { map.setView(CENTER, 18); setZoom(18); requestAnimationFrame(() => map.invalidateSize()); setLoading(false) } })
      cleanup.current = () => map.off("zoomend", handleZoom)
    }).catch(() => { if (!dead) { setError(text.error); setLoading(false) } })
    return () => { dead = true; cleanup.current?.(); cleanup.current = null; mapRef.current?.remove(); mapRef.current = null; leaflet.current = null; layers.current.clear(); blocks.current.clear(); bedLayers.current.clear() }
  }, [overlays, text.error])
  ...
  useEffect(() => {
    ...
    for (const o of objects.filter((x) => area(x.object_type))) {
      ...
      const begin = (e: LEvent) => {
        e.originalEvent?.preventDefault?.(); e.originalEvent?.stopPropagation?.(); cleanup.current?.()
        ...
        const off = () => { document.removeEventListener("mousemove", move); document.removeEventListener("touchmove", move); document.removeEventListener("mouseup", finish); document.removeEventListener("touchend", finish); document.removeEventListener("touchcancel", finish); map.dragging.enable(); cleanup.current = null }
