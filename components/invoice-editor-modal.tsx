...
type InvoiceRecord = {
  ...
  notes?: string | null; terms_conditions?: string | null; total_amount?: number | null; amount_paid?: number | null; balance_due?: number | null; finalized_at?: string | null
}
type InvoicePreview = {
...
export function InvoiceEditorModal({ open, onOpenChange, invoice, invoiceId, reservationId, guestName = "", guestEmail = "", guestPhone = "", onSave }: InvoiceEditorModalProps) {
  ...
  const [linkedReservationId, setLinkedReservationId] = useState<string | null>(invoice?.reservation_id ?? reservationId ?? null)
  const [financial, setFinancial] = useState({
    total: Number(invoice?.total_amount ?? 0),
    paid: Number(invoice?.amount_paid ?? 0),
    balance: Number(invoice?.balance_due ?? invoice?.total_amount ?? 0),
    finalized: Boolean(invoice?.finalized_at),
  })
      ...
      try {
        if (effectiveInvoiceId) {
          ...
          setFinancial({
            ...
            paid: Number(data.amount_paid ?? 0),
            balance: Number(data.balance_due ?? data.total_amount ?? 0),
            finalized: Boolean(data.finalized_at),
          })
          setPaymentAmount(String(Math.max(0, Number(data.balance_due ?? data.total_amount ?? 0)) || ""))
        ...
        setFinancial({ total: 0, paid: 0, balance: 0, finalized: false })
        setPaymentAmount("")
  ...
  const invoiceLocked = financial.finalized
  ...
  async function recordPayment() {
    if (!linkedReservationId || paymentSaving) return
    const amount = Number(paymentAmount)
    if (!Number.isFinite(amount) || amount <= 0) return setError(copy.paymentAmountInvalid)
    if (financial.balance > 0 && amount > financial.balance) return setError(copy.paymentExceedsBalance)
    setPaymentSaving(true); setError(null)
    try {
      const response = await fetch("/api/bookings/payments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reservation_id: linkedReservationId, amount, payment_method: paymentMethod, transaction_id: paymentReference.trim() || null }) })
      const result = (await response.json()) as { error?: string; folio?: { summary?: { payments?: number | string; balance?: number | string; grossTotal?: number | string } } }
      if (!response.ok) throw new Error(result.error ?? copy.paymentFailed)
      const summary = result.folio?.summary
      const paid = Number(summary?.payments ?? financial.paid + amount)
      const balance = Math.max(0, Number(summary?.balance ?? financial.balance - amount))
      const totalValue = Number(summary?.grossTotal ?? financial.total)
      setFinancial((current) => ({ ...current, total: totalValue || current.total, paid, balance }))
      setFormData((current) => ({ ...current, payment_status: balance <= 0 ? "paid" : paid > 0 ? "partial" : "pending" }))
      setPaymentAmount(balance > 0 ? String(balance) : "")
      setPaymentReference("")
      toast.success(copy.paymentRecorded)
      onSave?.()
    } catch (paymentError) {
      setError(paymentError instanceof Error ? paymentError.message : copy.paymentFailed)
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent showCloseButton={false} className="grid h-[min(92dvh,900px)] w-[calc(100vw-1rem)] max-w-[calc(100vw-1rem)] grid-rows-[auto_minmax(0,1fr)_auto] gap-0 overflow-hidden rounded-none p-0 sm:w-[calc(100vw-2rem)] sm:max-w-[1180px] xl:max-w-[1280px]">
      ...
      {loading ? <div className="min-h-0 overflow-y-auto overflow-x-hidden"><div className="flex min-h-80 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin" /></div></div> : <div className="min-h-0 overflow-y-auto overflow-x-hidden"><div className="space-y-6 p-4 sm:p-6">
        ...
        <section className="grid min-w-0 gap-6 lg:grid-cols-2">
          <div className="min-w-0 space-y-4 rounded-xl border p-4"><h3 className="font-semibold">{copy.customer}</h3><div><label className="mb-1 block text-xs text-muted-foreground">{copy.customerName}</label><Input value={formData.customer_name} onChange={(event) => updateField("customer_name", event.target.value)} placeholder={copy.customerPlaceholder} readOnly={!effectiveInvoiceId || invoiceLocked} /></div><div className="grid gap-4 sm:grid-cols-2"><div><label className="mb-1 block text-xs text-muted-foreground">{copy.email}</label><Input type="email" value={formData.customer_email} onChange={(event) => updateField("customer_email", event.target.value)} placeholder="cliente@correo.cl" readOnly={!effectiveInvoiceId || invoiceLocked} /></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.phone}</label><Input value={formData.customer_phone} onChange={(event) => updateField("customer_phone", event.target.value)} placeholder="+56 9 1234 5678" readOnly={!effectiveInvoiceId || invoiceLocked} /></div></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.address}</label><textarea value={formData.customer_address} onChange={(event) => updateField("customer_address", event.target.value)} placeholder={copy.addressPlaceholder} rows={3} readOnly={!effectiveInvoiceId || invoiceLocked} className="w-full rounded-md border bg-background px-3 py-2 text-sm" /></div></div>
          <div className="min-w-0 space-y-4 rounded-xl border p-4"><h3 className="font-semibold">{copy.issueAndStatus}</h3><div className="grid gap-4 sm:grid-cols-2"><div><label className="mb-1 block text-xs text-muted-foreground">{copy.issueDate}</label><Input type="date" value={formData.invoice_date} onChange={(event) => updateField("invoice_date", event.target.value)} readOnly={!effectiveInvoiceId || invoiceLocked} /></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.dueDate}</label><Input type="date" value={formData.due_date} onChange={(event) => updateField("due_date", event.target.value)} readOnly={invoiceLocked} /></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.documentStatus}</label><select value={formData.status} onChange={(event) => updateField("status", event.target.value)} disabled={financial.finalized} className="h-10 w-full rounded-md border bg-background px-3 text-sm">{financial.finalized && <option value="finalized">{language === "es" ? "Finalizada" : language === "de" ? "Finalisiert" : "Finalized"}</option>}<option value="draft">{copy.draft}</option><option value="sent">{copy.sent}</option><option value="paid">{copy.paid}</option><option value="cancelled">{copy.cancelled}</option></select></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.paymentStatus}</label><select value={formData.payment_status} disabled className="h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="pending">{copy.pending}</option><option value="partial">{copy.partial}</option><option value="paid">{copy.paid}</option><option value="overdue">{copy.overdue}</option></select></div></div></div>
        </section>
        <section className="min-w-0 space-y-3 rounded-xl border p-4"><div className="flex items-center justify-between"><div><h3 className="font-semibold">{copy.detail}</h3>{!effectiveInvoiceId && <p className="text-xs text-muted-foreground">{copy.calculated}</p>}</div>{effectiveInvoiceId && !invoiceLocked && <Button type="button" variant="outline" size="sm" onClick={() => setLineItems((current) => [...current, { ...EMPTY_ITEM }])}><Plus className="mr-2 h-4 w-4" />{copy.addItem}</Button>}</div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead><tr className="border-b text-left"><th className="p-2">{copy.description}</th><th className="w-24 p-2 text-right">{copy.quantity}</th><th className="w-40 p-2 text-right">{copy.unitValue}</th><th className="w-40 p-2 text-right">{copy.total}</th>{effectiveInvoiceId && !invoiceLocked && <th className="w-12 p-2" />}</tr></thead><tbody>{lineItems.map((item, index) => <tr key={index} className="border-b last:border-0"><td className="p-2"><Input value={item.description} onChange={(event) => updateLineItem(index, "description", event.target.value)} placeholder={copy.itemPlaceholder} readOnly={!effectiveInvoiceId || invoiceLocked} /></td><td className="p-2"><Input type="number" min="1" step="1" value={item.qty} onChange={(event) => updateLineItem(index, "qty", Math.max(1, Number(event.target.value) || 1))} className="text-right" readOnly={!effectiveInvoiceId || invoiceLocked} /></td><td className="p-2"><Input type="number" min="0" step="1" value={item.unit_price} onChange={(event) => updateLineItem(index, "unit_price", Math.max(0, Number(event.target.value) || 0))} className="text-right" readOnly={!effectiveInvoiceId || invoiceLocked} /></td><td className="p-2 text-right font-medium">{formatClp(item.qty * item.unit_price)}</td>{effectiveInvoiceId && !invoiceLocked && <td className="p-2"><Button type="button" size="icon" variant="ghost" disabled={lineItems.length === 1} onClick={() => setLineItems((current) => current.filter((_, itemIndex) => itemIndex !== index))} aria-label={copy.deleteItem}><Trash2 className="h-4 w-4" /></Button></td>}</tr>)}</tbody></table></div></section>
        <section className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(300px,360px)]"><div className="space-y-4 rounded-xl border p-4"><h3 className="font-semibold">{copy.observationsConditions}</h3><div><label className="mb-1 block text-xs text-muted-foreground">{copy.observations}</label><textarea value={formData.notes} onChange={(event) => updateField("notes", event.target.value)} rows={3} readOnly={invoiceLocked} className="w-full rounded-md border bg-background px-3 py-2 text-sm" placeholder={copy.observationsPlaceholder} /></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.terms}</label><textarea value={formData.terms_conditions} onChange={(event) => updateField("terms_conditions", event.target.value)} rows={3} readOnly={!effectiveInvoiceId || invoiceLocked} className="w-full rounded-md border bg-background px-3 py-2 text-sm" placeholder={copy.termsPlaceholder} /></div></div><div className="space-y-4 rounded-xl border p-4"><h3 className="font-semibold">{copy.totalsClp}</h3>{effectiveInvoiceId && !invoiceLocked && <div className="grid grid-cols-2 gap-3"><div><label className="mb-1 block text-xs text-muted-foreground">{copy.discountPercent}</label><Input type="number" min="0" max="100" step="1" value={formData.discount_percentage} onChange={(event) => updateField("discount_percentage", Math.min(100, Math.max(0, Number(event.target.value) || 0)))} /></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.discountAmount}</label><Input type="number" min="0" step="1" value={formData.discount_amount} onChange={(event) => updateField("discount_amount", Math.max(0, Number(event.target.value) || 0))} disabled={formData.discount_percentage > 0} /></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.taxPercent}</label><Input type="number" min="0" max="100" step="1" value={formData.tax_rate} onChange={(event) => updateField("tax_rate", Math.min(100, Math.max(0, Number(event.target.value) || 0)))} /></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.additionalFeesAmount}</label><Input type="number" min="0" step="1" value={formData.additional_fees} onChange={(event) => updateField("additional_fees", Math.max(0, Number(event.target.value) || 0))} /></div></div>}<dl className="space-y-2 border-t pt-4 text-sm"><div className="flex justify-between"><dt className="text-muted-foreground">{copy.subtotal}</dt><dd>{formatClp(effectiveInvoiceId ? subtotal : preview?.subtotal ?? subtotal)}</dd></div>{discountAmount > 0 && <div className="flex justify-between"><dt className="text-muted-foreground">{copy.discount}</dt><dd>-{formatClp(discountAmount)}</dd></div>}{taxAmount > 0 && <div className="flex justify-between"><dt className="text-muted-foreground">{copy.tax}</dt><dd>{formatClp(taxAmount)}</dd></div>}{formData.additional_fees > 0 && <div className="flex justify-between"><dt className="text-muted-foreground">{copy.additionalFees}</dt><dd>{formatClp(formData.additional_fees)}</dd></div>}<div className="flex justify-between border-t pt-3 text-lg font-semibold"><dt>{copy.total}</dt><dd>{formatClp(total)}</dd></div></dl></div></section>
        {effectiveInvoiceId && linkedReservationId && financial.finalized && <section className="grid gap-4 rounded-xl border p-4 lg:grid-cols-[1fr_1fr]">
          <div>
            <h3 className="font-semibold">{copy.payments}</h3>
            <dl className="mt-3 space-y-2 text-sm">
              <div className="flex justify-between"><dt className="text-muted-foreground">{copy.total}</dt><dd>{formatClp(financial.total)}</dd></div>
              <div className="flex justify-between"><dt className="text-muted-foreground">{copy.amountPaid}</dt><dd>{formatClp(financial.paid)}</dd></div>
              <div className="flex justify-between border-t pt-2 font-medium"><dt>{copy.balance}</dt><dd>{formatClp(financial.balance)}</dd></div>
            </dl>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div><label className="mb-1 block text-xs text-muted-foreground">{copy.paymentAmount}</label><Input type="number" min="1" step="1" value={paymentAmount} onChange={(event) => setPaymentAmount(event.target.value)} disabled={financial.balance <= 0 || paymentSaving} /></div>
            <div><label className="mb-1 block text-xs text-muted-foreground">{copy.paymentMethod}</label><select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)} disabled={financial.balance <= 0 || paymentSaving} className="h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="transfer">{copy.transfer}</option><option value="cash">{copy.cash}</option><option value="card">{copy.card}</option><option value="other">{copy.other}</option></select></div>
            <div className="sm:col-span-2"><label className="mb-1 block text-xs text-muted-foreground">{copy.paymentReference}</label><Input value={paymentReference} onChange={(event) => setPaymentReference(event.target.value)} disabled={financial.balance <= 0 || paymentSaving} /></div>
            <div className="sm:col-span-2 flex justify-end"><Button type="button" onClick={() => void recordPayment()} disabled={financial.balance <= 0 || paymentSaving}>{paymentSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{financial.balance <= 0 ? copy.paid : copy.recordPayment}</Button></div>
          </div>
        </section>}
      </div></div>}
      <footer className="sticky bottom-0 flex flex-col-reverse gap-2 border-t bg-background/95 px-4 py-3 backdrop-blur sm:flex-row sm:justify-end sm:px-6"><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>{copy.cancel}</Button>{!invoiceLocked && <Button type="button" onClick={() => void saveInvoice()} disabled={saving || loading || isPreflightBlocked}>{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{effectiveInvoiceId ? copy.saveChanges : copy.generateFromReservation}</Button>}</footer>
    </DialogContent>
  </Dialog>
}
