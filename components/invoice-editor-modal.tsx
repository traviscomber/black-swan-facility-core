    ...
    <DialogContent showCloseButton={false} className="grid h-[min(92dvh,900px)] w-[calc(100vw-1rem)] max-w-[calc(100vw-1rem)] grid-rows-[auto_minmax(0,1fr)_auto] gap-0 overflow-hidden rounded-none p-0 sm:w-[calc(100vw-2rem)] sm:max-w-[1180px] xl:max-w-[1280px]">
      ...
      {loading ? <div className="min-h-0 overflow-y-auto overflow-x-hidden"><div className="flex min-h-80 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin" /></div></div> : <div className="min-h-0 overflow-y-auto overflow-x-hidden"><div className="space-y-6 p-4 sm:p-6">
        ...
        <section className="grid min-w-0 gap-6 lg:grid-cols-2">
          ...
          <div className="min-w-0 space-y-4 rounded-xl border p-4"><h3 className="font-semibold">{copy.issueAndStatus}</h3><div className="grid gap-4 sm:grid-cols-2"><div><label className="mb-1 block text-xs text-muted-foreground">{copy.issueDate}</label><Input type="date" value={formData.invoice_date} onChange={(event) => updateField("invoice_date", event.target.value)} readOnly={!effectiveInvoiceId} /></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.dueDate}</label><Input type="date" value={formData.due_date} onChange={(event) => updateField("due_date", event.target.value)} /></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.documentStatus}</label><select value={formData.status} onChange={(event) => updateField("status", event.target.value)} disabled={financial.finalized} className="h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="draft">{copy.draft}</option><option value="sent">{copy.sent}</option><option value="paid">{copy.paid}</option><option value="cancelled">{copy.cancelled}</option></select></div><div><label className="mb-1 block text-xs text-muted-foreground">{copy.paymentStatus}</label><select value={formData.payment_status} disabled className="h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="pending">{copy.pending}</option><option value="partial">{copy.partial}</option><option value="paid">{copy.paid}</option><option value="overdue">{copy.overdue}</option></select></div></div></div>
        </section>
