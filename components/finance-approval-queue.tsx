...
function isMapped(row: QueueRow) { return Boolean(row.division_name && row.category_name) }
...
export function FinanceApprovalQueue() {
  ...
  const eligible = useMemo(() => filtered.filter(isMapped), [filtered])
  ...
  const unmappedOpen = useMemo(() => rows.filter((row) => ['ready','exception','manual_review'].includes(row.classification_status) && !isMapped(row)).length, [rows])
  const sums = useMemo(() => {
  ...
  async function approve(ids: string[]) {
    const validIds = ids.filter((id) => rows.some((row) => row.id === id && isMapped(row)))
    if (!validIds.length) { toast.error('Primero confirma el mapeo canónico de los documentos.'); return }
    setBusy(true)
    let approved = 0
      ...
      <section className="bg-[var(--bs-surface-primary)] p-5 md:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-2 text-xl font-normal text-[var(--bs-text-primary)]">Aprobar solo lo que ya está clasificado</h2>
            <p className="mt-2 text-sm leading-6 text-[var(--bs-text-secondary)]">El criterio histórico de Valentina define la prioridad. Raimundo solo decide cuando el documento ya tiene P&L y categoría Budget confirmados.</p>
          </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-4">
          <div className="bg-[var(--bs-surface-secondary)] p-4"><p className="text-xs uppercase tracking-[0.1em] text-[var(--bs-text-muted)]">Listas</p><p className="mt-2 text-xl text-[var(--bs-cool-sage)]">{counts.ready ?? 0}</p></div>
          ...
          <div className="bg-[var(--bs-surface-secondary)] p-4"><p className="text-xs uppercase tracking-[0.1em] text-[var(--bs-text-muted)]">Sin mapeo</p><p className="mt-2 text-xl text-[var(--bs-warm-yellow)]">{unmappedOpen}</p><p className="mt-1 text-xs text-[var(--bs-text-secondary)]">No se pueden aprobar todavía</p></div>
          <div className="bg-[var(--bs-surface-secondary)] p-4"><p className="text-xs uppercase tracking-[0.1em] text-[var(--bs-text-muted)]">Pendiente</p><div className="mt-2 space-y-1 text-base text-[var(--bs-text-primary)]">{sums.map(([currency, total]) => total.pending > 0 && <p key={currency}>{formatMoney(total.pending, currency)}</p>)}</div></div>
        </div>
      </section>
            ...
            <tbody>
              {filtered.map((row) => {
                ...
                return <tr key={row.id} className="border-t border-[var(--bs-divider-subtle)] align-top">
                  {canDecide && <td className="px-4 py-4"><input type="checkbox" checked={selected.has(row.id)} onChange={() => toggle(row.id)} disabled={!mapped} aria-label={`Seleccionar ${row.document_number}`} /></td>}
                  ...
                  <td className="px-4 py-4"><p className={mapped ? 'text-[var(--bs-text-primary)]' : 'text-[var(--bs-warm-yellow)]'}>{row.division_name ?? 'P&L pendiente'}</p><p className="mt-1 text-xs text-[var(--bs-text-secondary)]">{row.category_name ?? 'Categoría pendiente'}</p></td>
                  ...
                  <td className="px-4 py-4 text-right">{canDecide ? <div className="flex justify-end gap-2">{mapped ? <Button size="sm" onClick={() => void approve([row.id])} disabled={busy}><Check className="mr-2 h-4 w-4" />Aprobar</Button> : <span className="self-center text-xs text-[var(--bs-text-muted)]">Mapear arriba</span>}<Button size="sm" variant="outline" onClick={() => void reject(row)} disabled={busy}><X className="mr-2 h-4 w-4" />Rechazar</Button></div> : <span className="text-xs text-[var(--bs-text-muted)]">{row.classification_status === 'approved' ? 'Cerrado' : 'Rechazado'}</span>}</td>
                </tr>
              })}
