...
import { BedDouble, CircleDollarSign, ConciergeBell, Sparkles } from "lucide-react"
...
export function ReservationOperationIndicators({ reservationId }: { reservationId: string }) {
  ...
  const load = useCallback(async () => {
    const [housekeepingResult, hospitalityResult, extrasResult, paymentsResult] = await Promise.all([
  ...
  if (!indicators.some((indicator) => indicator.visible)) return null
