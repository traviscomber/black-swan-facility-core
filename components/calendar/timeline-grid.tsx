  ...
  return (
    <CardContent className="p-0">
      <div className="border-b bg-background">
        <div className="flex min-h-6 items-center gap-1 px-2 py-0.5" aria-label={c.viewControls}>
          <button type="button" title={c.layersButton} aria-label={c.layersButton} onClick={() => setPreferences((current) => ({ ...current, showLayerToolbar: !current.showLayerToolbar }))} className={`inline-flex h-5 w-5 items-center justify-center border text-muted-foreground transition hover:bg-muted ${preferences.showLayerToolbar ? "border-primary bg-primary text-primary-foreground" : "bg-background"}`}><Layers3 className="h-3.5 w-3.5" /></button>
