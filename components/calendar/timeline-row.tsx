    ...
    <div className={`border-b transition [content-visibility:auto] [contain-intrinsic-size:44px] ${isDropTarget ? "bg-emerald-500/10 ring-1 ring-inset ring-emerald-500" : "hover:bg-muted/20"}`} data-bed-id={bed.id}>
      <div className="flex" style={{ height: ROW_HEIGHT }}>
        <div className="sticky left-0 z-20 flex shrink-0 items-center border-r bg-background px-3" style={{ width: LABEL_WIDTH, height: ROW_HEIGHT }}>
          <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="truncate text-xs font-semibold">{bed.bed_number}</span><span className="truncate text-[10px] text-muted-foreground">{bed.bed_type}</span></div></div>
        </div>
          ...
          {bedEvents.map((event) => {
            ...
            return <div key={`${event.event_type}-${event.event_id}-${bed.id}`}>
              ...
              <button type="button" ref={(element) => blockRefCallback(event.event_id, element)}
                ...
                className={`group absolute top-[3px] h-[38px] overflow-hidden rounded-[3px] border px-2 text-left text-[11px] transition-all duration-150 hover:brightness-105 focus:outline-none focus:ring-2 focus:ring-primary ${isBulkConflict ? "ring-2 ring-amber-400" : ""} ${isSelected ? "ring-2 ring-white ring-offset-1 ring-offset-primary brightness-110" : ""} ${isBlock ? "border-zinc-700 bg-zinc-800 text-white" : STATUS_STYLES[normalizedStatus(event.status)] ?? "border-slate-700 bg-slate-600 text-white"} ${isMoving || isConfirmingResize ? "opacity-60" : ""}`} style={{ left: geometry.left, width: geometry.width }}>
                ...
                <div className="truncate pr-20 text-[9px] opacity-80">{isBlock ? event.label : STATUS_LABELS[normalizedStatus(event.status)] ?? event.status}</div>
