          ...
          {bedEvents.map((event) => {
            ...
            return <div key={`${event.event_type}-${event.event_id}-${bed.id}`}>
              ...
              <button type="button" ref={(element) => blockRefCallback(event.event_id, element)}
                ...
                <div className="truncate pr-20 font-semibold leading-4">{isMoving ? c.validating : isConfirmingResize ? c.confirming : isEventResizing ? c.resizing : isBlock ? c.blocks[blockKey] ?? c.blocks.other : event.guest_name ?? event.label}</div>
                <div className="truncate pr-20 text-[9px] opacity-80">{isBlock ? event.label : c.statuses[statusKey] ?? event.status}</div>
