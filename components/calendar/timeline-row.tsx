...
export function TimelineRow(props: TimelineRowProps) {
  ...
  return (
    <div className={`flex border-b transition ${isDropTarget ? "bg-emerald-500/10 ring-1 ring-inset ring-emerald-500" : "hover:bg-muted/20"}`} style={{ height: ROW_HEIGHT }} data-bed-id={bed.id}>
        ...
        {bedEvents.map((event) => {
          ...
          return <div key={`${event.event_type}-${event.event_id}-${bed.id}`}>
            ...
            >
              ...
              <div className="truncate pr-16 font-semibold leading-4">{isMoving ? "Validando…" : isConfirmingResize ? "Confirmando…" : isEventResizing ? "Ajustando…" : isBlock ? BLOCK_LABELS[event.block_type ?? "other"] ?? "Bloqueada" : event.guest_name ?? event.label}</div>
