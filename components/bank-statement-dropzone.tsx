...
type Statement = { id: string; original_filename: string; period_start: string; period_end: string; status: string; created_at: string; processed_at: string | null; matched_count: number; unmatched_count: number; processing_error: string | null }
...
export function BankStatementDropzone() {
  ...
  async function upload() {
    ...
    try {
      ...
      const result = await response.json() as { error?: string; status?: string; processed?: boolean; matched_count?: number; unmatched_count?: number; processing_error?: string | null }
      if (!response.ok) throw new Error(result.error ?? 'No fue posible guardar la cartola.')
      if (result.status === 'duplicate') toast.success('Esta cartola ya se había recibido.')
      else if (result.processed) toast.success(`Cartola cruzada automáticamente · ${result.matched_count ?? 0} pagos observados.`)
      else toast.warning('Cartola guardada. El cruce automático requiere revisión.')
  ...
  return <section className="mx-4 mt-4 bg-[var(--bs-surface-primary)] p-5 md:mx-8 md:p-6">
    ...
    <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--bs-text-secondary)]">Sube el PDF, CSV o XLSX original e indica el período. Cada carga se cruza automáticamente con los gastos aprobados. Un match marca pago observado, nunca conciliado final.</p>
    ...
    {rows.length > 0 && <div className="mt-6 border-t border-[var(--bs-divider-subtle)] pt-4"><h3 className="text-sm text-[var(--bs-text-primary)]">Últimas cartolas</h3><div className="mt-3 divide-y divide-[var(--bs-divider-subtle)]">{rows.map(row => <div key={row.id} className="flex flex-wrap justify-between gap-2 py-2 text-xs text-[var(--bs-text-secondary)]"><span><a className="underline" href={`/api/finance/bank-statements/source?id=${encodeURIComponent(row.id)}`} target="_blank" rel="noreferrer">{row.original_filename}</a> · {row.period_start} → {row.period_end}</span><span>{row.processing_error ? 'Cruce pendiente' : row.processed_at ? `${row.matched_count} pagos observados · ${row.unmatched_count} movimientos sin match` : 'Procesando'}</span></div>)}</div></div>}
  </section>
}
