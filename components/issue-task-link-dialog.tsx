...
export function IssueTaskLinkDialog({ issueId }: { issueId: string }) {
  ...
  const taskHref = issue ? buildOperationalTaskHref({
    area: "mantenimiento",
    title: `Resolver incidencia: ${issue.title || "sin título"}`,
    description: issue.description || "Revisar la incidencia y registrar la solución ejecutada.",
    category: issue.category || "Incidencia",
    priority: priorityMap[(issue.severity || issue.priority || "medium").toLowerCase()] || "media",
    sourceType: "issue",
    sourceId: issue.id,
    sourceLabel: issue.title || "Incidencia sin título",
    sourcePath: "/issues",
  }) : null
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader><DialogTitle>Tareas vinculadas</DialogTitle></DialogHeader>
        {loading ? <div className="flex items-center justify-center py-10"><Loader2 className="h-5 w-5 animate-spin" /></div> : <div className="space-y-4">
          {error && <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">{error}</div>}
          {taskHref && <Button asChild className="w-full"><Link href={taskHref}><ClipboardPlus className="mr-2 h-4 w-4" />Crear tarea desde esta incidencia</Link></Button>}
          <div className="border-t pt-4">
            <p className="mb-2 text-sm font-medium">Vincular tarea existente</p>
            {tasks.length === 0 ? <p className="text-sm text-muted-foreground">No hay tareas activas disponibles.</p> : <div className="max-h-72 space-y-2 overflow-y-auto">{tasks.map((task) => <label key={task.id} className="flex cursor-pointer items-center gap-3 rounded-md border p-3 hover:bg-muted/50"><input type="checkbox" checked={linkedTaskIds.includes(task.id)} onChange={() => void handleTaskToggle(task.id)} className="h-4 w-4" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{task.title}</p><div className="mt-1 flex gap-2"><Badge variant="outline" className="text-xs">{task.status}</Badge><Badge variant="outline" className="text-xs">{task.priority}</Badge></div></div></label>)}</div>}
          </div>
        </div>}
      </DialogContent>
    </Dialog>
