...
export function DailyOperationsPanel() {
  ...
  const loadItems = useCallback(async () => {
    ...
    const [housekeepingResult, hospitalityResult, tasksResult] = await Promise.all([
      ...
      supabase.from("hospitality_requests").select("id, request_type, status, priority, description, guest_name, due_at").not("status", "in", "(completed,resolved,cancelled)").gte("due_at", hospitalityStart).lt("due_at", hospitalityEnd).limit(20),
  ...
  return <section className="sticky bottom-0 z-40 border-t border-border/40 bg-[var(--bs-bg-secondary)]">
