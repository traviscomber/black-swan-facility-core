...
export function DailyOperationsPanel() {
  ...
  const loadItems = useCallback(async () => {
    ...
    const [housekeepingResult, hospitalityResult, tasksResult] = await Promise.all([
      ...
      supabase.from("hospitality_requests").select("id, request_type, status, priority, description, guest_name, created_at").not("status", "in", "(completed,resolved,cancelled)").gte("created_at", hospitalityStart).lt("created_at", hospitalityEnd).order("created_at", { ascending: false }).limit(20),
