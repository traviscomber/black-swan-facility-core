...
export default function ActivitiesCalendarPage() {
  ...
  const normalizedQuery = searchQuery.trim().toLocaleLowerCase(language === 'de' ? 'de-DE' : language === 'es' ? 'es-CL' : 'en-US')
  const filteredActivities = useMemo(() => activities
    .filter((activity) => selectedType === 'all' || activity.activity_type_id === selectedType)
    .filter((activity) => !normalizedQuery || activity.title.toLocaleLowerCase().includes(normalizedQuery) || activity.description?.toLocaleLowerCase().includes(normalizedQuery)), [activities, normalizedQuery, selectedType])
  ...
  return (
    <AppLayout>
      <div className="mx-auto max-w-[1600px] space-y-5 px-3 py-6 sm:px-4 lg:px-6">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold text-accent">{copy.title}</h1><p className="mt-1 text-sm text-muted-foreground">{copy.subtitle}</p></div><Button onClick={() => handleCreateActivity(null)}><Plus className="mr-2 h-4 w-4" />{copy.new}</Button></div>
        ...
        <div className="flex items-center justify-between rounded border bg-card px-3 py-2"><Button variant="ghost" size="sm" onClick={() => setCurrentMonth(subMonths(currentMonth, 1))}><ChevronLeft className="h-4 w-4" /></Button><div className="text-sm font-semibold capitalize">{format(currentMonth, 'MMMM yyyy', { locale: dateLocale })}</div><Button variant="ghost" size="sm" onClick={() => setCurrentMonth(addMonths(currentMonth, 1))}><ChevronRight className="h-4 w-4" /></Button></div>
        {loading ? <Card><CardContent className="p-8 text-center text-sm text-muted-foreground">{copy.loading}</CardContent></Card> : <ActivitiesTimeline activities={filteredActivities} activityTypes={activityTypes} dates={dates} onEdit={handleEditActivity} onCreate={(date) => handleCreateActivity(date)} />}
        <Card><CardHeader><CardTitle>{copy.upcoming}</CardTitle><CardDescription>{copy.compact}</CardDescription></CardHeader><CardContent className="space-y-3">{filteredActivities.length === 0 && <p className="text-sm text-muted-foreground">{copy.empty}</p>}{filteredActivities.slice().sort((a, b) => a.start_date.localeCompare(b.start_date)).slice(0, 12).map((activity) => {
          const type = activityTypes.find((item) => item.id === activity.activity_type_id)
          return <div key={activity.id} className="flex flex-col gap-3 rounded border p-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span>{type?.icon}</span><h3 className="truncate font-semibold">{activity.title}</h3></div>{activity.description && <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{activity.description}</p>}<div className="mt-2 flex flex-wrap gap-3 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1"><Calendar className="h-3 w-3" />{format(parseISO(activity.start_date), 'dd MMM yyyy', { locale: dateLocale })}</span>{activity.start_time && <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" />{activity.start_time}</span>}{activity.location && <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" />{activity.location}</span>}{activity.capacity > 0 && <span className="inline-flex items-center gap-1"><Users className="h-3 w-3" />{activity.current_attendees || 0}/{activity.capacity}</span>}</div></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => handleEditActivity(activity)}>{copy.edit}</Button><Button variant="destructive" size="sm" onClick={() => void handleDeleteActivity(activity.id)}>{copy.delete}</Button></div></div>
        })}</CardContent></Card>
      </div>
