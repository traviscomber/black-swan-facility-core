...
export default function BookingsCalendarPage() {
  ...
  function openReservationFromTimeline(bed: Bed, clientX: number, currentTarget: HTMLDivElement) { if (draggingEventId || isResizing || confirmingReservationId) return; const rect = currentTarget.getBoundingClientRect(); const offset = Math.max(0, Math.min(timelineWidth - 1, clientX - rect.left)); openNewReservation(bed, addDays(startDate, Math.floor(offset / DAY_WIDTH))) }

  function beginReservationResize(event: CalendarEvent, edge: ReservationResizeEdge, pointerEvent: React.PointerEvent<HTMLSpanElement>) { if (event.event_type !== "reservation" || movingReservationId || draggingEventId || confirmingReservationId) return; pointerEvent.preventDefault(); pointerEvent.stopPropagation(); pointerEvent.currentTarget.setPointerCapture(pointerEvent.pointerId); beginResize({ reservationId: event.event_id, bedId: event.bed_id, edge, pointerId: pointerEvent.pointerId, pointerStartX: pointerEvent.clientX, startsOn: event.starts_on, endsOn: event.ends_on }) }
  ...
  async function finishReservationResize(pointerEvent: React.PointerEvent<HTMLSpanElement>) {
    ...
    const { data, error: resizeError } = await supabase.rpc("resize_booking_reservation", {
      p_reservation_id: pendingResize.reservationId,
      p_check_in: pendingResize.previewStart,
      p_check_out: pendingResize.previewEnd,
    })
    const result = ((data ?? [])[0] ?? null) as ResizeRpcResult | null

    if (resizeError || !result?.success) {
      setEvents(previousEvents)
      const message = resizeError?.message ?? result?.message ?? "La disponibilidad cambió antes de confirmar las fechas"
      setError(message)
      toast.error("El cambio de fechas fue rechazado y se restauró la reserva")
      clearResize()
      return
    }
    ...
    toast.success(`Reserva actualizada: ${result.check_in} → ${result.check_out}`)
    clearResize()
    await loadData()
  }
  function beginReservationDrag(event: CalendarEvent, transfer: DataTransfer) { if (isResizing || confirmingReservationId) return; transfer.effectAllowed = "move"; transfer.setData("text/plain", event.event_id); setDraggingEventId(event.event_id) }
  ...
  return <div className="min-h-screen bg-background p-4 md:p-6"><div className="mx-auto max-w-[1800px] space-y-5">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"><div><p className="mb-1 text-xs font-semibold uppercase tracking-[0.16em] text-primary">Hospitalidad · Fundo Corcovado</p><h1 className="text-3xl font-semibold tracking-tight">Reservas y disponibilidad</h1><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Timeline operativo conectado al Availability Engine. Arrastra una reserva hacia otra cama para reasignarla con validación automática.</p></div><div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link href="/bookings/blocks"><Ban className="mr-2 h-4 w-4" />Gestionar bloqueos</Link></Button><Button variant="outline" onClick={() => setStartDate(startOfDay(new Date()))}><CalendarDays className="mr-2 h-4 w-4" />Hoy</Button><Button onClick={() => { setPreselectedBed(null); setPreselectedDate(null); setNewReservationOpen(true) }}><Plus className="mr-2 h-4 w-4" />Nueva reserva</Button></div></div>
    ...
    <Card className="overflow-hidden"><CardHeader className="flex flex-row items-center justify-between border-b py-3"><div><CardTitle className="text-base">{format(startDate, "dd MMM")} — {format(addDays(endDate, -1), "dd MMM yyyy")}</CardTitle><p className="text-xs text-muted-foreground">{visibleBeds.length} camas visibles · {metrics.blocks} bloqueos activos · clic en un espacio libre para reservar · arrastra una reserva para cambiarla de cama · ajusta sus extremos para cambiar fechas</p></div><div className="flex gap-2"><Button variant="outline" size="icon" onClick={() => setStartDate(addDays(startDate, -rangeDays))}><ChevronLeft className="h-4 w-4" /></Button><Button variant="outline" size="icon" onClick={() => setStartDate(addDays(startDate, rangeDays))}><ChevronRight className="h-4 w-4" /></Button></div></CardHeader><CardContent className="p-0"><div className="overflow-auto"><div style={{ minWidth: LABEL_WIDTH + timelineWidth }}>
      <div className="sticky top-0 z-30 flex border-b bg-background"><div className="sticky left-0 z-40 flex shrink-0 items-center border-r bg-background px-4 font-medium" style={{ width: LABEL_WIDTH, height: 58 }}>Propiedad / habitación / cama</div><div className="grid" style={{ width: timelineWidth, gridTemplateColumns: `repeat(${rangeDays}, ${DAY_WIDTH}px)` }}>{dates.map((date) => <div key={date.toISOString()} className={`flex flex-col items-center justify-center border-r text-center ${isSameDay(date, new Date()) ? "bg-amber-100" : ""}`} style={{ height: 58 }}><div className="text-xs text-muted-foreground">{format(date, "EEE")}</div><div className="font-semibold">{format(date, "dd MMM")}</div></div>)}</div></div>
      {loading ? <div className="p-12 text-center text-muted-foreground">Cargando Availability Engine…</div> : visibleBeds.length === 0 ? <div className="p-12 text-center text-muted-foreground">No hay camas para los filtros seleccionados.</div> : visibleBeds.map((bed) => { const bedEvents = eventsByBed.get(bed.id) ?? []; const isDropTarget = dropTargetBedId === bed.id && !!draggingEventId; return <div key={bed.id} className={`flex border-b transition ${isDropTarget ? "bg-emerald-500/10 ring-1 ring-inset ring-emerald-500" : "hover:bg-muted/20"}`} style={{ height: ROW_HEIGHT }} onDragOver={(dragEvent) => { if (!draggingEventId) return; dragEvent.preventDefault(); dragEvent.dataTransfer.dropEffect = "move" }} onDragEnter={() => draggingEventId && setDropTargetBedId(bed.id)} onDragLeave={(dragEvent) => { if (!dragEvent.currentTarget.contains(dragEvent.relatedTarget as Node | null)) setDropTargetBedId(null) }} onDrop={(dropEvent) => { dropEvent.preventDefault(); void moveReservationToBed(bed) }}><div className="sticky left-0 z-20 flex shrink-0 flex-col justify-center border-r bg-background px-4" style={{ width: LABEL_WIDTH, height: ROW_HEIGHT }}><div className="truncate font-medium">{bed.room.location_ref?.name ?? "Sin propiedad"}</div><div className="truncate text-xs text-muted-foreground">Hab. {bed.room.room_number} · {bed.bed_number} · {bed.bed_type}</div></div><div className="relative cursor-crosshair" style={{ width: timelineWidth, height: ROW_HEIGHT, backgroundImage: `repeating-linear-gradient(to right, transparent 0, transparent ${DAY_WIDTH - 1}px, hsl(var(--border)) ${DAY_WIDTH - 1}px, hsl(var(--border)) ${DAY_WIDTH}px)` }} onClick={(clickEvent) => openReservationFromTimeline(bed, clickEvent.clientX, clickEvent.currentTarget)}>{dates.map((date, index) => isSameDay(date, new Date()) ? <div key={`today-${bed.id}-${index}`} className="pointer-events-none absolute inset-y-0 bg-amber-50/70" style={{ left: index * DAY_WIDTH, width: DAY_WIDTH }} /> : null)}
        {bedEvents.map((event) => { const geometry = eventGeometry(event); const isBlock = event.event_type === "block"; const isMoving = movingReservationId === event.event_id; const isEventResizing = resizingReservationId === event.event_id; const isConfirmingResize = confirmingReservationId === event.event_id; const previewGeometry = isEventResizing && resizeState ? geometryForDates(resizeState.previewStart, resizeState.previewEnd) : null; return <div key={`${event.event_type}-${event.event_id}-${bed.id}`}>{previewGeometry && <div className="pointer-events-none absolute top-2 z-20 h-[52px] rounded-md border-2 border-dashed border-white/90 bg-primary/35 shadow-sm transition-all duration-150" style={{ left: previewGeometry.left, width: previewGeometry.width }}><div className="truncate px-3 pt-1 text-xs font-semibold text-white">{resizeState!.previewStart} → {resizeState!.previewEnd}</div></div>}<button type="button" draggable={!isBlock && !movingReservationId && !isResizing && !confirmingReservationId} onDragStart={(dragEvent) => !isBlock && beginReservationDrag(event, dragEvent.dataTransfer)} onDragEnd={finishReservationDrag} onClick={(buttonEvent) => { buttonEvent.stopPropagation(); if (draggingEventId || isResizing || confirmingReservationId) return; if (isBlock) void openBlock(event); else void openReservation(event) }} className={`group absolute top-2 h-[52px] overflow-hidden rounded-md border px-3 text-left text-xs shadow-sm transition-all duration-200 hover:brightness-110 focus:outline-none focus:ring-2 focus:ring-primary ${isBlock ? "border-zinc-500 bg-zinc-800 text-white" : `${STATUS_STYLES[event.status] ?? "bg-slate-200 text-slate-900"} cursor-grab active:cursor-grabbing`} ${draggingEventId === event.event_id || isEventResizing ? "opacity-35" : ""} ${isConfirmingResize ? "cursor-wait ring-2 ring-white/80" : ""}`} style={{ left: geometry.left, width: geometry.width }} title={isBlock ? `${event.label} · ${event.starts_on} → ${event.ends_on}` : `Arrastra para cambiar de cama · usa los extremos para ajustar fechas · ${event.guest_name ?? event.label} · ${event.starts_on} → ${event.ends_on}`}>
          ...
          <div className="truncate font-semibold">{isMoving ? "Validando movimiento…" : isConfirmingResize ? "Confirmando fechas…" : isEventResizing ? "Ajustando fechas…" : isBlock ? BLOCK_LABELS[event.block_type ?? "other"] ?? "Bloqueada" : event.guest_name ?? event.label}</div><div className="truncate opacity-80">{isBlock ? event.label : `${STATUS_LABELS[event.status] ?? event.status} · ${event.starts_on} → ${event.ends_on}`}</div></button></div> })}
      </div></div> })}
