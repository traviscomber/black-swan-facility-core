...
import { AlertTriangle, ArrowLeft, BedDouble, CalendarDays, ClipboardCheck, History, MapPin, Wrench } from "lucide-react"
...
type Incident = { id: string; title: string | null; status: string | null; severity: string | null; priority: string | null; created_at: string | null }
...
export default async function RoomObjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [stateResult, reservationsResult, housekeepingResult, maintenanceResult, incidentsResult, historyResult] = await Promise.all([
    ...
    supabase.from("incidents").select("id,title,status,severity,priority,created_at").eq("room_id", id).not("status", "in", "(resolved,closed)").order("created_at", { ascending: false }).limit(8),
  ...
  const queryError = stateResult.error || reservationsResult.error || housekeepingResult.error || maintenanceResult.error || incidentsResult.error || historyResult.error
    ...
    <AppLayout>
      <div className="space-y-6 p-4 md:p-6">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          ...
          <ObjectList title="Incidentes abiertos" icon={<AlertTriangle className="h-4 w-4" />} empty="Sin incidentes abiertos." href="/issues">
            {incidents.map((item) => <CompactRow key={item.id} title={item.title || "Incidente"} detail={item.created_at ? new Intl.DateTimeFormat("es-CL", { dateStyle: "medium", timeStyle: "short", timeZone: "America/Santiago" }).format(new Date(item.created_at)) : "Sin fecha"} status={item.severity || item.priority || item.status} />)}
          </ObjectList>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            {history.length === 0 ? <p className="text-sm text-muted-foreground">Todavía no hay historial operacional registrado para esta habitación.</p> : (
              <div className="space-y-3">{history.map((item) => <div key={item.id} className="border-l-2 border-border pl-4"><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-medium">{item.previous_status || "—"} → {item.new_status || "—"}</p><Badge variant="outline">{item.source || "system"}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{new Intl.DateTimeFormat("es-CL", { dateStyle: "medium", timeStyle: "short", timeZone: "America/Santiago" }).format(new Date(item.created_at))}{item.reason ? ` · ${item.reason}` : ""}</p></div>)}</div>
            )}
          </CardContent>
        </Card>
...
function CompactRow({ title, detail, status }: { title: string; detail: string; status: string | null }) {
  return <div className="flex items-start justify-between gap-3 rounded-md border p-3"><div><p className="font-medium">{title}</p><p className="mt-1 text-xs text-muted-foreground">{detail}</p></div><Badge variant={statusVariant(status)}>{status || "sin estado"}</Badge></div>
}
