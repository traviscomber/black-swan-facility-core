      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[880px] text-xs">
          ...
          <tbody className="divide-y divide-white/[0.06]">
            {loading ? <tr><td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">{copy.loading}</td></tr> : visible.length === 0 ? <tr><td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">{copy.noActivity}</td></tr> : visible.map((reservation) => <tr key={reservation.id} className="hover:bg-white/[0.025]"><td className="px-3 py-2"><div className="font-medium">{reservation.guest_name}</div><div className="text-[11px] text-muted-foreground">{reservation.guest_email || reservation.guest_phone || "—"}</div></td><td className="px-4 py-2.5 text-muted-foreground">{reservation.bed?.room?.location?.name ?? copy.noProperty} · {copy.roomShort} {reservation.bed?.room?.room_number ?? "—"} · {reservation.bed?.bed_number ?? "—"}</td><td className="whitespace-nowrap px-3 py-2 text-muted-foreground">{reservation.check_in} → {reservation.check_out} · {reservation.num_guests ?? 1} {copy.guests}</td><td className="px-4 py-2.5"><Badge variant="outline" className="h-5 rounded-[3px] px-1.5 text-[10px]">{statusLabels[reservation.status] ?? reservation.status}</Badge></td><td className="px-3 py-2 text-right">{reservation.status === "pending" ? <Button size="sm" className="h-7 rounded-[4px] px-2 text-xs" onClick={() => void updateReservationStatus(reservation.id, "confirmed")}>{copy.confirm}</Button> : reservation.status === "confirmed" ? <Button size="sm" className="h-7 rounded-[4px] px-2 text-xs" onClick={() => void updateReservationStatus(reservation.id, "checked_in")}><LogIn className="mr-1 h-3.5 w-3.5" />{copy.checkIn}</Button> : (reservation.status === "checked_in" || reservation.status === "checked-in") ? <Button size="sm" className="h-7 rounded-[4px] px-2 text-xs" onClick={() => void updateReservationStatus(reservation.id, "checked_out")}><LogOut className="mr-1 h-3.5 w-3.5" />{copy.checkOut}</Button> : null}</td></tr>)}
          </tbody>
        </table>
      </div>
    </section>
