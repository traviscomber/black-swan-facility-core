...
export default function ReservationChargesPage() {
  ...
  return <div className="min-h-screen bg-[#111213] text-foreground">
    <header className="flex min-h-[58px] flex-col gap-3 border-b border-white/10 px-4 py-3 md:flex-row md:items-center md:justify-between md:px-5"><div><h1 className="text-[15px] font-medium tracking-tight">{copy.title}</h1><p className="text-[11px] text-muted-foreground">{copy.subtitle}</p></div><Button className="h-8 rounded-[5px] bg-emerald-600 px-3 text-xs hover:bg-emerald-500" onClick={createInvoice} disabled={!selectedReservation || invoicing}><FileText className="mr-1.5 h-3.5 w-3.5" />{invoicing ? copy.generating : copy.generateInvoice}</Button></header>
    <div className="grid min-h-[calc(100dvh-58px)] lg:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="border-r border-white/10 bg-[#151718]"><div className="border-b border-white/10 p-2"><div className="relative"><Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} className="h-8 rounded-[4px] border-white/10 bg-[#111314] pl-8 text-xs" placeholder={copy.search} /></div></div><div className="max-h-[calc(100dvh-100px)] overflow-y-auto [content-visibility:auto] [contain-intrinsic-size:600px]">{loading ? <p className="p-4 text-xs text-muted-foreground">{copy.loading}</p> : filteredReservations.map((reservation) => <button key={reservation.id} onClick={() => setSelectedReservationId(reservation.id)} className={`w-full border-b border-white/[0.06] px-3 py-2.5 text-left ${selectedReservationId === reservation.id ? "bg-white/[0.06]" : "hover:bg-white/[0.025]"}`}><div className="flex items-center justify-between gap-2"><span className="truncate text-xs font-medium">{reservation.guest_name}</span><Badge variant="outline" className="h-5 rounded-[3px] px-1.5 text-[10px]">{reservation.status}</Badge></div><p className="mt-1 text-[11px] text-muted-foreground">{reservation.check_in} → {reservation.check_out}</p></button>)}</div></aside>
      <main className="min-w-0">
        ...
        <div className="flex min-h-10 flex-wrap items-center gap-4 border-b border-white/10 bg-[#17191a] px-3 py-1.5 text-xs"><span>{copy.lodging}: <b>{formatClp(lodgingTotal)}</b></span><span>{copy.extras}: <b>{formatClp(extrasSubtotal)}</b></span><span>{copy.extraTaxes}: <b>{formatClp(taxTotal)}</b></span><span className="ml-auto text-emerald-300">{copy.billableTotal}: <b>{formatClp(grandTotal)}</b></span></div>
