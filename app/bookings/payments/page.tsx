...
export default function BookingPaymentsPage() {
  ...
  return <div className="min-h-screen bg-[#111213] text-foreground">
    <header className="flex min-h-[58px] items-center justify-between gap-3 border-b border-white/10 bg-[#17191a] px-4 py-2"><div className="min-w-0"><h1 className="truncate text-lg font-medium">{copy.title}</h1><p className="truncate text-xs text-muted-foreground">{copy.subtitle}</p></div></header>
    <div className="flex min-h-10 items-center gap-2 border-b border-white/10 bg-[#151718] px-3 py-1.5"><div className="relative min-w-0 flex-1"><Search className="absolute left-2.5 top-2 h-3.5 w-3.5 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} className="h-8 rounded-[4px] border-white/10 bg-[#111314] pl-8 text-xs" placeholder={copy.search} /></div><Select value={filter} onValueChange={setFilter}><SelectTrigger className="h-8 w-44 rounded-[4px] border-white/10 bg-[#111314] text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{copy.allStatuses}</SelectItem><SelectItem value="pending">{copy.noPayment}</SelectItem><SelectItem value="partial">{copy.partial}</SelectItem><SelectItem value="paid">{copy.paid}</SelectItem></SelectContent></Select></div>
    {error && <div className="border-b border-red-500/30 bg-red-500/10 px-4 py-2 text-xs text-red-300">{error}</div>}
    <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-xs"><thead className="bg-[#17191a] text-left text-muted-foreground"><tr className="border-b border-white/10"><th className="px-3 py-2 font-medium">{copy.guest}</th><th className="px-3 py-2 font-medium">{copy.stay}</th><th className="px-3 py-2 text-right font-medium">{copy.total}</th><th className="px-3 py-2 text-right font-medium">{copy.paid}</th><th className="px-3 py-2 text-right font-medium">{copy.balance}</th><th className="px-3 py-2 font-medium">{copy.status}</th><th className="px-3 py-2 text-right font-medium">{copy.action}</th></tr></thead><tbody>{loading ? <tr><td colSpan={7} className="p-10 text-center text-muted-foreground">{copy.loading}</td></tr> : visibleRows.length === 0 ? <tr><td colSpan={7} className="p-10 text-center text-muted-foreground">{copy.noRows}</td></tr> : visibleRows.map((row) => <tr key={row.reservation.id} className="border-b border-white/[0.06] hover:bg-white/[0.025]"><td className="px-3 py-2"><div className="font-medium">{row.reservation.guest_name}</div><div className="text-[11px] text-muted-foreground">{row.reservation.guest_email || "—"}</div></td><td className="px-3 py-2">{format(new Date(`${row.reservation.check_in}T00:00:00`), "dd MMM", { locale: dateLocale })} — {format(new Date(`${row.reservation.check_out}T00:00:00`), "dd MMM yyyy", { locale: dateLocale })}</td><td className="px-3 py-2 text-right">{money(row.total)}</td><td className="px-3 py-2 text-right text-emerald-400">{money(row.paid)}</td><td className="px-3 py-2 text-right font-medium">{money(row.balance)}</td><td className="px-3 py-2"><PaymentBadge status={row.status} copy={copy} /></td><td className="px-3 py-2 text-right"><PermissionGate action="payments.record" department="finance"><Button size="sm" className="h-7 rounded-[4px] px-2 text-[11px]" onClick={() => openPayment(row.reservation, row.balance)} disabled={row.balance <= 0}><Plus className="mr-1.5 h-3 w-3" />{copy.recordPayment}</Button></PermissionGate></td></tr>)}</tbody></table></div>
