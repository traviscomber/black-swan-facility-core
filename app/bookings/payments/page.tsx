...
import { CheckCircle2, CreditCard, Download, Landmark, Plus, Search } from "lucide-react"
...
export default function BookingPaymentsPage() {
  ...
  return <div className="min-h-screen bg-[#171512] text-[#e7e1d8]">
    <header className="flex min-h-[58px] items-center justify-between gap-3 border-b border-white/[0.07] bg-[#211e1a] px-4 py-2"><div className="min-w-0"><h1 className="truncate text-lg font-medium">{copy.title}</h1><p className="truncate text-xs text-[#b9b0a4]">{copy.subtitle}</p></div><button type="button" onClick={()=>downloadCsv(`payments-${new Date().toISOString().slice(0,10)}.csv`,[copy.guest,copy.stay,copy.total,copy.paid,copy.balance,copy.status],visibleRows.map(row=>[row.reservation.guest_name,`${row.reservation.check_in} → ${row.reservation.check_out}`,row.total,row.paid,row.balance,row.status]))} className="inline-flex h-8 items-center gap-2 bg-[#2b2722] px-3 text-xs"><Download className="h-3.5 w-3.5"/>CSV</button></header>
