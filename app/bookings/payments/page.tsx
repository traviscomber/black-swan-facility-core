...
export default function BookingPaymentsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (queryError) setError(queryError.message); else setReservations((data ?? []) as Reservation[])
    setLoading(false)
  }, [supabase])
  ...
  return <div className="min-h-screen bg-background p-4 md:p-6"><div className="mx-auto max-w-7xl space-y-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric title={copy.bookedValue} value={formatClp(metrics.total)} icon={<CircleDollarSign className="h-4 w-4" />} /><Metric title={copy.paid} value={formatClp(metrics.paid)} icon={<CreditCard className="h-4 w-4" />} /><Metric title={copy.pendingBalance} value={formatClp(metrics.balance)} icon={<WalletCards className="h-4 w-4" />} /><Metric title={copy.overdueBalance} value={formatClp(metrics.overdue)} icon={<CircleDollarSign className="h-4 w-4" />} /></div>
