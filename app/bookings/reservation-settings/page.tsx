...
export default function BookingReservationSettingsPage() {
  ...
  return <section className="min-h-screen bg-[#171512] text-[#e7e1d8]">
    ...
    {!settings && !error ? <div className="px-5 py-10 text-sm text-[#8f867b]">{c.loading}</div> : <div className="max-w-4xl p-5">
      <Block title={c.stay}><div className="mb-px bg-[#211e1a] p-4"><div className="text-xs text-[#b9b0a4]">{c.timezone}</div><div className="mt-1 text-sm text-[#e7e1d8]">{settings?.timezone || BOOKING_TIME_ZONE}</div><div className="mt-1 text-[11px] text-[#8f867b]">{c.timezoneHelp}</div></div><div className="grid gap-px bg-[#39342d] sm:grid-cols-2"><TimeField label={c.checkIn} value={form.check_in_time} onChange={(value) => setForm({ ...form, check_in_time: value })} /><TimeField label={c.checkOut} value={form.check_out_time} onChange={(value) => setForm({ ...form, check_out_time: value })} /></div></Block>
