...
function OperationCard({ title, subtitle, status, description, actions }: { title: string; subtitle: string; status: string; description: string | null; actions: React.ReactNode }) { return <div className="col-span-full rounded-lg border p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{title}</p><p className="mt-1 text-xs text-muted-foreground">{subtitle}</p></div><Badge variant="outline">{status}</Badge></div>{description && <p className="mt-3 text-sm leading-6">{description}</p>}<div className="mt-3 flex flex-wrap gap-2">{actions}</div></div> }
