...
interface QuoteOption { room_id: string; room_number: string; room_type: string; location: string | null; capacity: number; nights: number; base_rate: number; required_min_stay: number; nightly_rates: NightlyRate[]; lodging_subtotal: number; lodging_tax: number; extras_subtotal: number; extras_tax: number; service_fee: number; total: number }
interface QuoteResponse { check_in: string; check_out: string; nights: number; guests: number; currency: string; options: QuoteOption[] }
...
export default function BookingQuotesPage() {
  ...
  async function calculate() {
    ...
    setLoading(true)
    try {
      const response = await fetch("/api/bookings/quote", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ check_in: form.check_in, check_out: form.check_out, guests: Number(form.guests), extras: selectedExtras }) })
  ...
  return <div className="min-h-screen bg-background p-4 md:p-6"><div className="mx-auto max-w-7xl space-y-5">
    <div><h1 className="text-3xl font-bold tracking-tight">{copy.title}</h1><p className="text-sm text-muted-foreground">{copy.subtitle}</p></div>
    <Card><CardHeader><CardTitle>{copy.parameters}</CardTitle></CardHeader><CardContent className="space-y-5">
      <div className="grid gap-4 md:grid-cols-3"><Field label="Check-in"><Input type="date" value={form.check_in} onChange={(event) => setForm({ ...form, check_in: event.target.value })} /></Field><Field label="Check-out"><Input type="date" value={form.check_out} onChange={(event) => setForm({ ...form, check_out: event.target.value })} /></Field><Field label={copy.guests}><Input type="number" min="1" value={form.guests} onChange={(event) => setForm({ ...form, guests: event.target.value })} /></Field></div>
      <div className="space-y-3"><Label>{copy.extras}</Label><div className="flex gap-2"><Select value={selectedExtraId} onValueChange={setSelectedExtraId}><SelectTrigger className="max-w-md"><SelectValue placeholder={copy.selectExtra} /></SelectTrigger><SelectContent>{extras.map((extra) => <SelectItem key={extra.id} value={extra.id}>{extra.name} · {money(Number(extra.price))}</SelectItem>)}</SelectContent></Select><Button type="button" variant="outline" onClick={addExtra} disabled={!selectedExtraId}><Plus className="mr-2 h-4 w-4" />{copy.add}</Button></div>
      {selectedExtras.length > 0 && <div className="space-y-2">{selectedExtras.map((item) => { const extra = extras.find((candidate) => candidate.id === item.extra_id); if (!extra) return null; return <div key={item.extra_id} className="flex items-center gap-3 rounded-lg border p-3"><div className="flex-1"><div className="font-medium">{extra.name}</div><div className="text-xs text-muted-foreground">{extra.unit} · {copy.tax} {Number(extra.tax_rate)}%</div></div><Input className="w-28" type="number" min="0" step="1" value={item.quantity} onChange={(event) => updateExtra(item.extra_id, Number(event.target.value))} /><Button size="icon" variant="ghost" onClick={() => removeExtra(item.extra_id)}><Trash2 className="h-4 w-4" /></Button></div> })}</div>}</div>
    ...
    {quote && <div className="space-y-4"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-xl font-semibold">{copy.availableOptions}</h2><p className="text-sm text-muted-foreground">{quote.nights} {copy.nights} · {quote.guests} {copy.guestCount} · {quote.options.length} {copy.alternatives}</p></div><div className="relative w-full md:w-80"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={copy.search} /></div></div>
      {filteredOptions.length === 0 ? <Card><CardContent className="p-10 text-center text-muted-foreground">{copy.noOptions}</CardContent></Card> : <div className="grid gap-4 lg:grid-cols-2">{filteredOptions.map((option) => <Card key={option.room_id}><CardHeader><CardTitle className="flex items-center justify-between gap-3"><span>{copy.room} {option.room_number} · {option.room_type}</span><span>{money(Number(option.total), quote.currency)}</span></CardTitle><p className="text-sm text-muted-foreground">{option.location || copy.noLocation} · {copy.capacity} {option.capacity}</p></CardHeader><CardContent className="space-y-4 text-sm"><div className="grid grid-cols-2 gap-3"><Metric label={copy.lodging} value={money(Number(option.lodging_subtotal), quote.currency)} /><Metric label={copy.lodgingTax} value={money(Number(option.lodging_tax), quote.currency)} /><Metric label={copy.extras} value={money(Number(option.extras_subtotal), quote.currency)} /><Metric label={copy.extrasTax} value={money(Number(option.extras_tax), quote.currency)} /></div><div className="rounded-lg border p-3"><div className="mb-2 font-medium">{copy.nightlyDetail}</div><div className="space-y-1 text-xs text-muted-foreground">{option.nightly_rates.map((night) => <div key={night.date} className="flex justify-between"><span>{night.date}{night.season ? ` · ${night.season}` : ""}</span><span>{money(Number(night.rate), quote.currency)}</span></div>)}</div></div></CardContent></Card>)}</div>}
    </div>}
  </div></div>
}

function Field({ label, children }: { label: string; children: React.ReactNode }) { return <div className="space-y-2"><Label>{label}</Label>{children}</div> }
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">{label}</div><div className="font-semibold">{value}</div></div> }
