...
function blockDays(start:string,end:string){ return Math.max(1,Math.ceil((new Date(`${end}T00:00:00Z`).getTime()-new Date(`${start}T00:00:00Z`).getTime())/86400000)) }
...
export default function RoomBlocksPage() {
  ...
  return <div className="min-h-screen bg-[#111213] text-foreground">
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[1020px] text-xs"><thead className="bg-[#17191a] text-left text-[11px] uppercase tracking-[0.08em] text-muted-foreground"><tr><th className="px-3 py-2">{copy.room}</th><th className="px-4 py-2.5">{copy.type}</th><th className="px-4 py-2.5">{copy.from}</th><th className="px-4 py-2.5">{copy.to}</th><th className="px-3 py-2">{copy.reason}</th><th className="px-3 py-2">{copy.duration}</th><th className="px-3 py-2">{copy.status}</th><th className="px-3 py-2 text-right">{copy.actions}</th></tr></thead><tbody className="divide-y divide-white/[0.06]">{loading ? <tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground">{copy.loading}</td></tr> : filtered.length === 0 ? <tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground">{copy.none}</td></tr> : filtered.map((block) => { const room = roomOf(block); return <tr key={block.id} className="bg-[#111213] hover:bg-white/[0.025]"><td className="px-4 py-2.5 font-medium">{room?.room_number || "—"}<div className="text-[11px] font-normal text-muted-foreground">{room?.location || "—"}</div></td><td className="px-4 py-2.5">{typeLabels[block.block_type] || block.block_type}</td><td className="px-4 py-2.5 text-muted-foreground">{block.start_date}</td><td className="px-4 py-2.5 text-muted-foreground">{block.end_date}</td><td className="max-w-md px-3 py-2"><div>{block.reason}</div>{block.notes && <div className="line-clamp-1 text-[11px] text-muted-foreground">{block.notes}</div>}</td><td className="px-3 py-2 tabular-nums text-muted-foreground">{blockDays(block.start_date,block.end_date)} {copy.days}</td><td className="px-3 py-2"><Badge variant="outline" className="h-5 rounded-[3px] px-1.5 text-[10px]">{block.status}</Badge></td><td className="px-4 py-2.5"><div className="flex justify-end gap-1">{block.status === "active" && <><Button size="sm" variant="outline" className="h-7 rounded-[3px] px-2 text-[11px]" onClick={() => void updateStatus(block.id, "completed")}>{copy.complete}</Button><Button size="sm" variant="ghost" className="h-7 rounded-[3px] px-2 text-[11px]" onClick={() => void updateStatus(block.id, "cancelled")}>{copy.cancel}</Button></>}</div></td></tr> })}</tbody></table></div>
  </div>
}
