...
interface Invoice { id: string; reservation_id: string | null; invoice_number: string; invoice_date: string; due_date: string; customer_name: string; customer_email: string | null; total_amount: number; amount_paid: number | null; payment_status: string; status: string; finalized_at: string | null }
...
export default function InvoicesPage() {
  ...
  const loadInvoices = useCallback(async () => {
    ...
    const { data, error: queryError } = await supabase
      .from("invoices")
      .select("id, reservation_id, invoice_number, invoice_date, due_date, status, customer_name, customer_email, total_amount, payment_status, amount_paid, finalized_at")
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[980px] text-xs">
        ...
        <tbody>{loading ? <tr><td colSpan={8} className="px-4 py-12 text-center text-[#b9b0a4]">{copy.loading}</td></tr> : filteredInvoices.length === 0 ? <tr><td colSpan={8} className="px-4 py-12 text-center"><FileText className="mx-auto mb-2 h-5 w-5 text-[#b9b0a4]" /><div className="text-sm">{copy.empty}</div><div className="mt-1 text-xs text-[#b9b0a4]">{copy.emptyHint}</div></td></tr> : filteredInvoices.map((invoice) => {
          ...
          return <tr key={invoice.id} className="border-b border-white/[0.05] hover:bg-white/[0.025]"><td className="px-3 py-2 font-medium">{invoice.invoice_number}</td><td className="px-3 py-2"><div>{invoice.customer_name}</div><div className="text-[10px] text-[#b9b0a4]">{invoice.customer_email || "—"}</div></td><td className="px-3 py-2">{formatDate(invoice.invoice_date)}</td><td className="px-3 py-2">{formatDate(invoice.due_date)}</td><td className="px-3 py-2"><Badge variant="outline" className={`h-5 rounded-none border-0 px-1.5 text-[10px] ${getStatusClass(invoice.payment_status)}`}>{paymentStatusLabels[invoice.payment_status] ?? invoice.payment_status}</Badge></td><td className="px-3 py-2 text-right font-medium">{formatClp(invoice.total_amount, locale)}</td><td className="px-3 py-2 text-right">{formatClp(balance, locale)}</td><td className="px-3 py-2"><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" className="h-7 w-7 rounded-none" onClick={() => { setSelectedInvoice(invoice); setEditorOpen(true) }} aria-label={`${copy.view} ${invoice.invoice_number}`}><Eye className="h-3.5 w-3.5" /></Button>{canEdit && !invoice.finalized_at && invoice.status !== "finalized" && <Button size="icon" variant="ghost" className="h-7 w-7 rounded-none" onClick={() => { setSelectedInvoice(invoice); setEditorOpen(true) }} aria-label={`${copy.edit} ${invoice.invoice_number}`}><Edit className="h-3.5 w-3.5" /></Button>}{canDelete && !invoice.finalized_at && invoice.status !== "finalized" && <Button size="icon" variant="ghost" className="h-7 w-7 rounded-none" onClick={() => void handleDeleteInvoice(invoice)} aria-label={`${copy.delete} ${invoice.invoice_number}`}><Trash2 className="h-3.5 w-3.5" /></Button>}</div></td></tr>
        })}</tbody>
      </table>
    </div>
