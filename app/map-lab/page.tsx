...
import { ArrowLeft, CheckCircle2, Layers, Loader2, MapPin, Satellite } from "lucide-react"
...
export default function MapLabPage() {
  ...
  useEffect(() => {
    ...
    stylesheet.dataset.maplibreLab = "true"
  ...
  useEffect(() => {
    ...
    async function loadMap() {
      ...
      try {
        ...
        map.on("load", async () => {
          ...
          for (const [index, overlay] of overlayRows.entries()) {
            try {
              ...
              console.error(`MapLibre lab could not load ${overlay.name}`, overlayError)
        ...
        console.error("MapLibre lab failed", loadError)
        setError(loadError instanceof Error ? loadError.message : "No fue posible iniciar la prueba MapLibre.")
      } finally {
    ...
    <AppLayout>
      ...
      <div className="space-y-5 p-4 sm:p-8">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Metric icon={CheckCircle2} label="Motor" value={`MapLibre ${MAPLIBRE_VERSION}`} />
        </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
          <Card className="overflow-hidden">
            <CardContent className="relative p-0">
              ...
              {loading && <div className="absolute inset-0 flex items-center justify-center gap-2 bg-background/70 text-sm text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" />Preparando comparación MapLibre…</div>}
            </CardContent>
          </Card>
          ...
          <div className="space-y-5">
            ...
            <Card>
              <CardHeader><CardTitle className="text-base">Qué comparar</CardTitle></CardHeader>
              <CardContent className="space-y-2 text-sm text-muted-foreground">
                <p>• Nitidez de la imagen satelital al ampliar.</p>
                <p>• Fluidez con las capas KMZ visibles.</p>
                <p>• Claridad de puntos y conexiones.</p>
                <p>• Uso táctil y lectura en móvil.</p>
                <Badge variant="outline">Prueba sin cambios de datos</Badge>
              </CardContent>
            </Card>
          </div>
        </div>
