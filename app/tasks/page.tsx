...
export default function TasksPage() {
  ...
  async function fetchTasks() {
    ...
    if (fetchError) { console.error("tasks load failed", fetchError); setError(copy.loadError) } else {
      ...
      if (selectedId) {
        ...
        if (selected && isDemoTask(selected)) setActiveTab("demo")
        else if (selected && isAsanaLegacyTask(selected)) setActiveTab("asana")
        else if (selected?.status === "completada") setActiveTab("completadas")
  ...
  const asanaReconciliationTasks = tasks.filter(isAsanaLegacyTask)
  ...
  const filteredTasks = tasks.filter((task) => activeTab === "operacion" ? isOpenTask(task) && !isAsanaLegacyTask(task) && !isDemoTask(task) : activeTab === "asana" ? isAsanaLegacyTask(task) : activeTab === "demo" ? isOpenTask(task) && isDemoTask(task) : activeTab === "completadas" ? task.status === "completada" : activeTab === "canceladas" ? task.status === "cancelada" : true)
