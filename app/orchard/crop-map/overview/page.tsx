 ...
 return <AppLayout><OrchardNavigation/><main className="flex h-[calc(100dvh-var(--orchard-nav-height,0px))] min-h-[680px] flex-col overflow-hidden bg-[#1a1a18] text-[#e8e5dc]">
  ...
  {loading?<div className="grid flex-1 place-items-center text-sm text-[#8f8a81]">…</div>:error&&!plan?<div className="grid flex-1 place-items-center text-sm text-red-300">{error}</div>:plan?<>
   ...
   <div className="flex min-h-0 flex-1">
    <section className="min-w-0 flex-1 overflow-auto bg-[#242522] p-3">
     <div className="flex min-h-[575px] w-max min-w-full items-start gap-2 border border-[#5b5b55] bg-[#1c1d1a] p-3">
    ...
    <aside className="w-[320px] shrink-0 border-l border-white/10 bg-[#171715] xl:w-[350px]">
...
function PlotTimeline({plot,beds,allocations,months,plan,selectedDate,cycleById,successionById,familyFor,onDrop,dragId}:{plot:Plot;beds:Bed[];allocations:Allocation[];months:{key:string;label:string;start:string;end:string}[];plan:Plan;selectedDate:string;cycleById:Map<string,Cycle>;successionById:Map<string,Succession>;familyFor:(crop:string)=>string|null;onDrop:(id:string,plot:Plot,bed:Bed)=>Promise<void>;dragId:string|null}){
 const width=Math.max(108,beds.length*22);const selectedTop=pctDate(selectedDate,plan.start_date,plan.end_date)
 return <div className="shrink-0" style={{width}}><div className="mb-1 flex items-center justify-between px-1"><h3 className="text-sm font-semibold">{plot.name}</h3><span className="text-[10px] text-[#77726a]">{beds.length}</span></div><div className="grid h-5 border-x border-t border-[#55564f] bg-[#22231f]" style={{gridTemplateColumns:`repeat(${beds.length},minmax(20px,1fr))`}}>{beds.map((bed,index)=><div key={bed.id} className="border-r border-[#454640] text-center text-[8px] leading-5 text-[#8f8a81] last:border-r-0">{index+1}</div>)}</div><div className="relative h-[510px] border border-[#55564f] bg-[#20211e]">
  ...
  <div className="absolute inset-0 grid" style={{gridTemplateColumns:`repeat(${beds.length},minmax(20px,1fr))`}}>{beds.map(bed=><div key={bed.id} onDragOver={event=>{if(dragId)event.preventDefault()}} onDrop={event=>{event.preventDefault();const id=dragId??event.dataTransfer.getData("text/plain");if(id)void onDrop(id,plot,bed)}} className={`border-r border-[#3d3e39] last:border-r-0 ${dragId?"hover:bg-[#79c5aa]/10":""}`}/>)}</div>
