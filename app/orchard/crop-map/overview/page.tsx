...
export default function CropMapOverview(){
 ...
 ]).then(([p,pl,b,c,s,a,l])=>{if(!live)return;const first=p.error??pl.error??b.error??c.error??s.error??a.error??l.error;if(first){setError(`${text.loadError} ${first.message}`);setLoading(false);return}setPlans((p.data??[]) as Plan[]);setPlots((pl.data??[]) as Plot[]);setBeds((b.data??[]) as Bed[]);setCycles((c.data??[]) as Cycle[]);setSuccessions((s.data??[]) as Succession[]);setAllocations((a.data??[]) as Allocation[]);setProfiles((l.data??[]) as CropProfile[]);setLoading(false)});return()=>{live=false}},[supabase,text.loadError])
 ...
 const familyByCrop=new Map(profiles.map(profile=>[normalize(profile.crop_name),profile.crop_family]));const familyFor=(crop:string|null)=>crop?familyByCrop.get(normalize(crop))??null:null
 ...
 return <AppLayout><OrchardNavigation/><main className="mx-auto w-full max-w-[1760px] px-4 pb-16 pt-6 sm:px-6 lg:px-8">
  ...
  {loading?<div className="py-12 text-sm text-muted-foreground">…</div>:error?<div className="border-y border-red-400/30 py-4 text-sm text-red-300">{error}</div>:<>
   <section className="mb-4 grid border-y border-[var(--bs-divider-subtle)] sm:grid-cols-3 xl:grid-cols-6"><Metric icon={MapIcon} label={text.blocks} value={physicalPlots.length}/><Metric icon={Rows3} label={text.beds} value={physicalBeds.length}/><Metric label={text.assigned} value={planAllocatedIds.size} suffix="/32"/><Metric label={text.occupied} value={occupiedBedIds.size}/><Metric label={text.planOccupied} value={planOccupiedBedIds.size}/><Metric label={text.meters} value={Math.round(occupiedMeters)} suffix={`/${Math.round(totalMeters)} m`}/></section>

   <section className="mb-4 border-y border-[var(--bs-divider-subtle)] py-3"><div className="grid gap-3 lg:grid-cols-[minmax(220px,auto)_1fr_minmax(230px,auto)] lg:items-center"><div className="flex items-center gap-3"><CalendarDays className="h-4 w-4 text-[var(--orchard-green)]"/><div><p className="text-[10px] uppercase tracking-[.14em] text-muted-foreground">{text.date}</p><p className="mt-0.5 text-sm font-medium">{new Date(`${selectedDate}T12:00:00`).toLocaleDateString(locale,{weekday:"short",day:"2-digit",month:"long",year:"numeric"})}</p></div></div><input aria-label={text.date} type="range" min={0} max={Math.max(0,weeks.length-1)} value={Math.min(dateIndex,Math.max(0,weeks.length-1))} onChange={e=>setDateIndex(Number(e.target.value))} className="orchard-date-range w-full cursor-pointer"/><div><p className="text-[10px] uppercase tracking-[.14em] text-muted-foreground">{text.nextActivity}</p><p className="mt-0.5 text-sm font-medium">{nextPlanLabel}</p></div></div><div className="mt-2 flex flex-wrap gap-4 text-[11px] text-muted-foreground"><Legend color="#536159" label={`${text.legendPlan} · crop color`}/><Legend color="#536159" opacity={0.45} label={text.legendOther}/><Legend color="var(--bs-surface-tertiary)" label={text.legendFree}/></div></section>

   <section><div className="mb-3 flex flex-col gap-1 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs uppercase tracking-[.16em] text-muted-foreground">{text.canvas}</p><p className="mt-1 max-w-4xl text-xs leading-5 text-muted-foreground">{text.canvasHelp}</p></div></div>
    <div className="grid gap-px overflow-hidden border-y border-[var(--bs-divider-subtle)] bg-[var(--bs-divider-subtle)] lg:grid-cols-2 2xl:grid-cols-4">{physicalPlots.map(plot=>{const blockBeds=physicalBeds.filter(b=>b.plot_id===plot.id).sort((a,b)=>(a.planning_order??0)-(b.planning_order??0));return <section key={plot.id} className="bg-[var(--bs-surface-primary)]"><div className="flex items-center justify-between border-b border-[var(--bs-divider-subtle)] px-4 py-2.5"><strong className="font-medium">{plot.name}</strong><span className="text-[11px] text-muted-foreground">{blockBeds.length} {text.beds.toLowerCase()}</span></div><div className="px-2">{blockBeds.map(bed=>{const active=activeByBed.get(bed.id)??[];const planActive=active.filter(a=>planSuccessionIds.has(a.crop_succession_id));const used=active.reduce((sum,a)=>sum+Number(a.allocated_length_m??0),0);const capacity=Number(bed.length_m??0);const otherActive=active.length>planActive.length;const visualSegments=active.map(a=>{const length=Math.max(0,Number(a.allocated_length_m??0));const crop=cropForAllocation(a);const isPlan=planSuccessionIds.has(a.crop_succession_id);const color=crop?cropColor(crop,familyFor(crop)):"#66736a";const width=capacity>0?Math.min(100,(length/capacity)*100):0;return {a,length,crop,isPlan,color,width}});return <div key={bed.id} className="grid grid-cols-[64px_1fr] items-center gap-2 border-b border-[var(--bs-divider-subtle)] px-1 py-1.5 last:border-b-0"><span className="truncate text-[10px] text-muted-foreground" title={bed.name}>{bed.name}</span><div><div className="flex h-3 overflow-hidden bg-[var(--bs-surface-tertiary)]">{visualSegments.map(segment=><div key={segment.a.id} className="h-full shrink-0 border-r border-black/10 last:border-r-0" style={{width:`${segment.width}%`,backgroundColor:segment.color,opacity:segment.isPlan?1:0.45}} title={`${segment.crop??text.legendOther} · ${segment.length.toLocaleString(locale,{maximumFractionDigits:1})} m`}/>)}</div><div className="mt-0.5 flex items-start justify-between gap-2 text-[8px] text-muted-foreground"><span className="flex min-w-0 flex-wrap gap-x-2 gap-y-0.5">{visualSegments.length?visualSegments.map(segment=><span key={segment.a.id} className="inline-flex items-center gap-1 whitespace-nowrap"><i className="h-1.5 w-1.5 shrink-0" style={{backgroundColor:segment.color,opacity:segment.isPlan?1:0.45}}/><span>{segment.crop??text.legendOther} {segment.length.toLocaleString(locale,{maximumFractionDigits:1})}m</span></span>):<span>{otherActive?text.legendOther:text.legendFree}</span>}</span><span className="shrink-0 tabular-nums">{used.toLocaleString(locale,{maximumFractionDigits:1})}/{capacity.toLocaleString(locale,{maximumFractionDigits:1})} m</span></div></div></div>})}</div></section>})}</div>
   </section>
   <footer className="mt-4 flex justify-end border-t border-[var(--bs-divider-subtle)] pt-4"><Link href={advancedHref} className="inline-flex items-center gap-2 text-sm text-foreground">{text.advanced}<ArrowRight className="h-4 w-4"/></Link></footer>
  </>}
 </main></AppLayout>
}
function Metric({icon:Icon,label,value,suffix}:{icon?:typeof MapIcon;label:string;value:number;suffix?:string}){return <div className="border-r border-[var(--bs-divider-subtle)] px-3 py-3 last:border-r-0">{Icon?<Icon className="h-3.5 w-3.5 text-muted-foreground"/>:null}<p className={`${Icon?"mt-2":""} text-[10px] uppercase tracking-[.1em] text-muted-foreground`}>{label}</p><p className="mt-1 text-2xl tabular-nums">{value}<span className="ml-1 text-xs text-muted-foreground">{suffix}</span></p></div>}
function Legend({color,label,opacity=1}:{color:string;label:string;opacity?:number}){return <span className="inline-flex items-center gap-2"><span className="h-2 w-2" style={{backgroundColor:color,opacity}}/>{label}</span>}
