...
const FARM_AREA_PREFIX = "Orchard BlackSwan Campo"

const copy = {
  en: { title:"Organize My Crop Map", description:"Assign the selected Game Plan to real beds using bed-meter capacity across the Farm Area 1 cultivation sectors.", refresh:"Refresh", gis:"Open GIS", plan:"Game Plan", assigned:"assigned plantings", pending:"Pending", areaCapacity:"Farm Area 1 capacity", peakDemand:"Plan peak", sectors:"Cultivation sectors", spareBeds:"Spare beds", queue:"Plantings to assign", queueHelp:"Only reconciled plantings without a safe physical allocation remain here. Capacity conflicts stay visible instead of forcing invalid placement.", fieldBlock:"Cultivation sector", startBed:"Starting bed", required:"Required", dateRange:"Date range", choosePlanting:"Choose a planting", chooseBed:"Choose the first bed. Large plantings continue into following beds within the selected sector.", available:"available", used:"used", place:"Assign planting", placing:"Assigning…", placed:"Planting assigned", bedsUsed:"beds used", noQueue:"All reconciled plantings are assigned.", insufficient:"Not enough contiguous bed-meter capacity from this starting bed.", crop:"Crop", source:"Core dates remain canonical. Farm Area 1 now contains three logical cultivation sectors: the observed 18-bed block plus two 4-bed expansion sectors. No GIS coordinates are fabricated until Core supports per-sector polygon geometry.", capacityConflict:"Capacity conflict", fixedPeak:"Fixed Core dates require", aboveCapacity:"above physical Farm Area 1 capacity", deferred:"bed m remain deferred across", plantings:"plantings", currentLayout:"The current layout respects every bed limit and uses all Farm Area 1 cultivation sectors." },
  es: { title:"Organizar mi Mapa de Cultivos", description:"Asigna el Plan seleccionado a camas reales usando capacidad en metros de cama a través de los sectores de cultivo de Farm Area 1.", refresh:"Actualizar", gis:"Abrir GIS", plan:"Plan de Cultivo", assigned:"plantaciones asignadas", pending:"Pendientes", areaCapacity:"Capacidad Farm Area 1", peakDemand:"Peak del plan", sectors:"Sectores de cultivo", spareBeds:"Camas libres", queue:"Plantaciones por asignar", queueHelp:"Aquí quedan sólo las plantaciones reconciliadas sin ubicación física segura. Los conflictos de capacidad permanecen visibles en vez de forzar asignaciones inválidas.", fieldBlock:"Sector de cultivo", startBed:"Cama inicial", required:"Requerido", dateRange:"Rango de fechas", choosePlanting:"Elige una plantación", chooseBed:"Elige la primera cama. Las plantaciones grandes continúan por las camas siguientes dentro del sector seleccionado.", available:"disponible", used:"usado", place:"Asignar plantación", placing:"Asignando…", placed:"Plantación asignada", bedsUsed:"camas usadas", noQueue:"Todas las plantaciones reconciliadas están asignadas.", insufficient:"No existe capacidad contigua suficiente desde esta cama.", crop:"Cultivo", source:"Las fechas de Core siguen siendo canónicas. Farm Area 1 ahora contiene tres sectores lógicos de cultivo: el bloque observado de 18 camas más dos sectores de expansión de 4 camas. No se inventan coordenadas GIS hasta que Core soporte geometría por sector.", capacityConflict:"Conflicto de capacidad", fixedPeak:"Las fechas canónicas de Core exigen", aboveCapacity:"sobre la capacidad física total de Farm Area 1", deferred:"m de cama quedan diferidos en", plantings:"plantaciones", currentLayout:"La distribución actual respeta el límite de cada cama y utiliza los sectores de cultivo de Farm Area 1." },
  de: { title:"Anbaukarte organisieren", description:"Ordne den ausgewählten Game Plan realen Beeten über die Anbausektoren von Farm Area 1 zu.", refresh:"Aktualisieren", gis:"GIS öffnen", plan:"Game Plan", assigned:"zugeordnete Pflanzungen", pending:"Offen", areaCapacity:"Kapazität Farm Area 1", peakDemand:"Plan-Spitze", sectors:"Anbausektoren", spareBeds:"Freie Beete", queue:"Pflanzungen zuordnen", queueHelp:"Hier bleiben nur abgeglichene Pflanzungen ohne sichere physische Zuordnung. Kapazitätskonflikte bleiben sichtbar.", fieldBlock:"Anbausektor", startBed:"Startbeet", required:"Benötigt", dateRange:"Zeitraum", choosePlanting:"Pflanzung auswählen", chooseBed:"Wähle das erste Beet. Große Pflanzungen laufen innerhalb des gewählten Sektors auf die folgenden Beete weiter.", available:"verfügbar", used:"belegt", place:"Pflanzung zuordnen", placing:"Zuordnung…", placed:"Pflanzung zugeordnet", bedsUsed:"Beete verwendet", noQueue:"Alle abgeglichenen Pflanzungen sind zugeordnet.", insufficient:"Ab diesem Startbeet gibt es nicht genügend zusammenhängende Beetmeter-Kapazität.", crop:"Kultur", source:"Core-Daten bleiben kanonisch. Farm Area 1 enthält jetzt drei logische Anbausektoren: den beobachteten 18-Beet-Block plus zwei Erweiterungssektoren mit je 4 Beeten. GIS-Koordinaten werden nicht erfunden.", capacityConflict:"Kapazitätskonflikt", fixedPeak:"Die festen Core-Daten benötigen", aboveCapacity:"über der physischen Gesamtkapazität von Farm Area 1", deferred:"Beetmeter bleiben über", plantings:"Pflanzungen zurückgestellt", currentLayout:"Die aktuelle Zuordnung hält jedes Beetlimit ein und nutzt die Anbausektoren von Farm Area 1." },
} as const
...
export default function OrchardAutoPlacePage(){
  ...
  const load=useCallback(async()=>{
    ...
    setPlotId(current=>current||nextPlots.find(x=>x.name===FARM_AREA_PREFIX)?.id||nextBeds[0]?.plot_id||"")
    setLoading(false)
  },[supabase])
  ...
  const farmPlots=useMemo(()=>plots.filter(p=>p.name.startsWith(FARM_AREA_PREFIX)),[plots])
