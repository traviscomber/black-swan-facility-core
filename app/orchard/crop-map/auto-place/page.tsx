...
const FARM_AREA_BLOCK_SET = new Set<string>(FARM_AREA_BLOCK_NAMES)

const copy = {
  en: { title:"Organize My Crop Map", description:"Review and maintain the 2026/27 Game Plan across the canonical Farm Area 1 bed layout.", refresh:"Refresh", gis:"Open GIS", plan:"Game Plan", assigned:"assigned plantings", pending:"Pending", areaCapacity:"Farm Area 1 capacity", peakDemand:"Plan peak", sectors:"Field blocks", spareBeds:"Never-used beds", queue:"Plantings to assign", queueHelp:"Only reconciled plantings without a physical allocation remain here. Existing assignments stay canonical and capacity conflicts remain visible.", fieldBlock:"Field block", startBed:"Starting bed", required:"Required", dateRange:"Date range", choosePlanting:"Choose a planting", chooseBed:"Choose the first bed inside this block. Placements that exceed one 10 m bed continue through the following beds in the same block.", available:"available", used:"used", place:"Assign planting", placing:"Assigning…", placed:"Planting assigned", bedsUsed:"beds used", noQueue:"All reconciled plantings are physically assigned.", insufficient:"Not enough contiguous bed-meter capacity from this starting bed in the selected block.", crop:"Crop", source:"Current canonical layout: 5 current blocks + 3 expansion blocks, each with 10 beds of 10.0 m × 0.76 m. Farm Area 1 therefore provides 80 active beds and 800 bed-m of physical capacity. The 0.46 m path standard is recorded separately and is not counted as cultivable bed area. Legacy 18×30 m Heirloom geometry remains reference evidence only.", capacityConflict:"Capacity conflict", fixedPeak:"Fixed Core dates require", aboveCapacity:"above physical Farm Area 1 capacity", deferred:"bed m remain deferred across", plantings:"plantings", currentLayout:"The current allocation is measured against the eight canonical Farm Area 1 blocks." },
  es: { title:"Organizar mi Mapa de Cultivos", description:"Revisa y mantiene el Plan 2026/27 sobre la distribución canónica de camas de Farm Area 1.", refresh:"Actualizar", gis:"Abrir GIS", plan:"Plan de Cultivo", assigned:"plantaciones asignadas", pending:"Pendientes", areaCapacity:"Capacidad Farm Area 1", peakDemand:"Peak del plan", sectors:"Bloques de campo", spareBeds:"Camas nunca usadas", queue:"Plantaciones por asignar", queueHelp:"Aquí quedan sólo las plantaciones reconciliadas sin ubicación física. Las asignaciones existentes siguen siendo canónicas y los conflictos de capacidad permanecen visibles.", fieldBlock:"Bloque de campo", startBed:"Cama inicial", required:"Requerido", dateRange:"Rango de fechas", choosePlanting:"Elige una plantación", chooseBed:"Elige la primera cama dentro de este bloque. Las plantaciones que superan una cama de 10 m continúan por las camas siguientes del mismo bloque.", available:"disponible", used:"usado", place:"Asignar plantación", placing:"Asignando…", placed:"Plantación asignada", bedsUsed:"camas usadas", noQueue:"Todas las plantaciones reconciliadas están físicamente asignadas.", insufficient:"No existe capacidad contigua suficiente desde esta cama dentro del bloque seleccionado.", crop:"Cultivo", source:"Layout canónico actual: 5 bloques actuales + 3 expansiones, cada uno con 10 camas de 10,0 m × 0,76 m. Farm Area 1 dispone así de 80 camas activas y 800 m de cama de capacidad física. El sendero estándar de 0,46 m se registra por separado y no se suma al área cultivable. La geometría Heirloom anterior de 18×30 m queda sólo como evidencia de referencia.", capacityConflict:"Conflicto de capacidad", fixedPeak:"Las fechas canónicas de Core exigen", aboveCapacity:"sobre la capacidad física total de Farm Area 1", deferred:"m de cama quedan diferidos en", plantings:"plantaciones", currentLayout:"La asignación actual se mide contra los ocho bloques canónicos de Farm Area 1." },
  de: { title:"Anbaukarte organisieren", description:"Prüfe und pflege den Game Plan 2026/27 auf dem kanonischen Beetlayout von Farm Area 1.", refresh:"Aktualisieren", gis:"GIS öffnen", plan:"Game Plan", assigned:"zugeordnete Pflanzungen", pending:"Offen", areaCapacity:"Kapazität Farm Area 1", peakDemand:"Plan-Spitze", sectors:"Feldblöcke", spareBeds:"Nie belegte Beete", queue:"Pflanzungen zuordnen", queueHelp:"Hier bleiben nur abgeglichene Pflanzungen ohne physische Zuordnung. Bestehende Zuordnungen bleiben kanonisch und Kapazitätskonflikte sichtbar.", fieldBlock:"Feldblock", startBed:"Startbeet", required:"Benötigt", dateRange:"Zeitraum", choosePlanting:"Pflanzung auswählen", chooseBed:"Wähle das erste Beet innerhalb dieses Blocks. Pflanzungen über 10 m laufen auf die folgenden Beete desselben Blocks weiter.", available:"verfügbar", used:"belegt", place:"Pflanzung zuordnen", placing:"Zuordnung…", placed:"Pflanzung zugeordnet", bedsUsed:"Beete verwendet", noQueue:"Alle abgeglichenen Pflanzungen sind physisch zugeordnet.", insufficient:"Ab diesem Startbeet ist im gewählten Block nicht genügend zusammenhängende Beetmeter-Kapazität verfügbar.", crop:"Kultur", source:"Aktuelles kanonisches Layout: 5 bestehende Blöcke + 3 Erweiterungsblöcke mit jeweils 10 Beeten à 10,0 m × 0,76 m. Farm Area 1 hat damit 80 aktive Beete und 800 Beetmeter physische Kapazität. Der Wegstandard von 0,46 m wird separat geführt. Die frühere Heirloom-Geometrie 18×30 m bleibt nur Referenzevidenz.", capacityConflict:"Kapazitätskonflikt", fixedPeak:"Die festen Core-Daten benötigen", aboveCapacity:"über der physischen Gesamtkapazität von Farm Area 1", deferred:"Beetmeter bleiben über", plantings:"Pflanzungen zurückgestellt", currentLayout:"Die aktuelle Zuordnung wird gegen die acht kanonischen Farm-Area-1-Blöcke gemessen." },
} as const
...
export default function OrchardAutoPlacePage(){
  ...
  const load=useCallback(async()=>{
    ...
    setPlotId(current=>current||nextPlots.find(x=>FARM_AREA_BLOCK_SET.has(x.name))?.id||"")
    setLoading(false)
  },[supabase])
  ...
  const farmPlots=useMemo(()=>FARM_AREA_BLOCK_NAMES.map(name=>plots.find(p=>p.name===name)).filter((p):p is Plot=>Boolean(p)),[plots])
  const farmPlotIds=useMemo(()=>new Set(farmPlots.map(p=>p.id)),[farmPlots])
  const farmBeds=useMemo(()=>beds.filter(b=>farmPlotIds.has(b.plot_id)),[beds,farmPlotIds])
  const plotBeds=useMemo(()=>beds.filter(b=>b.plot_id===plotId).sort((a,b)=>(a.planning_order??9999)-(b.planning_order??9999)||a.name.localeCompare(b.name)),[beds,plotId])
  const areaCapacity=farmBeds.reduce((sum,b)=>sum+Number(b.length_m??0),0)
  const spareBeds=farmBeds.filter(b=>!allocations.some(a=>a.bed_id===b.id)).length
  const overCapacity=Math.max(planPeak.bedM-areaCapacity,0)
  ...
  return <AppLayout>
    ...
    <main className="mx-auto w-full max-w-[1560px] space-y-6 px-4 pb-24 pt-6 sm:px-6 lg:px-8">
      ...
      <Card><CardContent className="grid gap-5 p-5 lg:grid-cols-[1fr_280px]"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--orchard-green)]">Crop Map · Farm Area 1</p><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{text.source}</p></div><div><Label>{text.plan}</Label><Select value={selectedPlanId} onValueChange={value=>{setSelectedPlanId(value);setSuccessionId("");setStartBedId("")}}><SelectTrigger className="mt-2"><SelectValue/></SelectTrigger><SelectContent>{plans.map(p=><SelectItem key={p.id} value={p.id}>{p.season??p.name} · {p.status}</SelectItem>)}</SelectContent></Select></div></CardContent></Card>

      <section className="grid overflow-hidden rounded-xl border border-[var(--orchard-line)] bg-white sm:grid-cols-2 xl:grid-cols-6">
        <Metric value={`${assigned}/${scoped.length}`} label={text.assigned}/><Metric value={String(queue.length)} label={text.pending}/><Metric value={`${planPeak.bedM} m`} label={text.peakDemand}/><Metric value={`${areaCapacity} m`} label={text.areaCapacity}/><Metric value={String(farmPlots.length)} label={text.sectors}/><Metric value={String(spareBeds)} label={text.spareBeds}/>
      </section>

      {overCapacity>0?<Card className="border-amber-300 bg-amber-50/70"><CardContent className="flex gap-3 p-5"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-700"/><div><p className="font-medium text-amber-950">{text.capacityConflict}</p><p className="mt-1 text-sm leading-6 text-amber-900">{text.fixedPeak} <strong>{planPeak.bedM} m</strong>{planPeak.date?` · ${planPeak.date}`:""}: <strong>{overCapacity} m</strong> {text.aboveCapacity}. <strong>{deferredBedM} {text.deferred}</strong> {text.plantings} {queue.length}.</p><p className="mt-1 text-xs text-amber-800">{text.currentLayout}</p></div></CardContent></Card>:null}
