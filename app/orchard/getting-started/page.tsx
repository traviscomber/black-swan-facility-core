  ...
  return <AppLayout>
    ...
    <main className="mx-auto w-full max-w-[1560px] px-4 py-8 sm:px-6 lg:px-8">
      <header className="grid gap-6 border-b border-[var(--orchard-line)] pb-7 lg:grid-cols-[1fr_auto] lg:items-end">
        <div className="max-w-4xl"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--orchard-green)]">{text.eyebrow}</p><h1 className="mt-3 text-3xl font-normal sm:text-4xl">{text.title}</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground sm:text-base">{text.description}</p></div>
        <div className="min-w-[260px]"><p className="mb-2 text-xs font-medium text-muted-foreground">{text.gamePlan}</p><Select value={selectedPlanId} onValueChange={setSelectedPlanId} disabled={loading||snapshot.plans.length===0}><SelectTrigger aria-label={text.gamePlan}><SelectValue placeholder={text.gamePlan}/></SelectTrigger><SelectContent>{snapshot.plans.map(plan=><SelectItem key={plan.id} value={plan.id}>{plan.season??plan.name} · {plan.status}</SelectItem>)}</SelectContent></Select></div>
      </header>
      ...
      {loading?<div className="py-16 text-sm text-muted-foreground">{text.loading}</div>:error?<div className="my-8 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}</div>:<>
        <section className="my-8 grid gap-4 lg:grid-cols-[280px_1fr]">
          <div className="rounded-xl border bg-white p-5"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium">{selectedPlan?.season??selectedPlan?.name??text.gamePlan}</p><Badge variant="secondary">{completedCount}/8</Badge></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-[#edf1ed]"><div className="h-full bg-[var(--orchard-green)] transition-all" style={{width:`${(completedCount/8)*100}%`}}/></div><p className="mt-3 text-xs text-muted-foreground">{completedCount} / 8 {text.complete}. {text.truth}</p></div>
          <div className="flex gap-3 rounded-xl border border-[#cfe0d5] bg-[#f3f8f5] p-5 text-[#234c39]"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0"/><div><p className="font-medium">{text.syncTitle}</p><p className="mt-1 text-sm leading-6 text-[#456351]">{text.syncBody}</p></div></div>
        </section>

        <section className="grid gap-px overflow-hidden rounded-xl border bg-[var(--orchard-line)] md:grid-cols-2 xl:grid-cols-4">
          {HEIRLOOM_ONBOARDING_STEPS.map((step,index)=>{const done=completion[index];return <Link key={step.id} href={scopedHref(step.coreHref)} className="group min-h-52 bg-white p-5 transition-colors hover:bg-[#f7faf7]"><div className="flex items-center justify-between gap-3"><span className="text-xs font-medium tabular-nums text-muted-foreground">0{step.order}</span>{done?<CheckCircle2 className="h-5 w-5 text-[var(--orchard-green)]"/>:<Circle className="h-5 w-5 text-[#b9c1ba]"/>}</div><h2 className="mt-8 text-lg font-normal">{step.label}</h2><p className="mt-2 text-sm leading-5 text-muted-foreground">{step.description}</p><div className="mt-5 flex items-center justify-between gap-2"><Badge variant={done?"secondary":"outline"}>{done?text.completed:text.pending}</Badge><span className="flex items-center gap-1 text-xs font-medium text-[var(--orchard-green)]">{text.openStep}<ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1"/></span></div></Link>})}
        </section>

        <section className="mt-8 grid gap-4 xl:grid-cols-2">
          <article className="rounded-xl border bg-white p-5"><div className="flex items-start gap-3"><MapPinned className="mt-0.5 h-5 w-5 text-[var(--orchard-green)]"/><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-muted-foreground">{text.reference}</p><h2 className="mt-2 text-xl font-normal">{HEIRLOOM_REFERENCE_SETUP.fieldBlockName}</h2><p className="mt-1 text-sm text-muted-foreground">{text.referenceHelp}</p></div></div><div className="mt-6 grid grid-cols-2 gap-px overflow-hidden rounded-lg bg-[var(--orchard-line)] sm:grid-cols-4"><Metric label={text.physicalBeds} value={`${HEIRLOOM_REFERENCE_SETUP.fieldBlockBeds}`}/><Metric label={text.plantings} value={`${HEIRLOOM_REFERENCE_PLANTINGS.length}`}/><Metric label={text.capacity} value={`${HEIRLOOM_REFERENCE_PHYSICAL_CAPACITY_BED_METERS} m`}/><Metric label={text.peak} value={`${HEIRLOOM_REFERENCE_PEAK_BED_METERS} m`}/></div></article>
          <article className="rounded-xl border bg-white p-5"><div className="flex items-start gap-3"><Sprout className="mt-0.5 h-5 w-5 text-[var(--orchard-green)]"/><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-muted-foreground">{text.coreLive}</p><h2 className="mt-2 text-xl font-normal">{selectedPlan?.name??text.gamePlan}</h2><p className="mt-1 text-sm text-muted-foreground">{text.coreHelp}</p></div></div><div className="mt-6 grid grid-cols-2 gap-px overflow-hidden rounded-lg bg-[var(--orchard-line)] sm:grid-cols-4"><Metric label={text.physicalBeds} value={`${coreBeds.length}`}/><Metric label={text.plantings} value={`${scopedSuccessions.length}`}/><Metric label={text.capacity} value={`${liveBedMeters.toFixed(0)} m`}/><Metric label={text.assigned} value={`${scopedSuccessions.length-unallocatedCount}/${scopedSuccessions.length}`}/></div></article>
        </section>
...
function Metric({label,value}:{label:string;value:string}){return <div className="bg-[#fafbf9] p-4"><p className="text-[11px] uppercase tracking-[.12em] text-muted-foreground">{label}</p><p className="mt-2 text-2xl tabular-nums">{value}</p></div>}
