...
type ExistingTask = { id: string; due_date: string | null; source_type: string | null; source_id: string | null; source_path: string | null; status: string }
...
export default function DietrichTaskCalendarPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      supabase.from("tasks").select("id,due_date,source_type,source_id,source_path,status").in("operational_area", ["orchard", "huerto_vinedo"]),
    ]).then(([p, c, s, a, t]) => {
      if (!live) return
