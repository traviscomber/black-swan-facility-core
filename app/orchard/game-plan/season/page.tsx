...
const copy = {
  en: {
    eyebrow:"Dietrich · Season Plan", title:"Planting calendar", description:"Read the reconciled field season as one continuous calendar. Every succession has its own row from sowing through field establishment and harvest.", crop:"Crop / succession", direct:"Direct sow", transplant:"Transplant", plantings:"plantings", bed:"bed", plants:"plants", sowToField:"Sow → field", fieldToHarvest:"Field → first harvest", harvestWindow:"Harvest window", advanced:"Edit Game Plan", source:"Canonical dates from the reconciled field plan. Crop color follows canonical botanical family; phase is expressed by intensity. Physical placement remains managed in Crop Map.", noRows:"No plantings match the current search.", today:"Today", collapseAll:"Collapse all", colorIdentity:"Color = botanical family / crop", search:"Search crops", searchPlaceholder:"Search crops…", schedule:"Planting Schedule", generic:"Generic"
  },
  es: {
    eyebrow:"Dietrich · Plan de temporada", title:"Calendario de plantación", description:"Lee la temporada reconciliada como un calendario continuo. Cada sucesión tiene su propia fila desde siembra, entrada a campo y cosecha.", crop:"Cultivo / sucesión", direct:"Siembra directa", transplant:"Trasplante", plantings:"plantaciones", bed:"de cama", plants:"plantas", sowToField:"Siembra → campo", fieldToHarvest:"Campo → primera cosecha", harvestWindow:"Ventana de cosecha", advanced:"Editar plan de cultivo", source:"Fechas canónicas del plan de campo reconciliado. El color del cultivo sigue su familia botánica canónica y la fase se expresa por intensidad. La ubicación física se sigue gestionando en Crop Map.", noRows:"No hay plantaciones para la búsqueda actual.", today:"Hoy", collapseAll:"Contraer todo", colorIdentity:"Color = familia botánica / cultivo", search:"Buscar cultivos", searchPlaceholder:"Buscar cultivos…", schedule:"Calendario de plantación", generic:"Genérico"
  },
  de: {
    eyebrow:"Dietrich · Saisonplan", title:"Pflanzkalender", description:"Die abgeglichene Feldsaison als durchgehenden Kalender lesen. Jede Folge hat eine eigene Zeile von Aussaat über Feldstart bis Ernte.", crop:"Kultur / Folge", direct:"Direktsaat", transplant:"Verpflanzung", plantings:"Pflanzungen", bed:"Beet", plants:"Pflanzen", sowToField:"Aussaat → Feld", fieldToHarvest:"Feld → erste Ernte", harvestWindow:"Erntefenster", advanced:"Anbauplan bearbeiten", source:"Kanonische Termine aus dem abgeglichenen Feldplan. Kulturfarbe folgt der kanonischen botanischen Familie; die Phase wird über Intensität dargestellt. Die physische Platzierung bleibt in Crop Map verwaltet.", noRows:"Keine Pflanzungen entsprechen der aktuellen Suche.", today:"Heute", collapseAll:"Alle einklappen", colorIdentity:"Farbe = botanische Familie / Kultur", search:"Kulturen suchen", searchPlaceholder:"Kulturen suchen…", schedule:"Pflanzkalender", generic:"Generisch"
  },
} as const
...
const varietyLabel = (value:string|null, generic:string) => !value || normalize(value)==="generic" ? generic : value
...
export default function DietrichSeasonPlanPage() {
  ...
  const visibleRows = rows.filter(({c})=>`${c.crop_name} ${varietyLabel(c.variety,text.generic)}`.toLowerCase().includes(query.trim().toLowerCase()))
