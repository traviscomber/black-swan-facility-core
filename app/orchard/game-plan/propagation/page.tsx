...
import { ArrowRight, CheckCircle2, CircleAlert, Database, Sprout } from "lucide-react"
...
const copy = {
  en: {
    eyebrow: "Dietrich · Sowing & Nursery", title: "Propagation on the reconciled field plan", description: "One operational view for direct sowing, nursery propagation and germination evidence. Source settings stay separate from observed field data.",
  ...
  es: {
    eyebrow: "Dietrich · Siembra y Almácigo", title: "Propagación sobre el plan de campo reconciliado", description: "Una sola vista operacional para siembra directa, propagación en almácigo y evidencia de germinación. Los parámetros fuente se mantienen separados de los datos observados.",
  ...
  de: {
    eyebrow: "Dietrich · Aussaat & Anzucht", title: "Vermehrung auf dem abgeglichenen Feldplan", description: "Eine operative Ansicht für Direktsaat, Anzucht und Keimungsnachweise. Quellparameter bleiben von beobachteten Daten getrennt.",
...
export default function DietrichPropagationPage() {
  ...
  ]).then(([p, c, s, a, lots, batches]) => { if (!live) return; const first = p.error ?? c.error ?? s.error ?? a.error ?? lots.error ?? batches.error; if (first) { setError(first.message); setLoading(false); return } setPlans((p.data ?? []) as Plan[]); setCycles((c.data ?? []) as Cycle[]); setSuccessions((s.data ?? []) as Succession[]); setAllocations((a.data ?? []) as Allocation[]); setSeedLots(lots.count ?? 0); setNurseryBatches(batches.count ?? 0); setLoading(false) }); return () => { live = false } }, [supabase])
  ...
  return <AppLayout><OrchardNavigation/><main className="mx-auto w-full max-w-[1560px] px-4 py-8 sm:px-6 lg:px-8">
    ...
    {loading ? <div className="py-12 text-sm text-muted-foreground">…</div> : error ? <div className="border border-red-300 bg-red-50 p-4 text-sm text-red-800">{error}</div> : <div className="space-y-9">
