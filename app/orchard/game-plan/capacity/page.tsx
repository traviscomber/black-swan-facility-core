...
import { ArrowRight, CheckCircle2, Gauge, Map, Rows3 } from "lucide-react"
...
type Bed = { id: string; plot_id: string; name: string; length_m: number | null; width_m: number | null; area_sqm: number | null; status: string | null }
...
const copy = {
  en: {
    eyebrow: "Dietrich · Beds & Capacity", title: "Verified field capacity", description: "Capacity is now read from the canonical Black Swan physical model. Seasonal allocation meters and simultaneous occupancy are kept separate so temporal bed reuse remains visible.",
    blocks: "Physical blocks", beds: "Canonical beds", capacity: "Physical capacity", peak: "Peak simultaneous", headroom: "Peak headroom", plantings: "Reconciled plantings", seasonal: "Season allocation", usedBeds: "Beds touched",
    verified: "The current production model is physically reconciled: 5 Current blocks + 3 Expansion blocks. Each canonical bed contributes its recorded length; no spreadsheet container is treated as a physical bed.",
    block: "Block", bedCount: "Beds", blockCapacity: "Capacity", touched: "Touched by plan", peakDate: "Peak date", occupancy: "Peak occupancy", advanced: "Open Crop Map workspace", source: "Physical ownership: Supabase orchard_plots + orchard_beds + orchard_bed_allocations. Workbook capacity sheets remain reference evidence only.", loadError: "Could not load canonical capacity.",
  },
  es: {
    eyebrow: "Dietrich · Camas y Capacidad", title: "Capacidad física verificada", description: "La capacidad ahora se lee desde el modelo físico canónico de Black Swan. Los metros asignados durante la temporada y la ocupación simultánea se mantienen separados para mostrar correctamente la reutilización temporal de camas.",
    blocks: "Bloques físicos", beds: "Camas canónicas", capacity: "Capacidad física", peak: "Peak simultáneo", headroom: "Holgura en peak", plantings: "Plantaciones reconciliadas", seasonal: "Asignación temporada", usedBeds: "Camas utilizadas",
    verified: "El modelo productivo actual está físicamente reconciliado: 5 bloques Current + 3 bloques Expansion. Cada cama aporta su longitud registrada; ningún contenedor histórico del Excel se trata como cama física.",
    block: "Bloque", bedCount: "Camas", blockCapacity: "Capacidad", touched: "Usadas por el plan", peakDate: "Fecha peak", occupancy: "Ocupación peak", advanced: "Abrir workspace de Crop Map", source: "Ownership físico: Supabase orchard_plots + orchard_beds + orchard_bed_allocations. Las hojas de capacidad del workbook quedan sólo como evidencia de referencia.", loadError: "No fue posible cargar la capacidad canónica.",
  },
  de: {
    eyebrow: "Dietrich · Beete & Kapazität", title: "Verifizierte Feldkapazität", description: "Die Kapazität wird jetzt aus dem kanonischen physischen Black-Swan-Modell gelesen. Saisonale Zuweisungsmeter und gleichzeitige Belegung bleiben getrennt, damit zeitliche Beet-Wiederverwendung sichtbar bleibt.",
    blocks: "Physische Blöcke", beds: "Kanonische Beete", capacity: "Physische Kapazität", peak: "Gleichzeitiger Peak", headroom: "Reserve am Peak", plantings: "Abgeglichene Pflanzungen", seasonal: "Saisonzuweisung", usedBeds: "Genutzte Beete",
    verified: "Das aktuelle Produktionsmodell ist physisch abgeglichen: 5 Current-Blöcke + 3 Expansion-Blöcke. Jedes kanonische Beet trägt seine erfasste Länge bei; kein historischer Spreadsheet-Container gilt als physisches Beet.",
    block: "Block", bedCount: "Beete", blockCapacity: "Kapazität", touched: "Vom Plan genutzt", peakDate: "Peak-Datum", occupancy: "Peak-Belegung", advanced: "Crop-Map-Arbeitsbereich öffnen", source: "Physischer Owner: Supabase orchard_plots + orchard_beds + orchard_bed_allocations. Workbook-Kapazitätsblätter bleiben Referenznachweise.", loadError: "Kanonische Kapazität konnte nicht geladen werden.",
  },
} as const
...
export default function DietrichCapacityPage() {
  ...
  useEffect(() => { let live = true; setLoading(true); setError(null); void Promise.all([
    ...
    supabase.from("orchard_beds").select("id,plot_id,name,length_m,width_m,area_sqm,status").order("planning_order"),
  ...
  const cycleIds = new Set(cycles.filter((cycle) => cycle.game_plan_id === plan?.id).map((cycle) => cycle.id))
  const successionIds = new Set(successions.filter((succession) => cycleIds.has(succession.crop_cycle_id)).map((succession) => succession.id))
  ...
  const capacityM = physicalBeds.reduce((sum, bed) => sum + Number(bed.length_m ?? 0), 0)
  const seasonMeters = scopedAllocations.reduce((sum, allocation) => sum + Number(allocation.allocated_length_m ?? 0), 0)
  const peak = peakUsage(scopedAllocations)
  const headroom = Math.max(0, capacityM - peak.meters)
  const occupancyPct = capacityM > 0 ? (peak.meters / capacityM) * 100 : 0
  ...
  return <AppLayout><OrchardNavigation/><main className="mx-auto w-full max-w-[1380px] px-4 py-8 sm:px-6 lg:px-8">
    ...
    {loading ? <div className="py-12 text-sm text-muted-foreground">…</div> : error ? <div className="border border-red-300 bg-red-50 p-4 text-sm text-red-800">{error}</div> : <>
      ...
      <section className="overflow-hidden border border-[var(--bs-divider-subtle)] bg-[var(--bs-surface-primary)]"><div className="grid grid-cols-[1fr_120px_150px_150px] border-b border-[var(--bs-divider-subtle)] bg-[var(--bs-surface-secondary)] px-4 py-3 text-xs uppercase tracking-wide text-muted-foreground"><span>{text.block}</span><span>{text.bedCount}</span><span>{text.blockCapacity}</span><span>{text.touched}</span></div>{physicalPlots.map((plot) => { const blockBeds = physicalBeds.filter((bed) => bed.plot_id === plot.id); const blockCapacity = blockBeds.reduce((sum, bed) => sum + Number(bed.length_m ?? 0), 0); const touched = blockBeds.filter((bed) => usedBedIds.has(bed.id)).length; return <div key={plot.id} className="grid grid-cols-[1fr_120px_150px_150px] border-b border-[var(--bs-divider-subtle)] px-4 py-3 text-sm last:border-b-0"><strong className="font-medium">{plot.name}</strong><span className="tabular-nums">{blockBeds.length}</span><span className="tabular-nums">{blockCapacity.toLocaleString(locale)} m</span><span className="tabular-nums">{touched}/{blockBeds.length}</span></div> })}</section>
...
function Metric({ icon: Icon, label, value, unit }: { icon?: typeof Map; label: string; value: number; unit?: string }) { return <div className="bg-[var(--bs-surface-primary)] p-5">{Icon ? <Icon className="h-4 w-4 text-muted-foreground"/> : null}<p className={`${Icon ? "mt-4" : ""} text-xs uppercase tracking-wide text-muted-foreground`}>{label}</p><p className="mt-2 text-3xl tabular-nums">{value.toLocaleString("es-CL", { maximumFractionDigits: 1 })}{unit ? <span className="ml-1 text-sm text-muted-foreground">{unit}</span> : null}</p></div> }
