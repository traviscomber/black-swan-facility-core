...
import { CapacityCurveChart } from "@/components/orchard/orchard-season-graphics"
...
const copy = {
  en:{eyebrow:"Dietrich · Beds & Capacity",title:"Verified field capacity",description:"Capacity comes from the canonical Black Swan physical model. Seasonal allocated meters and simultaneous occupancy stay separate so temporal bed reuse is visible.",blocks:"Physical blocks",beds:"Canonical beds",capacity:"Physical capacity",peak:"Peak simultaneous",headroom:"Peak headroom",plantings:"Reconciled plantings",seasonal:"Season allocation",usedBeds:"Beds touched",verified:"The production model is physically reconciled: 5 Current blocks + 3 Expansion blocks. Each bed contributes its recorded length; spreadsheet containers are not treated as physical beds.",today:"Today",next:"Next field occupancy",noNext:"No later allocation",peakContext:"Season peak",block:"Block",bedCount:"Beds",blockCapacity:"Capacity",touched:"Touched by plan",peakDate:"Peak date",advanced:"Open Crop Map workspace",source:"Physical ownership: Supabase orchard_plots + orchard_beds + orchard_bed_allocations. Workbook capacity sheets remain reference evidence only.",loadError:"Could not load canonical capacity.",curve:"Season capacity curve",curveHelp:"Weekly simultaneous occupancy from physical bed allocations against the fixed canonical capacity. This is temporal occupancy, not the sum of all meters reused across the season."},
  es:{eyebrow:"Dietrich · Camas y Capacidad",title:"Capacidad física verificada",description:"La capacidad viene del modelo físico canónico de Black Swan. Los metros asignados durante la temporada y la ocupación simultánea se mantienen separados para mostrar la reutilización temporal de camas.",blocks:"Bloques físicos",beds:"Camas canónicas",capacity:"Capacidad física",peak:"Peak simultáneo",headroom:"Holgura en peak",plantings:"Plantaciones reconciliadas",seasonal:"Asignación temporada",usedBeds:"Camas utilizadas",verified:"El modelo productivo está físicamente reconciliado: 5 bloques Current + 3 bloques Expansion. Cada cama aporta su longitud registrada; los contenedores históricos del Excel no se tratan como camas físicas.",today:"Hoy",next:"Próxima ocupación de campo",noNext:"Sin asignaciones posteriores",peakContext:"Peak de temporada",block:"Bloque",bedCount:"Camas",blockCapacity:"Capacidad",touched:"Usadas por el plan",peakDate:"Fecha peak",advanced:"Abrir workspace de Crop Map",source:"Ownership físico: Supabase orchard_plots + orchard_beds + orchard_bed_allocations. Las hojas de capacidad del workbook quedan sólo como evidencia de referencia.",loadError:"No fue posible cargar la capacidad canónica.",curve:"Curva de capacidad de temporada",curveHelp:"Ocupación simultánea semanal desde asignaciones físicas de cama contra la capacidad canónica fija. Es ocupación temporal, no la suma de metros reutilizados durante toda la temporada."},
  de:{eyebrow:"Dietrich · Beete & Kapazität",title:"Verifizierte Feldkapazität",description:"Die Kapazität stammt aus dem kanonischen physischen Black-Swan-Modell. Saisonale Zuweisungsmeter und gleichzeitige Belegung bleiben getrennt.",blocks:"Physische Blöcke",beds:"Kanonische Beete",capacity:"Physische Kapazität",peak:"Gleichzeitiger Peak",headroom:"Reserve am Peak",plantings:"Abgeglichene Pflanzungen",seasonal:"Saisonzuweisung",usedBeds:"Genutzte Beete",verified:"Das Produktionsmodell ist physisch abgeglichen: 5 Current-Blöcke + 3 Expansion-Blöcke. Jedes Beet trägt seine erfasste Länge bei; Spreadsheet-Container gelten nicht als physische Beete.",today:"Heute",next:"Nächste Feldbelegung",noNext:"Keine spätere Zuweisung",peakContext:"Saison-Peak",block:"Block",bedCount:"Beete",blockCapacity:"Kapazität",touched:"Vom Plan genutzt",peakDate:"Peak-Datum",advanced:"Crop-Map-Arbeitsbereich öffnen",source:"Physischer Owner: Supabase orchard_plots + orchard_beds + orchard_bed_allocations. Workbook-Kapazitätsblätter bleiben Referenznachweise.",loadError:"Kanonische Kapazität konnte nicht geladen werden.",curve:"Saisonale Kapazitätskurve",curveHelp:"Wöchentliche gleichzeitige Belegung aus physischen Beetzuordnungen im Vergleich zur festen kanonischen Kapazität. Dies ist zeitliche Belegung, nicht die Summe der über die Saison wiederverwendeten Meter."},
} as const
...
function todaySantiago(){return new Intl.DateTimeFormat("en-CA",{timeZone:"America/Santiago",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date())}
...
export default function DietrichCapacityPage(){
  ...
  const scopedCycles=cycles.filter(cycle=>cycle.game_plan_id===plan?.id); const cycleIds=new Set(scopedCycles.map(cycle=>cycle.id))
  const scopedSuccessions=successions.filter(s=>cycleIds.has(s.crop_cycle_id)); const successionIds=new Set(scopedSuccessions.map(s=>s.id))
  ...
  const todayKey=todaySantiago()
  const currentMeters=scopedAllocations.reduce((sum,a)=>sum+(a.planned_start_date&&a.planned_end_date&&a.planned_start_date<=todayKey&&todayKey<a.planned_end_date?Number(a.allocated_length_m??0):0),0)
  const currentPct=capacityM>0?(currentMeters/capacityM)*100:0
  const successionById=new Map(scopedSuccessions.map(s=>[s.id,s])); const cycleById=new Map(scopedCycles.map(c=>[c.id,c]))
  const nextStartDate=scopedAllocations.map(a=>a.planned_start_date).filter((value):value is string=>Boolean(value&&value>todayKey)).sort()[0]??null
  const nextStartAllocations=nextStartDate?scopedAllocations.filter(a=>a.planned_start_date===nextStartDate):[]
  const nextMeters=nextStartAllocations.reduce((sum,a)=>sum+Number(a.allocated_length_m??0),0)
  const nextCrops=[...new Set(nextStartAllocations.map(a=>{const s=successionById.get(a.crop_succession_id);return s?cycleById.get(s.crop_cycle_id)?.crop_name??null:null}).filter((value):value is string=>Boolean(value)))]
  const formatDate=(value:string|null,month:"short"|"long"="short")=>value?new Date(`${value}T12:00:00`).toLocaleDateString(locale,{day:"2-digit",month,year:"numeric"}):"—"
  const curveData=useMemo(()=>{
    if(!plan)return []
    const rows:{label:string;occupancy:number;capacity:number;date:string}[]=[]
    const cursor=new Date(`${plan.start_date}T12:00:00`);const end=new Date(`${plan.end_date}T12:00:00`);let week=1
    while(cursor<=end&&week<=60){const date=dateKey(cursor);const occupancy=scopedAllocations.reduce((sum,a)=>sum+(a.planned_start_date&&a.planned_end_date&&a.planned_start_date<=date&&date<a.planned_end_date?Number(a.allocated_length_m??0):0),0);rows.push({label:new Date(`${date}T12:00:00`).toLocaleDateString(locale,{day:"2-digit",month:"short"}),occupancy,capacity:capacityM,date});cursor.setDate(cursor.getDate()+7);week+=1}
    return rows
  },[plan,scopedAllocations,capacityM,locale])
  const peakCurveLabel=peak.date?curveData.reduce<{label:string;distance:number}|null>((best,row)=>{const distance=Math.abs(new Date(`${row.date}T12:00:00`).getTime()-new Date(`${peak.date}T12:00:00`).getTime());return !best||distance<best.distance?{label:row.label,distance}:best},null)?.label??null:null
  ...
  return <AppLayout><OrchardNavigation/><main className="mx-auto w-full max-w-[1380px] px-4 py-8 sm:px-6 lg:px-8">
    ...
    {loading?<div className="py-12 text-sm text-muted-foreground">…</div>:error?<div className="border border-red-300 bg-red-50 p-4 text-sm text-red-800">{error}</div>:<>
      ...
      <section className="mb-6 grid gap-px bg-[var(--bs-divider-subtle)] lg:grid-cols-3">
        <ContextCell label={text.today} value={`${currentMeters.toLocaleString(locale,{maximumFractionDigits:1})} m`} detail={`${formatDate(todayKey)} · ${currentPct.toFixed(1)}%`}/>
        <ContextCell label={text.next} value={nextStartDate?`${formatDate(nextStartDate)} · ${nextMeters.toLocaleString(locale,{maximumFractionDigits:1})} m`:text.noNext} detail={nextCrops.join(", ")||"—"}/>
        <ContextCell label={text.peakContext} value={`${peak.meters.toLocaleString(locale,{maximumFractionDigits:1})} m · ${occupancyPct.toFixed(1)}%`} detail={formatDate(peak.date,"long")}/>
      </section>
      <section className="mb-6 border border-[var(--bs-divider-subtle)] bg-[var(--bs-surface-primary)] p-5 sm:p-6"><div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-xl font-normal">{text.curve}</h2><p className="mt-1 max-w-4xl text-xs leading-5 text-muted-foreground">{text.curveHelp}</p></div><div className="text-right"><p className="text-xs uppercase tracking-wide text-muted-foreground">{text.peakDate}</p><p className="mt-1 text-sm">{formatDate(peak.date)}</p></div></div><CapacityCurveChart data={curveData} language={lang} peakLabel={peakCurveLabel}/></section>
...
function ContextCell({label,value,detail}:{label:string;value:string;detail:string}){return <div className="bg-[var(--bs-surface-primary)] p-5"><p className="text-[10px] uppercase tracking-[.14em] text-muted-foreground">{label}</p><p className="mt-2 text-lg font-medium">{value}</p><p className="mt-1 text-xs text-muted-foreground">{detail}</p></div>}
