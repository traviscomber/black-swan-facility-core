...
import { Activity, Camera, Droplets, Leaf, Plus, RefreshCw, Sprout, Trash2 } from "lucide-react"
...
const CARE_PHOTOS = [
  ...
  image("photo-1500937386664-56d1dfef3854"),
  image("photo-1471193945509-9ad0617afabf"),
]
const cropPhoto = (name: string) => {
  ...
  return CARE_PHOTOS[Math.abs(name.length) % CARE_PHOTOS.length]
}
...
export default function OrchardCarePage() {
  ...
  const [logs, setLogs] = useState<Care[]>([])
  ...
  const load = useCallback(async () => {
    ...
    else { setLogs((l.data ?? []) as Care[]); setCrops((c.data ?? []) as Crop[]) }
    setLoading(false)
  }, [supabase])
  ...
  const todayCount = logs.filter((log) => log.activity_date === today).length
  const activeCrops = crops.filter((crop) => !["harvested", "failed"].includes(crop.status)).length
  const lastByCrop = useMemo(() => {
    const map = new Map<string, Care>()
    logs.forEach((log) => { if (!map.has(log.crop_id)) map.set(log.crop_id, log) })
    return map
  }, [logs])
  ...
  return <AppLayout><OrchardNavigation /><main className="mx-auto w-full max-w-[1560px] space-y-8 px-4 pb-20 pt-4 sm:px-6 lg:px-8">
    <section className="relative min-h-[360px] overflow-hidden bg-neutral-950 text-white sm:min-h-[420px]">
      ...
      <div className="relative flex min-h-[360px] max-w-3xl flex-col justify-end p-6 sm:min-h-[420px] sm:p-10">
        <p className="text-xs uppercase tracking-[.2em] text-emerald-200">Orchard · Operations</p><h1 className="mt-3 text-4xl font-medium tracking-[-.03em] sm:text-5xl">{text.title}</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-white/75">{text.description}</p>
        <div className="mt-6 flex flex-wrap gap-2"><Button onClick={() => void load()} disabled={loading} className="bg-white text-black hover:bg-white/90"><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />{text.refresh}</Button><Badge className="border-white/20 bg-black/30 px-3 py-2 text-white">{logs.length} {text.total.toLowerCase()}</Badge></div>
      </div>
      <div className="absolute bottom-6 right-6 hidden grid-cols-2 gap-px bg-white/10 lg:grid"><HeroMetric label={text.total} value={logs.length} /><HeroMetric label={text.today} value={todayCount} /><HeroMetric label={text.active} value={activeCrops} /><HeroMetric label={text.withCare} value={lastByCrop.size} /></div>
    </section>
