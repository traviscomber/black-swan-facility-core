...
const copy = {
  en: { title: "Crop Care", description: "Run the field-care journal from recorded interventions, crop context and field evidence.", new: "Record care activity", crop: "Crop", date: "Date", type: "Activity", hours: "Hours", descriptionLabel: "Description", weather: "Weather", temp: "Temperature °C", humidity: "Humidity %", observations: "Observations", create: "Record activity", refresh: "Refresh", empty: "No care activities recorded yet.", delete: "Delete this care record?", saveError: "Could not save care record", advanced: "Optional field details", journal: "Intervention journal", journalHelp: "Every real action stays attached to the crop dossier with time, field context and evidence.", total: "Interventions", today: "Today", active: "Active crops", withCare: "Crops with care", evidence: "Evidence", latest: "Latest intervention", planted: "Planted", harvest: "Expected harvest", openHealth: "Health observations", viewCrop: "Crop dossier", scope: "Game Plan scope", all: "All Orchard", scopedHelp: "Care records and crop choices are limited to crops linked to this Game Plan.", aiCreated: "Created by Orchard AI · focused record", heroEyebrow: "Orchard · Operations", cropFallback: "Crop" },
  es: { title: "Cuidados", description: "Opera el diario de terreno desde intervenciones registradas, contexto del cultivo y evidencia de campo.", new: "Registrar cuidado", crop: "Cultivo", date: "Fecha", type: "Actividad", hours: "Horas", descriptionLabel: "Descripción", weather: "Clima", temp: "Temperatura °C", humidity: "Humedad %", observations: "Observaciones", create: "Registrar cuidado", refresh: "Actualizar", empty: "Aún no hay actividades de cuidado.", delete: "¿Eliminar este registro de cuidado?", saveError: "No fue posible guardar el cuidado", advanced: "Detalles opcionales de terreno", journal: "Diario de intervenciones", journalHelp: "Cada acción real queda unida al dossier del cultivo con tiempo, contexto de terreno y evidencia.", total: "Intervenciones", today: "Hoy", active: "Cultivos activos", withCare: "Cultivos con cuidados", evidence: "Evidencia", latest: "Última intervención", planted: "Plantado", harvest: "Cosecha esperada", openHealth: "Observaciones sanitarias", viewCrop: "Dossier del cultivo", scope: "Alcance del Game Plan", all: "Todo Orchard", scopedHelp: "Los cuidados y las opciones de cultivo se limitan a cultivos vinculados a este Game Plan.", aiCreated: "Creado por Orchard AI · registro enfocado", heroEyebrow: "Orchard · Operaciones", cropFallback: "Cultivo" },
  de: { title: "Pflanzenpflege", description: "Führe das Pflegejournal aus erfassten Maßnahmen, Pflanzenkontext und Feldnachweisen.", new: "Pflegemaßnahme erfassen", crop: "Kultur", date: "Datum", type: "Maßnahme", hours: "Stunden", descriptionLabel: "Beschreibung", weather: "Wetter", temp: "Temperatur °C", humidity: "Luftfeuchte %", observations: "Beobachtungen", create: "Maßnahme erfassen", refresh: "Aktualisieren", empty: "Noch keine Pflegemaßnahmen erfasst.", delete: "Diesen Pflegeeintrag löschen?", saveError: "Pflegeeintrag konnte nicht gespeichert werden", advanced: "Optionale Felddetails", journal: "Maßnahmenjournal", journalHelp: "Jede reale Maßnahme bleibt mit Zeit, Feldkontext und Nachweisen an der Kulturakte verknüpft.", total: "Maßnahmen", today: "Heute", active: "Aktive Kulturen", withCare: "Kulturen mit Pflege", evidence: "Nachweise", latest: "Letzte Maßnahme", planted: "Gepflanzt", harvest: "Erwartete Ernte", openHealth: "Pflanzengesundheit", viewCrop: "Kulturakte", scope: "Game-Plan-Umfang", all: "Gesamter Orchard", scopedHelp: "Pflegeeinträge und auswählbare Kulturen sind auf Kulturen dieses Game Plans begrenzt.", aiCreated: "Von Orchard AI erstellt · fokussierter Eintrag", heroEyebrow: "Orchard · Betrieb", cropFallback: "Kultur" },
} as const

const activityLabels = {
  en: { watering: "Watering", feeding: "Feeding", weeding: "Weeding", pruning: "Pruning", cultivation: "Cultivation", inspection: "Inspection", other: "Other" },
  es: { watering: "Riego", feeding: "Fertilización", weeding: "Desmalezado", pruning: "Poda", cultivation: "Labranza", inspection: "Inspección", other: "Otro" },
  de: { watering: "Bewässerung", feeding: "Düngung", weeding: "Unkrautpflege", pruning: "Schnitt", cultivation: "Bodenbearbeitung", inspection: "Kontrolle", other: "Sonstiges" },
} as const
const statusLabels: Record<"en" | "es" | "de", Record<string, string>> = {
  en: { active: "Active", growing: "Growing", seedling: "Seedling", mature: "Mature", harvested: "Harvested", failed: "Failed", planned: "Planned" },
  es: { active: "Activo", growing: "En crecimiento", seedling: "Almácigo", mature: "Maduro", harvested: "Cosechado", failed: "Fallido", planned: "Planificado" },
  de: { active: "Aktiv", growing: "Im Wachstum", seedling: "Jungpflanze", mature: "Reif", harvested: "Geerntet", failed: "Ausgefallen", planned: "Geplant" },
}
...
export default function OrchardCarePage() {
  ...
  const activityLabel = (value: string) => activityLabels[lang][value as keyof typeof activityLabels.en] ?? value.replaceAll("_", " ")
  const statusLabel = (value: string | null | undefined) => value ? statusLabels[lang][value] ?? value.replaceAll("_", " ") : "—"
  ...
  return <AppLayout><OrchardNavigation /><main className="mx-auto w-full max-w-[1560px] space-y-8 px-4 pb-20 pt-4 sm:px-6 lg:px-8">
    <section className="border bg-[var(--bs-surface-secondary)] p-6 sm:p-10">
      <div className="max-w-4xl"><p className="text-xs uppercase tracking-[.2em] text-muted-foreground">{text.heroEyebrow}</p><h1 className="mt-3 text-4xl font-medium tracking-[-.03em] sm:text-5xl">{text.title}</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">{text.description}</p><div className="mt-6 flex flex-wrap gap-2"><Button onClick={() => void load()} disabled={loading} variant="outline"><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />{text.refresh}</Button><Badge variant="outline" className="px-3 py-2">{scopedLogs.length} {text.total.toLowerCase()}</Badge><Badge variant="outline" className="px-3 py-2">{text.scope}: {scopeLabel}</Badge></div><div className="mt-7 grid grid-cols-2 gap-2 lg:grid-cols-4"><HeroMetric label={text.total} value={scopedLogs.length} /><HeroMetric label={text.today} value={todayCount} /><HeroMetric label={text.active} value={activeCrops} /><HeroMetric label={text.withCare} value={lastByCrop.size} /></div></div>
    </section>
    ...
    <section className="grid gap-6 xl:grid-cols-[390px_1fr]">
      ...
      <div className="space-y-6"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">01</p><h2 className="mt-2">{text.journal}</h2><p className="mt-1 text-sm text-muted-foreground">{text.journalHelp}</p></div>
        {scopedLogs.length === 0 ? <div className="border border-dashed p-8 text-sm text-muted-foreground">{text.empty}</div> : <div className="grid gap-4 lg:grid-cols-2">{scopedLogs.map((log) => { const crop = cropById.get(log.crop_id); const focused = log.id === focusedEntity; return <article id={`care-${log.id}`} key={log.id} className={`overflow-hidden border bg-background ${focused ? "border-primary ring-2 ring-primary/40" : ""}`}><div className="bg-[var(--bs-surface-secondary)] p-4"><div className="flex flex-wrap gap-2"><Badge variant="outline">{activityLabel(log.activity_type)}</Badge><Badge variant="outline">{statusLabel(crop?.status)}</Badge>{focused && <Badge>Orchard AI</Badge>}</div><h3 className="mt-3 text-xl">{crop?.crop_name ?? text.cropFallback}{crop?.variety ? ` · ${crop.variety}` : ""}</h3><p className="mt-1 text-xs text-muted-foreground">{new Date(`${log.activity_date}T12:00:00`).toLocaleDateString(locale)}{log.hours_spent != null ? ` · ${log.hours_spent}h` : ""}{log.weather_conditions ? ` · ${log.weather_conditions}` : ""}</p></div><div className="space-y-4 p-4"><p className="text-sm leading-6">{log.description || log.observations || "—"}</p><div className="grid grid-cols-2 gap-2 text-xs"><Datum icon={<Sprout className="h-3.5 w-3.5"/>} label={text.planted} value={crop?.planting_date ?? "—"}/><Datum icon={<Leaf className="h-3.5 w-3.5"/>} label={text.harvest} value={crop?.expected_harvest_date ?? "—"}/>{log.temperature_c != null && <Datum icon={<Activity className="h-3.5 w-3.5"/>} label={text.temp} value={`${log.temperature_c}°C`}/>} {log.humidity_percent != null && <Datum icon={<Droplets className="h-3.5 w-3.5"/>} label={text.humidity} value={`${log.humidity_percent}%`}/>}</div><div className="flex flex-wrap gap-2"><Button asChild size="sm" variant="outline"><Link href={scopedHref("/orchard/crops")}>{text.viewCrop}</Link></Button><Button asChild size="sm" variant="outline"><Link href={scopedHref("/orchard/pests")}>{text.openHealth}</Link></Button><Button size="icon" variant="ghost" className="ml-auto" aria-label={text.delete} onClick={() => void remove(log.id)}><Trash2 className="h-4 w-4" /></Button></div><div className="border-t pt-4"><div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground"><Camera className="h-3.5 w-3.5" />{text.evidence}</div><OrchardEvidence cropId={log.crop_id} careLogId={log.id} /></div></div></article> })}</div>}
        {scopedCrops.length > 0 && <Card><CardHeader><CardTitle>{text.latest}</CardTitle></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{scopedCrops.slice(0, 9).map((crop) => { const latest = lastByCrop.get(crop.id); return <div key={crop.id} className="border p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-medium">{crop.crop_name}{crop.variety ? ` · ${crop.variety}` : ""}</p><p className="mt-1 text-xs text-muted-foreground">{latest ? `${activityLabel(latest.activity_type)} · ${latest.activity_date}` : text.empty}</p></div><Badge variant="outline">{statusLabel(crop.status)}</Badge></div></div> })}</div></CardContent></Card>}
      </div>
    </section>
  </main></AppLayout>
}
...
function HeroMetric({ label, value }: { label: string; value: number }) { return <div className="border bg-background/60 px-5 py-4"><p className="text-[10px] uppercase tracking-[.14em] text-muted-foreground">{label}</p><p className="mt-1 text-2xl font-medium tabular-nums">{value}</p></div> }
