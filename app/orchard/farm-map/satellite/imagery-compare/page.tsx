...
type Provider="maptiler"|"esri"|"oam"
...
const copy={
 en:{label:"Satellite imagery comparison",maptiler:"MapTiler Satellite",esri:"Esri World Imagery",oam:"OpenAerialMap",location:"Fundo Corcovado",initError:"Could not initialize comparison map.",keyError:"MapTiler key is not available in this deployment."},
 es:{label:"Comparación de imagen satelital",maptiler:"MapTiler Satélite",esri:"Esri World Imagery",oam:"OpenAerialMap",location:"Fundo Corcovado",initError:"No fue posible iniciar el mapa de comparación.",keyError:"La clave de MapTiler no está disponible en este deployment."},
 de:{label:"Vergleich der Satellitenbilder",maptiler:"MapTiler Satellit",esri:"Esri World Imagery",oam:"OpenAerialMap",location:"Fundo Corcovado",initError:"Die Vergleichskarte konnte nicht gestartet werden.",keyError:"Der MapTiler-Schlüssel ist in diesem Deployment nicht verfügbar."}
} as const
...
export default function ImageryComparePage(){
 ...
 useEffect(()=>{if(!mapNode.current)return;let cancelled=false
  void import("leaflet").then(mod=>{if(cancelled||!mapNode.current)return;const L=mod as unknown as RuntimeLeaflet;leafletRef.current=L
   const map=L.map(mapNode.current,{zoomControl:false,attributionControl:true,minZoom:4,maxZoom:21}).setView(FARM_CENTER,19);mapRef.current=map;L.control.zoom({position:"bottomleft"}).addTo(map);requestAnimationFrame(()=>map.invalidateSize())
 ...
 useEffect(()=>{const L=leafletRef.current;const map=mapRef.current;if(!L||!map)return;baseRef.current?.remove();setError(null)
  ...
  const url=provider==="maptiler"?MAPTILER:provider==="oam"?OAM:ESRI
  const attribution=provider==="maptiler"?'Imagery © MapTiler':provider==="oam"?'Imagery © OpenAerialMap / HOT':'Imagery © Esri and contributors'
  const layer=L.tileLayer(url,{maxZoom:21,tileSize:256,attribution}).addTo(map);baseRef.current=layer
 ...
 return <AppLayout><OrchardNavigation/><main className="relative h-[calc(100dvh-var(--orchard-nav-height,0px))] min-h-[620px] w-full overflow-hidden bg-[#11110f]">
  ...
  <div className="absolute left-1/2 top-4 z-[500] flex -translate-x-1/2 gap-1 border border-white/15 bg-[#171512]/95 p-1 shadow-xl backdrop-blur-md">
   ...
   <button type="button" onClick={()=>setProvider("oam")} className={`px-4 py-2 text-xs ${provider==="oam"?"bg-[#173328] text-[#bde1cf]":"text-[#aaa69c] hover:bg-white/[.05]"}`}>{text.oam}</button>
  </div>
