...
export default function OrchardFarmMapSatellitePage(){
 ...
 useEffect(()=>{
  ...
  for(const item of farmObjects.filter(row=>isOperationalArea(row.object_type))){
   ...
   const layer=L.polygon(farmMapRectToLatLngs(rect),{color:selected?"#f0eee7":"#d7b17a",weight:selected?2.8:1.8,opacity:.98,dashArray:selected?"":"5 4",fillColor:"#d7b17a",fillOpacity:selected?.2:.11,className:"cursor-move"})
 ...
 },[farmObjects,showBlocks,selectedBlockId,text.provisional,text.saveError,mapReady,supabase])
 ...
 const selectedBlock=farmObjects.find(row=>row.id===selectedBlockId)??null
 const previewRotation=(value:number)=>{if(!selectedBlockId)return;const rotation=clamp(Math.round(value),-180,180);setFarmObjects(current=>current.map(row=>row.id===selectedBlockId?{...row,rotation_deg:rotation}:row))}
 const saveRotation=(value:number)=>{if(!selectedBlockId)return;const rotation=clamp(Math.round(value),-180,180);void supabase.from("orchard_farm_map_objects").update({rotation_deg:rotation,updated_at:new Date().toISOString()}).eq("id",selectedBlockId).then(result=>{if(result.error)setError(text.saveError)})}
 ...
 return <AppLayout><OrchardNavigation/><main ref={workspaceRef} className="relative h-[calc(100dvh-var(--orchard-nav-height,0px))] min-h-[620px] w-full overflow-hidden bg-[#11110f]">
  ...
  <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 p-4 sm:p-5">
   ...
   <button type="button" onClick={()=>{setDrawerOpen(value=>!value);setSelectedBlockId(null)}} className="pointer-events-auto flex h-10 items-center gap-2 border border-white/15 bg-[#171512]/92 px-3 text-xs text-[#f1eee7] shadow-xl backdrop-blur-md transition hover:bg-[#211e1a]" aria-expanded={drawerOpen}><Layers3 className="h-4 w-4"/><span className="hidden sm:inline">{text.layers}</span></button>
  </div>
  {selectedBlock&&!drawerOpen?<aside className="absolute right-4 top-[70px] z-30 w-[min(340px,calc(100%-32px))] border border-white/15 bg-[#171512]/97 shadow-2xl backdrop-blur-xl sm:right-5">
   <div className="flex items-start justify-between gap-4 border-b border-white/10 px-4 py-3"><div><p className="text-sm font-medium text-[#f1eee7]">{selectedBlock.name}</p><p className="mt-0.5 text-[11px] text-[#8f8a81]">{text.field}</p></div><button type="button" onClick={()=>setSelectedBlockId(null)} className="border border-[#91c9ae]/40 bg-[#173328] px-3 py-1.5 text-xs font-medium text-[#bde1cf] hover:bg-[#1d4032]">{text.done}</button></div>
   <div className="px-4 py-4"><div className="mb-3 flex items-center justify-between"><span className="text-xs text-[#aaa69c]">{text.rotation}</span><span className="text-sm font-medium tabular-nums text-[#f1eee7]">{Math.round(n(selectedBlock.rotation_deg))}°</span></div><input type="range" min={-180} max={180} step={1} value={Math.round(n(selectedBlock.rotation_deg))} onChange={event=>previewRotation(Number(event.currentTarget.value))} onPointerUp={event=>saveRotation(Number(event.currentTarget.value))} onKeyUp={event=>saveRotation(Number(event.currentTarget.value))} onBlur={event=>saveRotation(Number(event.currentTarget.value))} className="w-full accent-[#91c9ae]" aria-label={`${text.rotation} · ${selectedBlock.name}`}/><div className="mt-2 flex justify-between text-[10px] tabular-nums text-[#746f68]"><span>-180°</span><span>0°</span><span>180°</span></div></div>
  </aside>:null}
