...
const copy={
 en:{title:"Farm Map · Satellite",subtitle:"Fundo Corcovado · canonical GIS",layers:"GIS layers",focus:"Focus farm",fullscreen:"Fullscreen",exitFullscreen:"Exit fullscreen",loading:"Loading satellite map…",error:"The satellite farm map could not be loaded.",baseError:"Satellite imagery could not be loaded.",geometry:"Canonical GIS overlays",geometryNote:"Layers start hidden so the farm stays readable. Turn on only the Corcovado GIS evidence you need; operational field blocks stay on the aerial editor until their geographic bounds are explicitly calibrated.",source:"Satellite · Esri World Imagery",close:"Close",ready:"ready",loadingState:"loading",errorState:"error",idle:"idle"},
 es:{title:"Mapa de la granja · Satélite",subtitle:"Fundo Corcovado · GIS canónico",layers:"Capas GIS",focus:"Centrar predio",fullscreen:"Pantalla completa",exitFullscreen:"Salir de pantalla completa",loading:"Cargando mapa satelital…",error:"No fue posible cargar el mapa satelital de la granja.",baseError:"No fue posible cargar la imagen satelital.",geometry:"Capas GIS canónicas",geometryNote:"Las capas parten ocultas para mantener legible la granja. Activa sólo la evidencia GIS de Corcovado que necesites; los bloques operacionales siguen en el editor aéreo hasta calibrar explícitamente sus límites geográficos.",source:"Satélite · Esri World Imagery",close:"Cerrar",ready:"lista",loadingState:"cargando",errorState:"error",idle:"pendiente"},
 de:{title:"Hofkarte · Satellit",subtitle:"Fundo Corcovado · kanonisches GIS",layers:"GIS-Ebenen",focus:"Hof zentrieren",fullscreen:"Vollbild",exitFullscreen:"Vollbild beenden",loading:"Satellitenkarte wird geladen…",error:"Die Satelliten-Hofkarte konnte nicht geladen werden.",baseError:"Das Satellitenbild konnte nicht geladen werden.",geometry:"Kanonische GIS-Ebenen",geometryNote:"Die Ebenen starten ausgeblendet, damit der Hof lesbar bleibt. Aktiviere nur die benötigten Corcovado-GIS-Daten; operative Feldblöcke bleiben im Luftbild-Editor, bis ihre geografischen Grenzen ausdrücklich kalibriert sind.",source:"Satellit · Esri World Imagery",close:"Schließen",ready:"bereit",loadingState:"lädt",errorState:"Fehler",idle:"wartet"}
} as const
...
export default function OrchardFarmMapSatellitePage(){
 ...
 return <AppLayout><OrchardNavigation/><main ref={workspaceRef} className="relative h-[calc(100dvh-var(--orchard-nav-height,0px))] min-h-[620px] w-full overflow-hidden bg-[#11110f]">
  ...
  <div className="absolute bottom-5 left-5 z-20 flex items-center gap-1.5 border border-white/15 bg-[#171512]/92 p-1 shadow-xl backdrop-blur-md"><button type="button" onClick={()=>focusFarm()} className="flex h-9 items-center gap-2 px-3 text-xs text-[#e8e5dc] hover:bg-white/[.05]"><LocateFixed className="h-4 w-4 text-[#91c9ae]"/>{text.focus}</button><span className="h-5 w-px bg-white/10"/><button type="button" onClick={()=>void toggleFullscreen()} className="flex h-9 items-center gap-2 px-3 text-xs text-[#e8e5dc] hover:bg-white/[.05]" aria-label={isFullscreen?text.exitFullscreen:text.fullscreen}>{isFullscreen?<Minimize2 className="h-4 w-4 text-[#91c9ae]"/>:<Maximize2 className="h-4 w-4 text-[#91c9ae]"/>}<span className="hidden sm:inline">{isFullscreen?text.exitFullscreen:text.fullscreen}</span></button><span className="h-5 w-px bg-white/10"/><span className="hidden px-2 text-[10px] uppercase tracking-[.1em] text-[#77726a] sm:block">{text.source}</span></div>
