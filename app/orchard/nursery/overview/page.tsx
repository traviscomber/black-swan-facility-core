...
export default function NurseryOverviewPage(){
  ...
  return <AppLayout><OrchardNavigation/><main className="mx-auto w-full max-w-[1560px] px-4 pb-16 pt-7 sm:px-6 lg:px-8">
    ...
    {loading?<div className="py-12 text-sm text-muted-foreground">…</div>:error?<div className="border border-red-300 bg-red-50 p-4 text-sm text-red-800">{error}</div>:<>
      <section className="mb-6 grid gap-px bg-[var(--bs-divider-subtle)] sm:grid-cols-2 xl:grid-cols-4"><Metric icon={Sprout} label={text.transplantPlantings} value={String(rows.length)}/><Metric icon={PackageOpen} label={text.seedLots} value={String(seedLots)}/><Metric icon={FlaskConical} label={text.batches} value={String(batches)}/><Metric icon={CircleAlert} label={text.observedGerm} value={observedGermination}/></section>

      <section className="mb-8 border border-[var(--bs-divider-subtle)] bg-[var(--bs-surface-primary)]"><div className="border-b border-[var(--bs-divider-subtle)] px-5 py-4 sm:px-6"><h2 className="text-xl font-normal">{text.projection}</h2><p className="mt-1 max-w-4xl text-xs leading-5 text-muted-foreground">{text.projectionHelp}</p></div><div className="overflow-x-auto"><div className="min-w-[760px]"><div className="grid grid-cols-[1.4fr_140px_140px_140px_1fr] border-b border-[var(--bs-divider-subtle)] bg-[var(--bs-surface-secondary)] px-5 py-2 text-[11px] uppercase tracking-[.12em] text-muted-foreground"><span>{text.container}</span><span>{text.maxProjected}</span><span>{text.totalUsage}</span><span>{text.currentUse}</span><span/></div>{(containerPlan.containers as ContainerRow[]).map(row=><div key={row.container} className="grid grid-cols-[1.4fr_140px_140px_140px_1fr] items-center border-b border-[var(--bs-divider-subtle)] px-5 py-3 text-sm last:border-b-0"><strong className="font-medium">{row.container}</strong><span className="tabular-nums">{row.max_projected}</span><span className="tabular-nums">{row.total_usage}</span><span className="tabular-nums">{row.current_in_use}</span><div className="h-2 bg-[var(--bs-surface-tertiary)]"><div className="h-full bg-[var(--orchard-green)]" style={{width:`${Math.max(1,(row.total_usage/maxUsage)*100)}%`}}/></div></div>)}</div></div></section>

      <section className="mb-7"><div className="mb-3"><p className="text-xs uppercase tracking-[.16em] text-muted-foreground">{text.schedule}</p><h2 className="mt-1 text-2xl font-normal">{rows.length} · {plan?.season??"—"}</h2></div><div className="overflow-x-auto border border-[var(--bs-divider-subtle)] bg-[var(--bs-surface-primary)]"><div className="min-w-[1200px]"><div className="grid grid-cols-[220px_135px_135px_110px_160px_105px_105px_150px_1fr] border-b border-[var(--bs-divider-subtle)] bg-[var(--bs-surface-secondary)] px-3 py-2 text-[11px] uppercase tracking-[.11em] text-muted-foreground"><span>{text.crop}</span><span>{text.sow}</span><span>{text.transplant}</span><span>{text.plants}</span><span>{text.refContainer}</span><span>{text.germDays}</span><span>{text.nurseryDays}</span><span>{text.technique}</span><span>{text.evidence}</span></div>{rows.map(({s,cycle,ref})=>{const family=familyFor(cycle.crop_name);const chip=cropChipStyle(cycle.crop_name,family);const color=cropColor(cycle.crop_name,family);return <div key={s.id} className="grid grid-cols-[220px_135px_135px_110px_160px_105px_105px_150px_1fr] items-center border-b border-[var(--bs-divider-subtle)] px-3 py-3 text-sm last:border-b-0" style={{boxShadow:`inset 3px 0 0 ${color}`}}><div><strong className="inline-flex items-center gap-2 font-medium"><i className="h-2.5 w-2.5 rounded-full" style={{backgroundColor:color}}/>{cycle.crop_name}</strong><p className="mt-1 text-xs" style={{color:chip.color}}>#{s.sequence_no}{family?` · ${family}`:""}</p></div><span>{dateLabel(s.planned_sow_date,locale)}</span><span>{dateLabel(s.planned_transplant_date,locale)}</span><span className="tabular-nums">{s.planned_plants?.toLocaleString(locale)??"—"}</span><span>{ref?.tray_cells??"—"}</span><span>{ref?`${ref.days_to_germinate} ${text.days}`:"—"}</span><span>{ref?`${ref.days_in_nursery} ${text.days}`:"—"}</span><span>{ref?.seeding_technique??"—"}</span><span className={s.germination_rate_pct?"":"text-[var(--bs-warm-amber)]"}>{s.germination_rate_pct?`${s.germination_rate_pct}%`:text.unresolved}</span></div>})}</div></div></section>
