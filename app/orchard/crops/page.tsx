...
import { Activity, ArrowRight, CalendarClock, HeartPulse, Link2, MapPin, Plus, RefreshCw, Sprout, Trash2 } from "lucide-react"
...
const localDateKey = () => new Date().toISOString().slice(0, 10)
...
const copy = {
  en: { title: "Crops", description: "Operate every live crop from one visual cockpit with lineage, care, health, work and harvest readiness in view.", newCrop: "Create live crop", succession: "Planned succession", manual: "Manual / unplanned crop", plot: "Plot", crop: "Crop", type: "Crop type", variety: "Variety", planted: "Planting date", harvest: "Expected harvest", quantity: "Quantity planted", unit: "Unit", spacing: "Spacing (cm)", depth: "Depth (cm)", water: "Water frequency", yield: "Estimated yield", yieldUnit: "Yield unit", fertilizer: "Fertilizer schedule", notes: "Notes", create: "Create crop", refresh: "Refresh", loading: "Loading…", empty: "No operational crops yet.", saveError: "Could not save crop", delete: "Delete this crop and its related logs?", actualYield: "Actual yield", actualHarvest: "Actual harvest", lineage: "Planning lineage", command: "Live crop operations", commandHelp: "See what is growing, where it lives, what happened recently and what needs attention next.", active: "Active crops", mature: "Mature / ready", healthAlerts: "Health alerts", openWork: "Open work", lastCare: "Last care", health: "Health", nextTask: "Next task", harvestIn: "Harvest in", days: "days", noCare: "No care logged", clear: "Clear", attention: "Attention", noTask: "No open task", careAction: "Open care", healthAction: "Open health", lifecycleAction: "Lifecycle", plannedLineage: "Planned", manualLineage: "Manual", fieldStatus: "Field status" },
  es: { title: "Cultivos", description: "Opera cada cultivo vivo desde un cockpit visual con trazabilidad, cuidados, sanidad, trabajo y preparación de cosecha a la vista.", newCrop: "Crear cultivo vivo", succession: "Sucesión planificada", manual: "Cultivo manual / no planificado", plot: "Sector", crop: "Cultivo", type: "Tipo", variety: "Variedad", planted: "Fecha de plantación", harvest: "Cosecha esperada", quantity: "Cantidad plantada", unit: "Unidad", spacing: "Distancia (cm)", depth: "Profundidad (cm)", water: "Frecuencia de riego", yield: "Rendimiento estimado", yieldUnit: "Unidad rendimiento", fertilizer: "Plan de fertilización", notes: "Notas", create: "Crear cultivo", refresh: "Actualizar", loading: "Cargando…", empty: "Aún no hay cultivos operativos.", saveError: "No fue posible guardar el cultivo", delete: "¿Eliminar este cultivo y sus registros relacionados?", actualYield: "Rendimiento real", actualHarvest: "Cosecha real", lineage: "Trazabilidad del plan", command: "Operación de cultivos vivos", commandHelp: "Mira qué está creciendo, dónde está, qué pasó recientemente y qué requiere atención después.", active: "Cultivos activos", mature: "Maduros / listos", healthAlerts: "Alertas sanitarias", openWork: "Trabajo abierto", lastCare: "Último cuidado", health: "Sanidad", nextTask: "Próxima tarea", harvestIn: "Cosecha en", days: "días", noCare: "Sin cuidados registrados", clear: "Sin alertas", attention: "Atención", noTask: "Sin tarea abierta", careAction: "Abrir cuidados", healthAction: "Abrir sanidad", lifecycleAction: "Ciclo", plannedLineage: "Planificado", manualLineage: "Manual", fieldStatus: "Estado en terreno" },
} as const
...
export default function OrchardCropsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [c, p, s, cy, a, b, careResult, healthResult, taskResult] = await Promise.all([
    ...
    const queryError = c.error ?? p.error ?? s.error ?? cy.error ?? a.error ?? b.error ?? careResult.error ?? healthResult.error ?? taskResult.error
    if (queryError) setError(queryError.message)
    else {
      setCrops((c.data ?? []) as Crop[]); setPlots((p.data ?? []) as Plot[]); setSuccessions((s.data ?? []) as Succession[]); setCycles((cy.data ?? []) as Cycle[]); setAllocations((a.data ?? []) as Allocation[]); setBeds((b.data ?? []) as Bed[]); setCare((careResult.data ?? []) as Care[]); setHealth((healthResult.data ?? []) as Health[]); setTasks((taskResult.data ?? []) as Task[])
    }
    setLoading(false)
  }, [supabase])
  ...
  const openTasks = tasks.filter((task) => !["completada", "cancelada"].includes(task.status))
  const activeCrops = crops.filter((crop) => !["harvested", "failed"].includes(crop.status))
  const healthAlerts = health.filter((item) => ["high", "critical"].includes(item.severity_level ?? "")).length

  function chooseSuccession(id: string) {
    if (id === "none") { setForm((current) => ({ ...current, crop_succession_id: "none" })); return }
    const succession = successionById.get(id); const cycle = succession ? cycleById.get(succession.crop_cycle_id) : null; const allocation = allocations.find((item) => item.crop_succession_id === id); const bed = allocation ? bedById.get(allocation.bed_id) : null
  ...
  return <AppLayout><PageHeader title={text.title} description={text.description} actions={<Button variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />{text.refresh}</Button>} /><OrchardNavigation /><div className="space-y-7 p-4 sm:p-8">
    {error && <Card className="border-destructive/60"><CardContent className="p-4 text-sm text-destructive">{error}</CardContent></Card>}

    <section className="relative min-h-[340px] overflow-hidden bg-neutral-950 text-white"><img src={photo("photo-1464226184884-fa280b87c399")} alt="Healthy crops in active field operations" className="absolute inset-0 h-full w-full object-cover opacity-100 [filter:none]" /><div className="absolute inset-0" style={{background:"linear-gradient(90deg,rgba(5,8,7,.94),rgba(5,8,7,.66) 58%,rgba(5,8,7,.18)),linear-gradient(0deg,rgba(5,8,7,.72),rgba(5,8,7,.05) 60%)"}}/><div className="relative flex min-h-[340px] max-w-3xl flex-col justify-end p-6 sm:p-9"><p className="text-xs uppercase tracking-[.2em] text-white/60">Orchard · Operations</p><h2 className="mt-3 text-4xl font-medium tracking-[-.04em] sm:text-5xl">{text.command}</h2><p className="mt-4 max-w-2xl text-sm leading-6 text-white/70">{text.commandHelp}</p><div className="mt-6 grid max-w-2xl grid-cols-2 gap-px bg-white/10 sm:grid-cols-4"><HeroMetric label={text.active} value={activeCrops.length} /><HeroMetric label={text.mature} value={crops.filter((crop) => crop.status === "mature").length} /><HeroMetric label={text.healthAlerts} value={healthAlerts} /><HeroMetric label={text.openWork} value={openTasks.length} /></div></div></section>

    <section><div className="mb-5"><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">01</p><h2 className="mt-2">{text.fieldStatus}</h2><p className="mt-1 text-sm text-muted-foreground">{text.commandHelp}</p></div>{loading ? <p className="text-sm text-muted-foreground">{text.loading}</p> : crops.length === 0 ? <div className="border border-dashed p-6 text-sm text-muted-foreground">{text.empty}</div> : <div className="grid gap-5 lg:grid-cols-2 2xl:grid-cols-3">{crops.map((crop) => {
      const plot = plotById.get(crop.plot_id); const cropCare = care.filter((item) => item.crop_id === crop.id); const lastCare = cropCare[0] ?? null; const cropHealth = health.filter((item) => item.crop_id === crop.id); const serious = cropHealth.filter((item) => ["high", "critical"].includes(item.severity_level ?? "")); const cropTasks = crop.crop_succession_id ? openTasks.filter((task) => task.source_id === crop.crop_succession_id) : []; const nextTask = cropTasks.find((task) => task.due_date) ?? cropTasks[0] ?? null; const harvestDays = daysUntil(crop.expected_harvest_date); const allocation = crop.crop_succession_id ? allocations.find((item) => item.crop_succession_id === crop.crop_succession_id) : null; const bed = allocation ? bedById.get(allocation.bed_id) : null
...
function Signal({ icon, label, value, danger = false }: { icon: ReactNode; label: string; value: string; danger?: boolean }) { return <div className="bg-background p-3"><div className={`flex items-center gap-2 text-[10px] uppercase tracking-[.12em] ${danger ? "text-destructive" : "text-muted-foreground"}`}>{icon}{label}</div><p className={`mt-2 line-clamp-2 text-xs ${danger ? "text-destructive" : "text-foreground"}`}>{value}</p></div> }
