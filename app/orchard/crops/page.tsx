...
export default function OrchardCropsPage() {
  ...
  const openTasks = tasks.filter((task) => !["done", "completed", "cancelled", "completada", "cancelada"].includes(normalize(task.status)))
  const activeCrops = crops.filter((crop) => !["harvested", "failed"].includes(crop.status))
  const highSeverityObservations = health.filter((item) => ["high", "critical"].includes(normalize(item.severity_level))).length
