...
export default function OrchardCropCatalogPage(){
 ...
 return <AppLayout><OrchardNavigation/><main className="mx-auto w-full max-w-[1580px] px-4 pb-16 pt-6 sm:px-6 lg:px-8">
  <header className="mb-5 flex flex-col gap-4 border-b border-[var(--orchard-line)] pb-5 lg:flex-row lg:items-end lg:justify-between">
   <div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[var(--orchard-green)]">{text.canonical}</p><h1 className="mt-1 text-3xl font-medium">{text.title}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{text.description}</p></div>
  ...
  {loading?<div className="py-12 text-sm text-muted-foreground">…</div>:error?<div className="border border-red-400/40 bg-red-950/10 p-4 text-sm">{error}</div>:filtered.length===0?<div className="border border-[var(--orchard-line)] p-10 text-sm text-muted-foreground">{text.empty}</div>:<section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
   {filtered.map(profile=>{const key=normalized(profile.crop_name);const count=countByCrop.get(key)??0;const image=photoByCrop.get(key);return <article key={profile.id} className="overflow-hidden border border-[var(--orchard-line)] bg-[var(--bs-surface-primary)] transition-colors hover:border-[var(--orchard-green)]">
    <div className="relative h-32 overflow-hidden" style={{backgroundColor:cropColor(profile.crop_name)}}>{image?<img src={image} alt="" className="h-full w-full object-cover" loading="lazy"/>:<div className="flex h-full items-center justify-center"><Sprout className="h-12 w-12 text-white/70"/></div>}<span className="absolute right-2 top-2 bg-black/55 px-2 py-1 text-[10px] text-white">{cycleLabel(profile.default_cycle_type,text)}</span></div>
    <div className="p-3"><h2 className="truncate text-base font-medium" title={profile.crop_name}>{profile.crop_name}</h2><p className="mt-1 min-h-5 truncate text-xs italic text-muted-foreground">{profile.crop_family??profile.scientific_name??(profile.days_to_maturity?`${profile.days_to_maturity} ${text.days}`:"Corcovado")}</p>
     <div className="mt-3 flex items-center justify-between gap-2 border-t border-[var(--orchard-line)] pt-3">{count>0?<Link href={calendarHref} className="inline-flex min-h-9 items-center gap-1.5 text-xs font-medium text-[var(--orchard-green)]"><ArrowRight className="h-3.5 w-3.5"/>{count} {count===1?text.planting:text.plantings}</Link>:<span className="text-xs text-muted-foreground">0 {text.plantings}</span>}<Link href={planHref} className="inline-flex min-h-9 items-center gap-1.5 bg-[var(--orchard-green)] px-2.5 text-xs font-medium text-white"><CalendarPlus className="h-3.5 w-3.5"/>{count?text.openPlan:text.add}</Link></div>
    </div>
   </article>})}
  </section>}
 </main></AppLayout>
