...
type LocationContext = { locationId: string; label: string }
...
export default function OrchardWorkPage() {
  ...
  const [bulkEmployeeId,setBulkEmployeeId] = useState("none")
  ...
  const load = useCallback(async () => {
    ...
    const [gp, t, s, c, e, a, b, p] = await Promise.all([
      ...
      supabase.from("tasks").select("id, title, priority, status, due_date, location_name, task_category, estimated_minutes, source_type, source_id, source_label").in("operational_area", ["orchard", "huerto_vinedo"]).order("due_date", { ascending: true, nullsFirst: false }),
      ...
      supabase.from("orchard_plots").select("id, name, location_id"),
    ])
    ...
    const allocatedSuccessionIds = new Set(allAllocations.map((item) => item.crop_succession_id))
    const reconciledSuccessionIds = new Set([...graph.successionIds].filter((id) => allocatedSuccessionIds.has(id)))
    const reconciledSuccessions = graph.scopedSuccessions.filter((item) => reconciledSuccessionIds.has(item.id))
    const reconciledCycleIds = new Set(reconciledSuccessions.map((item) => item.crop_cycle_id))
    const reconciledCycles = graph.scopedCycles.filter((item) => reconciledCycleIds.has(item.id))
    const allTasks = (t.data ?? []) as Task[]
    const nextTasks = requestedPlanId === ALL_GAME_PLANS ? allTasks : allTasks.filter((task) => Boolean(task.source_id && reconciledSuccessionIds.has(task.source_id)))
    ...
    setPlans(nextPlans); setSelectedPlanId(requestedPlanId); setTasks(nextTasks); setAssignments(nextAssignments); setSuccessions(reconciledSuccessions as Succession[]); setCycles(reconciledCycles as Cycle[]); setEmployees((e.data ?? []) as Employee[]); setAllocations(allAllocations.filter((item) => reconciledSuccessionIds.has(item.crop_succession_id))); setBeds((b.data ?? []) as Bed[]); setPlots((p.data ?? []) as Plot[]); setLoading(false)
  ...
  const locationContextFor = useCallback((id: string):LocationContext|null => { const allocation = allocations.find((item) => item.crop_succession_id === id); const bed = allocation ? bedById.get(allocation.bed_id) : null; const plot = bed ? plotById.get(bed.plot_id) : null; if(!bed||!plot?.location_id)return null; return {locationId:plot.location_id,label:`${plot.name} · ${bed.name}`} }, [allocations, bedById, plotById])
  const locationFor = useCallback((id: string) => locationContextFor(id)?.label ?? null, [locationContextFor])
  ...
  function prefillMilestone(item: Milestone) { setForm((current)=>({ succession_id: item.successionId, source_type: item.sourceType, title: `${item.title}: ${item.detail}`, description: "", category: item.category, due_date: item.date, priority: "media", estimated_minutes: String(item.minutes), employee_id: current.employee_id })); document.getElementById("orchard-task-form")?.scrollIntoView({ behavior: "smooth", block: "start" }) }
  async function createTask(event: FormEvent) {
    ...
    if (!succession) { setError(`${text.saveError}: ${text.successionRequired}`); return }
    if (form.employee_id === "none") { setError(`${text.saveError}: ${text.ownerRequired}`); return }
    const location=locationContextFor(succession.id)
    ...
    const result = await supabase.rpc("create_operational_task_atomic", {
      p_title:form.title.trim(), p_description:form.description.trim() || null, p_priority:form.priority, p_due_date:form.due_date,
      p_location_id:location.locationId, p_location_name:location.label, p_latitude:null, p_longitude:null, p_operational_area:"orchard",
      p_task_category:form.category.trim() || text.general, p_estimated_minutes:form.estimated_minutes ? Number(form.estimated_minutes) : null,
      p_animal_handling:false, p_safety_notes:null, p_employee_ids:[form.employee_id], p_volunteer_ids:[], p_source_type:form.source_type,
      p_source_id:succession.id, p_source_label:successionLabel(succession), p_source_path:withGamePlanQuery(`/${language}/orchard/work`, selectedPlanId),
    })
    if (result.error) { setError(`${text.saveError}: ${result.error.message}`); setSaving(false); return }
    setForm((current)=>({ succession_id: "none", source_type: "orchard_general", title: "", description: "", category: text.general, due_date: "", priority: "media", estimated_minutes: "60", employee_id: current.employee_id })); await load(); setSaving(false)
  }
  async function generateMissingTasks() {
    if (!missingMilestones.length) return
    if(bulkEmployeeId==="none"){setError(`${text.saveError}: ${text.ownerRequired}`);return}
    setSaving(true);setError(null)
    for(const item of missingMilestones){
      const location=locationContextFor(item.successionId)
      if(!location){setError(`${text.saveError}: ${item.detail}: ${text.locationRequired}`);break}
      const result=await supabase.rpc("create_operational_task_atomic",{
        p_title:`${item.title}: ${item.detail}`, p_description:null, p_priority:"media", p_due_date:item.date,
        p_location_id:location.locationId, p_location_name:location.label, p_latitude:null, p_longitude:null, p_operational_area:"orchard",
        p_task_category:item.category, p_estimated_minutes:item.minutes, p_animal_handling:false, p_safety_notes:null,
        p_employee_ids:[bulkEmployeeId], p_volunteer_ids:[], p_source_type:item.sourceType, p_source_id:item.successionId,
        p_source_label:item.detail, p_source_path:withGamePlanQuery(`/${language}/orchard/work`, selectedPlanId),
      })
      if(result.error){setError(`${text.saveError}: ${item.detail}: ${result.error.message}`);break}
    }
  ...
  return <AppLayout>
    ...
    <main className="mx-auto w-full max-w-[1560px] space-y-10 px-4 pb-16 pt-4 sm:px-6 lg:px-8">
      ...
      <section>
        <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">01</p><h2 className="mt-2">{text.milestones}</h2><p className="mt-1 text-sm text-muted-foreground">{text.milestonesHelp}</p></div><div className="flex flex-col gap-2 sm:flex-row sm:items-end"><Field label={text.bulkOwner}><Select value={bulkEmployeeId} onValueChange={setBulkEmployeeId}><SelectTrigger className="min-w-60"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">{text.unassigned}</SelectItem>{employees.map((employee) => <SelectItem key={employee.id} value={employee.id}>{employee.name}</SelectItem>)}</SelectContent></Select></Field><Button onClick={() => void generateMissingTasks()} disabled={saving || missingMilestones.length === 0 || bulkEmployeeId === "none"}><Sparkles className="mr-2 h-4 w-4" />{text.generateMissing}</Button></div></div>
