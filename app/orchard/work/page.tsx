...
export default function OrchardWorkPage() {
  ...
  const load = useCallback(async () => {
    ...
    const reconciledSuccessionIds = requestedPlanId === ALL_GAME_PLANS ? graph.successionIds : new Set([...graph.successionIds].filter((id) => allocatedSuccessionIds.has(id)))
    const reconciledSuccessions = requestedPlanId === ALL_GAME_PLANS ? graph.scopedSuccessions : graph.scopedSuccessions.filter((item) => reconciledSuccessionIds.has(item.id))
    const reconciledCycleIds = new Set(reconciledSuccessions.map((item) => item.crop_cycle_id))
    const reconciledCycles = requestedPlanId === ALL_GAME_PLANS ? graph.scopedCycles : graph.scopedCycles.filter((item) => reconciledCycleIds.has(item.id))
    ...
    setPlans(nextPlans); setSelectedPlanId(requestedPlanId); setTasks(nextTasks); setAssignments(nextAssignments); setSuccessions(reconciledSuccessions as Succession[]); setCycles(reconciledCycles as Cycle[]); setEmployees((e.data ?? []) as Employee[]); setAllocations(allAllocations.filter((item) => requestedPlanId === ALL_GAME_PLANS || reconciledSuccessionIds.has(item.crop_succession_id))); setBeds((b.data ?? []) as Bed[]); setPlots((p.data ?? []) as Plot[]); setLoading(false)
