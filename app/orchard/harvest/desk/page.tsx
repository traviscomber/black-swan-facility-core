...
export default function DietrichHarvestDesk(){
 ...
 return <AppLayout><OrchardNavigation/><main className="mx-auto w-full max-w-[1700px] px-4 pb-16 pt-7 sm:px-6 lg:px-8">
  ...
  {loading?<div className="py-12 text-sm text-muted-foreground">…</div>:error?<div className="border border-red-300 bg-red-50 p-4 text-sm text-red-800">{error}</div>:<>
   ...
   <section className="mb-7"><div className="mb-3"><p className="text-xs uppercase tracking-[.16em] text-muted-foreground">{text.matrix}</p><h2 className="mt-1 text-2xl font-normal">{cropRows.length} {text.crop.toLowerCase()} · {weeks.length} {text.weeks}</h2><p className="mt-1 max-w-4xl text-xs leading-5 text-muted-foreground">{text.matrixHelp}</p></div>
