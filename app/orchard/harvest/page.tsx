...
const CROP_PHOTOS: Record<string, CropPhoto> = {
  Tomato: { src: cropPhotoUrl("photo-1592924357228-91a4daadcfea"), alt: "Fresh ripe tomatoes at harvest" },
  Lettuce: { src: cropPhotoUrl("photo-1622206151226-18ca2c9ab4a1"), alt: "Fresh lettuce leaves ready to harvest" },
  "Bell Pepper": { src: cropPhotoUrl("photo-1563565375-f3fdfdbefa83"), alt: "Fresh bell peppers" },
  Carrot: { src: cropPhotoUrl("photo-1447175008436-170170753e16"), alt: "Fresh harvested carrots" },
  Zucchini: { src: cropPhotoUrl("photo-1597362925123-77861d3fbac7"), alt: "Fresh zucchini harvest" },
  Onion: { src: cropPhotoUrl("photo-1518977956815-dee006c00e1f"), alt: "Fresh onions after harvest" },
  Basil: { src: cropPhotoUrl("photo-1618164436241-4473940d1f5c"), alt: "Fresh basil leaves" },
  Parsley: { src: cropPhotoUrl("photo-1588879460618-9249e7d947d1"), alt: "Fresh parsley" },
  Spinach: { src: cropPhotoUrl("photo-1576045057995-568f588f82fb"), alt: "Fresh spinach leaves" },
  Arugula: { src: cropPhotoUrl("photo-1592419391068-9bd09dd58510"), alt: "Fresh arugula leaves" },
  Potato: { src: cropPhotoUrl("photo-1518977676601-b53f82aba655"), alt: "Fresh potatoes after harvest" },
  Beet: { src: cropPhotoUrl("photo-1506807803488-8eafc15316c5"), alt: "Fresh beetroot harvest" },
...
export default function OrchardHarvestPage() {
  ...
  const successionPerformance = useMemo(() => successions.map((succession) => {
    ...
    const areaCandidates = allocations.filter((item) => item.crop_succession_id === succession.id).map((item) => item.allocated_area_sqm ?? bedArea(item.id) ?? 0).filter((areaValue) => areaValue > 0)
    const area = areaCandidates.length ? areaCandidates.reduce((sum, areaValue) => sum + areaValue, 0) : succession.planned_area_sqm
    const productivity = unit && actualYield != null && area && area > 0 ? actualYield / area : null
    const photoCrop = linkedCrops[0]?.crop_name ?? cycleById.get(succession.crop_cycle_id)?.crop_name
  ...
  }).filter((item): item is NonNullable<typeof item> => Boolean(item)), [successions, crops, harvests, allocations, bedById, allocationById, cycleById])

  const varietyRows = useMemo(() => {
    const groups = new Map<string, { label: string; cropName: string; unit: string; quantity: number; passes: number }>()
    harvests.forEach((item) => { const crop = cropById.get(item.crop_id); if (!crop) return; const unit = item.harvest_unit?.trim() || "unit"; const label = cropLabel(crop); const key = `${label}|${unit}`; const current = groups.get(key) ?? { label, cropName: crop.crop_name, unit, quantity: 0, passes: 0 }; current.quantity += item.quantity_harvested ?? 0; current.passes += 1; groups.set(key, current) })
    return [...groups.values()].sort((a, b) => b.quantity - a.quantity)
  }, [harvests, cropById])

  const bedRows = useMemo(() => {
    const groups = new Map<string, { label: string; cropName: string | null; unit: string; quantity: number; passes: number; area: number | null }>()
    harvests.filter((item) => item.bed_allocation_id).forEach((item) => { const allocationId = item.bed_allocation_id as string; const unit = item.harvest_unit?.trim() || "unit"; const label = bedLabel(allocationId); const cropName = cropById.get(item.crop_id)?.crop_name ?? null; const key = `${allocationId}|${unit}`; const current = groups.get(key) ?? { label, cropName, unit, quantity: 0, passes: 0, area: bedArea(allocationId) }; current.quantity += item.quantity_harvested ?? 0; current.passes += 1; groups.set(key, current) })
  ...
  }, [harvests, allocationById, bedById, plotById, cropById])
  ...
  return <AppLayout>
    ...
    <div className="space-y-7 p-4 sm:p-8">
      ...
      <section className="relative isolate min-h-[340px] overflow-hidden border bg-neutral-950 sm:min-h-[390px]">
        <img src={HERO_PHOTO} alt="Freshly harvested produce in the field" className="absolute inset-0 h-full w-full object-cover opacity-100 [filter:none]" />
        <div className="absolute inset-0" style={{ background: "linear-gradient(90deg, rgba(8,11,10,.88) 0%, rgba(8,11,10,.55) 46%, rgba(8,11,10,.16) 100%)" }} />
        <div className="relative flex min-h-[340px] max-w-3xl flex-col justify-end p-6 text-white sm:min-h-[390px] sm:p-10">
          <p className="text-xs font-medium uppercase tracking-[0.2em] text-white/65">{text.heroEyebrow}</p>
          <h2 className="mt-3 max-w-2xl text-4xl font-medium tracking-[-0.035em] text-white sm:text-5xl">{text.heroTitle}</h2>
          <p className="mt-4 max-w-xl text-sm leading-6 text-white/75 sm:text-base">{text.heroHelp}</p>
          <div className="mt-7 grid max-w-3xl grid-cols-2 gap-2 sm:grid-cols-4">
            <HeroMetric label={text.passes} value={harvests.length.toLocaleString(locale)} />
            <HeroMetric label={text.output} value={outputLabel} />
            <HeroMetric label={text.qualityAvg} value={quality ? quality.toFixed(1) : "—"} />
            <HeroMetric label={text.value} value={value ? displayNumber(value, locale) : "—"} />
          </div>
        </div>
      </section>

      <section>
        <div className="mb-4 flex items-end justify-between gap-4"><div><p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">01</p><h2 className="mt-2 text-2xl font-medium">{text.performance}</h2><p className="mt-1 text-sm text-muted-foreground">{text.performanceHelp}</p></div><BarChart3 className="hidden h-6 w-6 text-muted-foreground sm:block" /></div>
        {successionPerformance.length === 0 ? <Card><CardContent className="p-6 text-sm text-muted-foreground">{text.noPerformance}</CardContent></Card> : <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">{successionPerformance.map((item) => { const photo = cropPhoto(item.photoCrop); return <article key={item.succession.id} className="overflow-hidden border bg-background"><div className="relative h-48 overflow-hidden bg-muted"><img src={photo.src} alt={photo.alt} loading="lazy" decoding="async" className="h-full w-full object-cover opacity-100 [filter:none]" /><div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(0,0,0,.04) 30%, rgba(0,0,0,.72) 100%)" }} /><div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-3 text-white"><div><p className="text-lg font-medium">{successionLabel(item.succession.id)}</p><p className="mt-1 text-xs text-white/70">{item.rows.length} {text.passes.toLowerCase()} · {item.area ? `${displayNumber(item.area, locale)} m²` : "—"}</p></div><Badge className="border-white/20 bg-black/45 text-white">{item.timing == null ? "—" : item.timing === 0 ? text.onTime : item.timing > 0 ? `${item.timing}d ${text.late}` : `${Math.abs(item.timing)}d ${text.early}`}</Badge></div></div><div className="grid grid-cols-2 border-t sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-2"><Mini label={text.plannedFirst} value={dateLabel(item.succession.planned_first_harvest_date, locale)} /><Mini label={text.firstHarvest} value={dateLabel(item.first, locale)} /><Mini label={text.yieldVsPlan} value={item.unit && item.actualYield != null ? `${displayNumber(item.actualYield, locale)} ${item.unit}${item.expectedYield != null ? ` / ${displayNumber(item.expectedYield, locale)} ${text.expected}` : ""}` : text.unitsMixed} /><Mini label={text.productivity} value={item.productivity != null && item.unit ? `${displayNumber(item.productivity, locale)} ${item.unit} ${text.perSqm}` : "—"} /></div></article>})}</div>}
      </section>

      <section>
        <div className="mb-4"><p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">02</p><h2 className="mt-2 text-2xl font-medium">{lang === "es" ? "Qué está rindiendo mejor" : "What is performing best"}</h2></div>
        <div className="grid gap-4 xl:grid-cols-2"><VisualRankCard title={text.variety} rows={varietyRows.map((row) => ({ label: row.label, detail: `${displayNumber(row.quantity, locale)} ${row.unit} · ${row.passes} ${text.passes.toLowerCase()}`, photo: cropPhoto(row.cropName) }))} /><VisualRankCard title={text.bed} rows={bedRows.map((row) => ({ label: row.label, detail: `${displayNumber(row.quantity, locale)} ${row.unit} · ${row.area ? `${displayNumber(row.quantity / row.area, locale)} ${row.unit} ${text.perSqm}` : `${row.passes} ${text.passes.toLowerCase()}`}`, photo: cropPhoto(row.cropName) }))} /></div>
      </section>

      <section className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <Card><CardHeader><CardTitle>{text.new}</CardTitle><CardDescription>{text.description}</CardDescription></CardHeader><CardContent><form onSubmit={createHarvest} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3"><Field label={text.date}><Input type="date" value={form.harvest_date} onChange={(e) => setForm((f) => ({ ...f, harvest_date: e.target.value }))} required /></Field><Field label={text.quantity}><Input type="number" min="0.001" step="0.001" value={form.quantity_harvested} onChange={(e) => setForm((f) => ({ ...f, quantity_harvested: e.target.value }))} required /></Field></div>
          <div className="grid grid-cols-2 gap-3"><Field label={text.unit}><Input value={form.harvest_unit} onChange={(e) => setForm((f) => ({ ...f, harvest_unit: e.target.value }))} /></Field><Field label={text.quality}><Input type="number" min="1" max="5" value={form.quality_rating} onChange={(e) => setForm((f) => ({ ...f, quality_rating: e.target.value }))} /></Field></div>
          <Field label={text.storageMethod}><Input value={form.storage_method} onChange={(e) => setForm((f) => ({ ...f, storage_method: e.target.value }))} /></Field><Field label={text.storageLocation}><Input value={form.storage_location} onChange={(e) => setForm((f) => ({ ...f, storage_location: e.target.value }))} /></Field>
          <div className="grid grid-cols-2 gap-3"><Field label={text.shelf}><Input type="number" min="0" value={form.shelf_life_days} onChange={(e) => setForm((f) => ({ ...f, shelf_life_days: e.target.value }))} /></Field><Field label={text.valueUnit}><Input type="number" min="0" step="0.01" value={form.market_value_per_unit} onChange={(e) => setForm((f) => ({ ...f, market_value_per_unit: e.target.value }))} /></Field></div>
          <Field label={text.notes}><Textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} /></Field><Button type="submit" className="w-full" disabled={saving}><Plus className="mr-2 h-4 w-4" />{text.create}</Button>
        </form></CardContent></Card>

        <div>
          <div className="mb-4"><p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">03</p><h2 className="mt-2 text-2xl font-medium">{text.recent}</h2></div>
          {loading ? <div className="h-56 animate-pulse bg-muted" /> : harvests.length === 0 ? <Card><CardContent className="p-6 text-sm text-muted-foreground">{text.empty}</CardContent></Card> : <div className="grid gap-4 md:grid-cols-2">{harvests.map((item) => { const crop = cropById.get(item.crop_id); const photo = cropPhoto(crop?.crop_name); return <article key={item.id} className="overflow-hidden border bg-background"><div className="relative h-40 overflow-hidden bg-muted"><img src={photo.src} alt={photo.alt} loading="lazy" decoding="async" className="h-full w-full object-cover opacity-100 [filter:none]" /><div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(0,0,0,.02) 25%, rgba(0,0,0,.72) 100%)" }} /><div className="absolute bottom-3 left-3 right-3 flex items-end justify-between gap-3 text-white"><div><p className="font-medium">{crop ? cropLabel(crop) : item.crop_id}</p><p className="text-xs text-white/70">{dateLabel(item.harvest_date, locale)}</p></div><Badge className="border-white/20 bg-black/45 text-white">{displayNumber(item.quantity_harvested ?? 0, locale)} {item.harvest_unit || "unit"}</Badge></div></div><div className="p-4"><div className="flex flex-wrap gap-2">{item.harvest_lot_code && <Badge variant="outline">{item.harvest_lot_code}</Badge>}{item.quality_rating != null && <Badge variant="secondary">{text.quality}: {item.quality_rating}/5</Badge>}</div><p className="mt-3 text-xs leading-5 text-muted-foreground"><Link2 className="mr-1 inline h-3 w-3" />{crop ? cropLabel(crop) : item.crop_id} → {successionLabel(item.crop_succession_id)} → {bedLabel(item.bed_allocation_id)}</p>{item.notes && <p className="mt-2 text-sm leading-5">{item.notes}</p>}<div className="mt-4 flex justify-end"><Button variant="ghost" size="icon" onClick={() => void remove(item.id)} disabled={saving}><Trash2 className="h-4 w-4" /></Button></div></div></article>})}</div>}
        </div>
      </section>
    </div>
  </AppLayout>
