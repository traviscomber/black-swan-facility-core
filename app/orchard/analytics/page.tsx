...
export default function OrchardAnalyticsPage() {
  ...
  const cues = useMemo<Cue[]>(() => {
    ...
    health.filter((item) => ["high", "critical"].includes(item.severity_level ?? "") && !["effective", "very_effective"].includes(item.treatment_effectiveness ?? "")).forEach((item) => { const crop = cropById.get(item.crop_id); result.push({ level: "attention", title: crop ? cropLabel(crop) : text.health, detail: `${text.untreated}: ${item.pest_type || item.disease_name || item.observation_date}`, href: "/orchard/pests" }) })
