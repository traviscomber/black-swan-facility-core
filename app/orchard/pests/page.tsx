...
export default function OrchardPestsPage() {
  ...
  const highSeverityCount = logs.filter((log) => log.severity_level === "critical" || log.severity_level === "high").length
  const noTreatmentCount = logs.filter((log) => !log.treatment_applied).length
  const effectiveResponseCount = logs.filter((log) => log.treatment_effectiveness === "effective" || log.treatment_effectiveness === "very_effective").length
