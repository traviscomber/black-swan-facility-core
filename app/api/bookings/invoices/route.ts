...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("generate_reservation_invoice", {
    ...
    const payload = data as { invoiceId?: string; invoiceNumber?: string; alreadyFinalized?: boolean } | null
    if (!payload?.invoiceId || !isValidUuid(payload.invoiceId)) throw new Error("El RPC no devolvió una factura final válida")

    const { data: invoice, error: invoiceError } = await supabase.from("invoices").select("*").eq("id", payload.invoiceId).maybeSingle()
    if (invoiceError) throw invoiceError
    if (!invoice) throw new Error("La factura final no pudo recuperarse")

    return NextResponse.json(invoice, { status: payload.alreadyFinalized ? 200 : 201 })
  } catch (error) {
    console.error("[invoices] final generation failed", error)
