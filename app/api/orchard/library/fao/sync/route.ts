...
function parseCatalog(csv:string){
  ...
  const nameIndex=pickIndex(headers,["title_en","label_en","name_en","common_name","vernacular_name","title","label","name"],["vernacular","common","title_en","label_en","name_en"])
  const scientificIndex=pickIndex(headers,["scientific_name","scientificname","dwc_scientificname"],["scientific"])
  const iccIndex=pickIndex(headers,["icc_1_1_code","icc11_code","icc_code"],["icc_1_1","icc11"])
  if(nameIndex<0)throw new Error("FAO WCA crop-list schema changed")
  const seen=new Set<string>();const items:CatalogItem[]=[]
  for(let index=1;index<rows.length;index+=1){const row=rows[index];const name=(row[nameIndex]??"").trim();if(!name)continue;const key=name.toLowerCase();if(seen.has(key))continue;seen.add(key);items.push({externalId:(codeIndex>=0?(row[codeIndex]??"").trim():"")||`wca-${index}`,name,scientificName:scientificIndex>=0?(row[scientificIndex]??"").trim()||null:null,iccCode:iccIndex>=0?(row[iccIndex]??"").trim()||null:null})}
  return items
}
