  ...
  try {
    ...
    const [metricsResult, dependenciesResult, timelineResult] = await Promise.all([
      supabase
        ...
        .select("category, metric_name, unit, current_value, target_value, self_sufficiency_percentage, last_updated, notes")
        .order("category", { ascending: true }),
      supabase
        .from("sovereignty_dependencies")
        .select("category, dependency_name, status, risk_level, criticality, mitigation_strategy")
        .eq("status", "active")
        .order("risk_level", { ascending: false }),
      supabase
        .from("sovereignty_timeline")
        .select("event_date, event_type, title, description, impact_area, before_percentage, after_percentage")
    ...
    if (dataError) {
      ...
      return Response.json({ error: "Unable to load authorized sovereignty data" }, { status: 500 })
    }
    ...
    const overallAverage = metrics.length
      ? metrics.reduce((sum, metric) => sum + Number(metric.self_sufficiency_percentage ?? 0), 0) / metrics.length
      : 0
