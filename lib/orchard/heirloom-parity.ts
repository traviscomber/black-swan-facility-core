...
// Historical field-study evidence. These values describe what was observed in
// authenticated Heirloom onboarding and must not be treated as current Core layout.
...
// Current Black Swan Core operational layout confirmed on 2026-09-01.
export const CORE_FARM_AREA_1_LAYOUT_REFERENCE = {
  effectiveDate: "2026-09-01",
  geometryStatus: "logical_block_membership_only",
  currentBlockCount: 5,
  expansionBlockCount: 3,
  blockCount: 8,
  bedsPerBlock: 10,
  bedLengthM: 10,
  bedWidthM: 0.76,
  pathWidthM: 0.46,
  totalBeds: 80,
  totalCapacityBedMeters: 800,
  totalCultivableAreaSqm: 608,
  reconciledPlantings: 32,
  allocatedPlantings: 32,
  allocatedBedMeters: 744,
  canonicalPeakBedMeters: 702,
  peakBufferBedMeters: 98,
  blocks: [
    { name: "Current 01", kind: "current", beds: 10, bedLengthM: 10 },
    { name: "Current 02", kind: "current", beds: 10, bedLengthM: 10 },
    { name: "Current 03", kind: "current", beds: 10, bedLengthM: 10 },
    { name: "Current 04", kind: "current", beds: 10, bedLengthM: 10 },
    { name: "Current 05", kind: "current", beds: 10, bedLengthM: 10 },
    { name: "Expansion 01", kind: "expansion", beds: 10, bedLengthM: 10 },
    { name: "Expansion 02", kind: "expansion", beds: 10, bedLengthM: 10 },
    { name: "Expansion 03", kind: "expansion", beds: 10, bedLengthM: 10 },
  ],
} as const
...
Historical reference physical structure: Farm Area 1 contained Field Block "Orchard BlackSwan Campo" with 18 beds of 30 m during the Heirloom field study. This is not the current Core physical layout.
...
Capacity finding from the 32 reference plantings: total requested bed-meter work = ${HEIRLOOM_REFERENCE_TOTAL_BED_METERS} bed m; historical reference instantaneous capacity = ${HEIRLOOM_REFERENCE_PHYSICAL_CAPACITY_BED_METERS} bed m; peak concurrent demand = ${HEIRLOOM_REFERENCE_PEAK_BED_METERS} bed m on ${HEIRLOOM_REFERENCE_PEAK_DATE}. Treat this as field-study evidence, not current Core capacity.

CURRENT CORE FARM AREA 1 LAYOUT (${CORE_FARM_AREA_1_LAYOUT_REFERENCE.effectiveDate})
Farm Area 1 is currently modeled as ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.blockCount} canonical field blocks: ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.currentBlockCount} current blocks and ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.expansionBlockCount} expansion blocks. Every block has ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.bedsPerBlock} beds of ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.bedLengthM} m × ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.bedWidthM} m, with ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.pathWidthM} m path standard recorded separately. Total physical capacity = ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.totalCapacityBedMeters} bed m across ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.totalBeds} beds, with ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.totalCultivableAreaSqm} m² cultivable bed area. The reconciled 2026/27 plan contains ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.allocatedPlantings}/${CORE_FARM_AREA_1_LAYOUT_REFERENCE.reconciledPlantings} allocated plantings and ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.allocatedBedMeters} allocated bed m; canonical plan peak = ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.canonicalPeakBedMeters} bed m, leaving ${CORE_FARM_AREA_1_LAYOUT_REFERENCE.peakBufferBedMeters} bed m of aggregate peak buffer. Current live claims must still be confirmed by ORCHARD_SNAPSHOT. Per-block polygon geometry is not yet stored; do not fabricate coordinates.
`.trim()
