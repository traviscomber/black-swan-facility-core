...
function template(notification: Notification) {
  ...
  switch (notification.notification_type) {
    ...
    default:
      throw new DeliveryError('unsupported_notification_type')
  }
}
...
async function runBatch(env: Env) {
  ...
  const claimed = await supabaseRpc(env, 'claim_event_notifications', {
    p_machine_token: env.NOTIFICATION_MACHINE_TOKEN,
    p_limit: limit,
  }) as Notification[]
  ...
  for (const notification of claimed || []) {
    try {
      ...
      try { await complete(env, notification, false, undefined, message) } catch (completeError) {
        console.error(JSON.stringify({ level: 'error', service: 'black-swan-event-notifications', notification_id: notification.id, stage: 'complete_failure', error: String(completeError) }))
      }
      failed += 1
    }
  }
...
export default {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === 'GET' && url.pathname === `/${version}/health`) {
      return Response.json({ status: 'ok', service: 'black-swan-event-notifications', environment: env.ENVIRONMENT || 'unknown' })
    }
