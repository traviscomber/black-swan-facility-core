...
alter table public.invoice_payments
  ...
  add constraint invoice_payments_amount_clp check (amount > 0 and amount = trunc(amount)),
...
begin
  ...
  if nullif(btrim(p_payment_method), '') is null then raise exception 'payment_method is required'; end if;
  ...
  if v_balance <= 0 then raise exception 'Invoice has no outstanding balance'; end if;
  ...
  insert into public.invoice_payments (
    ...
    payment_date, notes, created_by, idempotency_key
  ) values (
    p_invoice_id, p_amount, btrim(p_payment_method), nullif(btrim(p_transaction_id), ''),
    coalesce(p_payment_date, now()), nullif(btrim(p_notes), ''), v_created_by,
...
grant execute on function public.register_invoice_payment(uuid, numeric, text, text, text, text, timestamptz) to authenticated, service_role;
revoke all on function public.set_invoice_lifecycle(uuid, text, date, text) from public;
grant execute on function public.set_invoice_lifecycle(uuid, text, date, text) to authenticated, service_role;
