...
begin
  ...
  begin
    ...
    if v_action='create_task' then
      ...
      values(v_title,nullif(trim(v_payload->>'description'),''),coalesce(nullif(v_payload->>'priority',''),'media'),nullif(v_payload->>'due_date','')::date,v_location_id,'huerto_vinedo','orchard_ai',v_minutes,null,'Orchard AI approved action','/orchard/assistant',v_employee_id)
