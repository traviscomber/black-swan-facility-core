...
begin
  ...
  select * into v_upload from public.finance_sii_uploads where id = p_upload_id for update;
  ...
  if v_supplier_name is null or v_supplier_rut is null or v_document_number is null or v_document_date is null or v_total is null or v_total < 0 then raise exception 'Proveedor, RUT, folio, fecha y total son obligatorios'; end if;
  ...
  insert into public.finance_documents(
    ...
    coalesce(v_upload.parsed_payload,'{}'::jsonb) || p_metadata || jsonb_build_object(
      'extraction_method','manual_pdf_metadata','sii_upload_id',v_upload.id,'storage_bucket',v_upload.storage_bucket,
      'storage_path',v_upload.storage_path,'original_filename',v_upload.original_filename
    ),
    v_approval_status
  ...
  values('finance_document',v_document_id,'INSERT','finance',p_actor_id,
    jsonb_build_object('operation','sii_pdf_metadata_completed','upload_id',v_upload.id,'document_type',v_document_type,'supplier_rut',v_supplier_rut,'approval_status',v_approval_status,'classification_status',v_classification_status),
