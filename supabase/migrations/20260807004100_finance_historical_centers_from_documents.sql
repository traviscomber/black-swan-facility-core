-- Every raw historical label appearing in an approval document must exist in the mapping queue,
-- even when it was not present in the workbook's catalog sheet.

insert into public.finance_historical_cost_centers(source_workbook_hash,historical_label,header_frequency)
select distinct fd.source_payload->>'source_workbook_hash', fd.source_payload->>'historical_cost_center', 0
from public.finance_documents fd
where fd.external_source='raimundo_workbook'
  and nullif(fd.source_payload->>'source_workbook_hash','') is not null
  and nullif(fd.source_payload->>'historical_cost_center','') is not null
on conflict (source_workbook_hash,historical_label) do nothing;
...
declare
  ...
  v_label text;
begin
  if v_role not in ('admin','approver','service_role') then raise exception 'Not authorized to import finance workbook'; end if;
  ...
  for v_center in select value from jsonb_array_elements(p_centers) loop
    ...
    on conflict (source_workbook_hash,historical_label) do update set header_frequency=excluded.header_frequency,updated_at=now()
  ...
  for v_rule in select value from jsonb_array_elements(p_rules) loop
    ...
    on conflict (source_workbook_hash,historical_label) do update set updated_at=now()
    returning id into v_center_id;
