-- Manual SII invoice intake: private source files -> deterministic fiscal metadata -> canonical Finance queue.

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values (
  'finance-sii-invoices',
  'finance-sii-invoices',
  false,
  15728640,
  array['application/pdf','application/xml','text/xml','text/plain']
)
...
create table if not exists public.finance_sii_uploads (
  id uuid primary key default gen_random_uuid(),
  file_hash text not null unique,
  storage_bucket text not null default 'finance-sii-invoices',
  storage_path text not null unique,
  original_filename text not null,
  mime_type text not null,
  size_bytes bigint not null check (size_bytes > 0 and size_bytes <= 15728640),
  upload_kind text not null check (upload_kind in ('pdf','xml')),
  status text not null default 'received' check (status in ('received','needs_metadata','classified','linked','duplicate','failed')),
  parsed_payload jsonb not null default '{}'::jsonb,
  finance_document_id uuid references public.finance_documents(id) on delete restrict,
...
create index if not exists finance_sii_uploads_document_idx
  on public.finance_sii_uploads(finance_document_id)
  where finance_document_id is not null;
create index if not exists finance_sii_uploads_created_idx
  on public.finance_sii_uploads(created_at desc);

alter table public.finance_sii_uploads enable row level security;

drop policy if exists finance_sii_uploads_read on public.finance_sii_uploads;
create policy finance_sii_uploads_read
on public.finance_sii_uploads
for select
to authenticated
using (public.can_app_action('finance.adjust'));
...
create or replace function public.register_sii_finance_upload(
  p_file_hash text,
  p_storage_path text,
...
declare
  ...
  v_existing_upload public.finance_sii_uploads%rowtype;
...
begin
  ...
  begin v_tax := nullif(p_parsed_payload->>'tax_amount','')::numeric; exception when others then v_tax := null; end;
