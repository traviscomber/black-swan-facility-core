...
begin
  ...
  if v_row.status<>'pending' then
    return jsonb_build_object('proposal_id',p_proposal_id,'status',v_row.status,'error','Proposal is not pending');
  end if;
  ...
  begin
    ...
    if v_action='create_task' then
      ...
      values(v_title,nullif(trim(v_payload->>'description'),''),coalesce(nullif(v_payload->>'priority',''),'media'),nullif(v_payload->>'due_date','')::date,v_location_id,'huerto_vinedo','orchard_ai',v_minutes,null,'Orchard AI approved action','/orchard/assistant',auth.uid()) returning id into v_result_id;
    ...
    set status='executed',executed_at=now(),updated_at=now(),execution_result=jsonb_build_object('entity_id',v_result_id,'action_type',v_action),error_message=null
  ...
  exception when others then
    ...
    where id=p_proposal_id and status in ('pending','approved');
