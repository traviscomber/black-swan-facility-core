-- Align the housekeeping inspection workflow with reservation_room_readiness.
-- The canonical operation function transitions inspection-required tasks through
-- an explicit `inspection` state and records auth user / employee identities
-- in their respective foreign-key domains.
...
create or replace function public.update_housekeeping_task_operation(
  ...
  p_quality_score integer default null::integer
)
...
begin
  ...
  if not public.can_app_action('housekeeping.operate') then raise exception 'housekeeping operation permission required'; end if;
  ...
  if p_action='assign' then
    ...
    set assigned_to=p_assigned_to,status='assigned',updated_at=v_now
    where id=p_task_id;
    ...
    update public.housekeeping_tasks
    set status=case when requires_inspection then 'inspection' else 'completed' end,
        completed_at=v_now,
        actual_duration_minutes=case
          when started_at is null then actual_duration_minutes
          else greatest(1,round(extract(epoch from (v_now-started_at))/60)::int)
        end,
    ...
    if v_task.room_id is not null then
      ...
      where id=v_task.room_id
        and operational_status not in ('out_of_service','maintenance');
    end if;
