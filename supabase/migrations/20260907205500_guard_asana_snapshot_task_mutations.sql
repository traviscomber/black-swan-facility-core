create or replace function public.update_operational_task_atomic(p_task_id uuid, p_title text, p_description text default null::text, p_priority text default 'media'::text, p_status text default 'nueva'::text, p_due_date date default null::date, p_location_id uuid default null::uuid, p_location_name text default null::text, p_latitude double precision default null::double precision, p_longitude double precision default null::double precision, p_operational_area text default null::text, p_task_category text default null::text, p_estimated_minutes integer default null::integer, p_animal_handling boolean default false, p_safety_notes text default null::text, p_employee_ids uuid[] default '{}'::uuid[], p_volunteer_ids uuid[] default '{}'::uuid[], p_source_type text default null::text, p_source_id uuid default null::uuid, p_source_label text default null::text, p_source_path text default null::text)
returns void
language plpgsql
security definer
...
declare
  ...
  v_old_task_category text;
  v_old_source_label text;
  v_changed_by uuid;
begin
  ...
  select status, operational_area, location_id, task_category, source_label
    into v_old_status, v_old_operational_area, v_old_location_id, v_old_task_category, v_old_source_label
  from public.tasks
  where id=p_task_id
  for update;
  if not found then raise exception 'Tarea no encontrada'; end if;

  if coalesce(v_old_task_category,'') like 'asana_import%' or coalesce(v_old_source_label,'') like 'Asana ·%' then
    raise exception 'Tarea importada desde Asana: concilia su estado vivo en Asana antes de modificarla en Black Swan';
  end if;
