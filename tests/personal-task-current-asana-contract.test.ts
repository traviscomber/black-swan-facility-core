...
const identityMigration = fs.readFileSync("supabase/migrations/20260908023000_asana_identity_links.sql", "utf8")
...
test("personal tasks expose refresh and only current Asana intake", () => {
  ...
  assert.match(myTasks, /asana_identity_links/)
...
test("employee to Asana identity is stored canonically and self-readable", () => {
  assert.match(identityMigration, /create table if not exists public\.asana_identity_links/)
  assert.match(identityMigration, /asana_current_tasks_self_read/)
  assert.match(identityMigration, /travis@blackswn\.org/)
  assert.match(identityMigration, /raimundo@blackswn\.org/)
})
