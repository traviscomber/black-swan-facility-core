...
test("reservation object is the connected hospitality object graph", () => {
  ...
  assert.match(reservationObjectView, /href={`\/bookings\/rooms\/\$\{room\.id\}`}/)
...
test("object pages degrade partial related-data failures without fabricating state", () => {
  ...
  assert.match(reservationObjectView, /Parte del contexto relacionado no pudo cargarse/)
  assert.match(roomObjectView, /sin señal/)
  assert.match(reservationObjectView, /sin señal/)
})
