...
test("field dashboard uses Asana as the primary task source with system fallback", () => {
  assert.match(fieldAdminHome, /asana_identity_links/)
  assert.match(fieldAdminHome, /asana_sync_baselines/)
  assert.match(fieldAdminHome, /asana_current_tasks/)
  assert.match(fieldAdminHome, /task_external_refs/)
  assert.match(fieldAdminHome, /\.eq\('last_seen_at', latestSeenAt\)/)
  assert.match(fieldAdminHome, /\.gte\('created_at_source', cutoverAt\)/)
  assert.match(fieldAdminHome, /isHistoricalAsanaTask/)
  assert.match(fieldAdminHome, /isDemoTask/)
  assert.match(fieldAdminHome, /normalizeTitle/)
  assert.match(fieldAdminHome, /source: 'asana'/)
  assert.match(fieldAdminHome, /source: 'system'/)
  assert.match(fieldAdminHome, /matchBasis = 'exact_title'/)
  assert.match(fieldAdminHome, /syncRequired: Boolean\(identity\?\.asana_email \|\| identity\?\.asana_user_gid\)/)
  assert.match(fieldAdminHome, /Asana es la fuente principal/)
  assert.match(fieldAdminHome, /Créala en Asana para mantener estado y vencimiento sincronizados/)
  assert.match(fieldAdminHome, /Black Swan no la crea automáticamente/)
  assert.match(fieldAdminHome, /Crear en Asana/)
  assert.doesNotMatch(fieldAdminHome, /\.insert\(/)
  assert.doesNotMatch(fieldAdminHome, /\.update\(/)
})

test("field dashboard keeps Asana refresh available without turning the dashboard into an Asana writer", () => {
  assert.match(fieldAdminHome, /hasNavKey\(nav, 'asana-live'\)/)
  assert.match(fieldAdminHome, /fetch\('\/api\/asana-live\/sync', \{ method: 'POST' \}\)/)
  assert.match(fieldAdminHome, /Actualizar Asana/)
  assert.match(fieldAdminHome, /ASANA_MY_TASKS_URL/)
  assert.doesNotMatch(fieldAdminHome, /api\/1\.0\/tasks/)
})

test("field dashboard separates tasks from maintenance housekeeping and issue triage", () => {
  ...
  assert.match(fieldAdminHome, /<h2 className="text-lg font-semibold">Tareas<\/h2>/)
  assert.match(fieldAdminHome, /<h2 className="text-lg font-semibold">Operación asignada<\/h2>/)
  assert.match(fieldAdminHome, /quedan separados de la cola de tareas/)
})

test("field desktop puts invoice decisions before Asana-first tasks, operations, field exceptions and workspaces", () => {
  const financeIndex = fieldAdminHome.indexOf('<h2 className="text-lg font-semibold">Facturas por aprobar</h2>')
  const tasksIndex = fieldAdminHome.indexOf('<h2 className="text-lg font-semibold">Tareas</h2>')
  const operationsIndex = fieldAdminHome.indexOf('<h2 className="text-lg font-semibold">Operación asignada</h2>')
  ...
  assert.ok(tasksIndex > financeIndex)
  assert.ok(operationsIndex > tasksIndex)
  assert.ok(attentionIndex > operationsIndex)
